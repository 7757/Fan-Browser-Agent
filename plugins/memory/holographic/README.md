...
```bash
fan memory setup    # select "holographic"
```

Or manually:
```bash
fan config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.fan-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$FAN_HOME/memory_store.db` | SQLite database path |
