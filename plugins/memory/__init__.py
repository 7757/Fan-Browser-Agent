...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with fan-agent)
2. User-installed providers: ``$FAN_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_fan_user_memory"
...
def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    ...
    User-installed providers import as ``_fan_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_fan_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$FAN_HOME/plugins/`` or None if unavailable."""
    try:
        from fan_constants import get_fan_home
        d = get_fan_home() / "plugins"
...
def is_valid_memory_provider_name(name: object) -> bool:
    """Return whether *name* is safe to use as a provider directory name.

    Provider names originate in config and desktop API payloads, then flow to
    :func:`find_provider_dir`. Keep the path boundary here so a malformed or
    traversal-shaped persisted value cannot escape either provider root.
    """
    return isinstance(name, str) and bool(_MEMORY_PROVIDER_NAME_RE.fullmatch(name))
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    """
    if not is_valid_memory_provider_name(name):
        return None
    # Bundled
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$FAN_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _fan_user_memory.<name>.cli, usually
