...
Fan pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.fan/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<fan_home>/bin/bws`` on
  first use.  Fan pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.fan/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``fan`` invocations don't
  hammer the API.
* Failures NEVER block Fan startup.  Missing binary, no network,
...
# In-process cache so repeated load_fan_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `fan chat -q ...`
...
# <fan_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.fan/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under fan_home/cache/.

    `home_path` is what `load_fan_dotenv()` already resolved; falling back
    to `$FAN_HOME` / `~/.fan` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("FAN_HOME", Path.home() / ".fan"))
...
def _fan_bin_dir() -> Path:
    """Where Fan stores its managed binaries.  Profile-aware."""
    from fan_constants import get_fan_home

    return get_fan_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<fan_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _fan_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``fan secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _fan_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="fan-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "fan-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<fan_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
def _run_bws_list(
    ...
    # bws needs exactly its bootstrap token (and optional endpoint), not every
    # provider/tool credential currently loaded into the Fan process.
    env = fan_subprocess_env(inherit_provider_credentials=False)
    ...
    if proc.returncode != 0:
        ...
        from agent.redact import redact_sensitive_text
        err = redact_sensitive_text(err, force=True)
        raise RuntimeError(
...
# ---------------------------------------------------------------------------
# Public entry point — called from fan_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_fan_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `fan secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `fan secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `fan secrets bitwarden setup` to install."
        )
        return result
