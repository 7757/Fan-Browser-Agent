...
from fan_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from fan_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _validated_openrouter_provider_sort(raw_sort: Any) -> Optional[str]:
    """Return a supported OpenRouter provider.sort value, or omit it."""
...
# A fully exhausted fallback chain should not be replayed on every immediately
# following user turn.  Rate-limit/billing paths already arm a longer cooldown;
# this protects the non-rate-limit path (for example repeated 400s) from
# re-marshalling the same large context through every provider again.
...
# A provider can accept a request yet never deliver a response.  Per-call
# watchdogs bound one attempt, but a long-lived desktop session would still
# spend the full stale timeout again on every following turn.  Keep the streak
# on the agent, reset it only after a real response or a provider change, and
# stop retrying a demonstrably wedged route across turns.
...
def _check_stale_giveup(agent) -> None:
    """Fail fast once repeated stale calls prove the active route is wedged."""
    giveup = env_int("FAN_STREAM_STALE_GIVEUP", 5)
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # Quiet-mode, subagent, and headless turns use this non-streaming path;
    # they need the same cross-turn protection as streamed conversations.
    ...
    # activity. Operators can tune via FAN_CODEX_TTFB_TIMEOUT_SECONDS and
    # FAN_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # FAN_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("FAN_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("FAN_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 25_000.0)
        _ttfb_strict = os.environ.get("FAN_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set FAN_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("FAN_CODEX_TTFB_MAX_SECONDS", 120.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set FAN_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "FAN_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # Project large, old tool results only in the provider-bound copy.  The
    # mapping is frozen per user turn so multi-step tasks keep a stable prefix
    # and retain the provider's implicit-cache discount.  Canonical history,
    # SQLite, resume, and the desktop transcript remain lossless.
    projector = getattr(agent, "_api_context_projector", None)
    if projector is None:
        from agent.api_context_projector import TurnStableToolResultProjector

        projector = agent._api_context_projector = TurnStableToolResultProjector()
    api_messages = projector.project(
        api_messages,
        turn_id=getattr(agent, "_current_turn_id", None),
        compressor=getattr(agent, "context_compressor", None),
    )

    # 折叠历史中【过时的浏览器页面观测】(保留最新一次完整,更早的每次 → 固定存根)。放在这里是
    # 因为这是 bedrock / codex / chat 三条 provider 支路的【唯一汇聚点】—— 一处覆盖全部。
    # 只作用于本次
    # 调用的 api_messages 副本,确定性 + 单调 ⇒ prompt 缓存安全。见 agent/stale_observation_collapser.py。
    collapser = getattr(agent, "_observation_collapser", None)
    if collapser is None:
        from agent.stale_observation_collapser import StaleBrowserObservationCollapser
        collapser = agent._observation_collapser = StaleBrowserObservationCollapser()
    api_messages = collapser.collapse(api_messages)
    ...
    if agent.api_mode == "codex_responses":
        ...
        # Fan removed the GitHub Models / Copilot provider integration. Keep
        # the generic Responses transport without forwarding upstream-only
        # provider hooks that no longer exist in the Fan runtime.
        ...
        # agent (auxiliary task or provider fallback,
    ...
    # LM Studio is not a Fan product provider. Do not retain partial upstream
    # behavior that depends on helper methods removed from Fan.
    _is_lmstudio = False
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "fan",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects FAN_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
    ...
    # (glm, MiniMax, gpt-5.x via compatible gateways)
    ...
    if hasattr(assistant_message, 'reasoning_details') and assistant_message.reasoning_details:
        ...
        # providers can maintain reasoning continuity across turns.
    ...
    if assistant_tool_calls:
        ...
        for tool_call in assistant_tool_calls:
            ...
            # Tool-call arguments intentionally remain raw in this in-memory
            # history shape. The model replays it on later turns (and after a
            # resumed session); replacing a credential with ``***`` here makes
            # the model copy that placeholder into its next command and breaks
            # otherwise valid authenticated work. This pass also cannot
            # provide end-to-end secrecy because the same value may appear in
            # tool output. Persistence/telemetry redaction remains handled at
            # their own explicit boundaries.
...
def rewrite_prompt_model_identity(agent, model: str, provider: str) -> None:
    """Align cached prompt identity with the runtime serving the response.

    Only final identity lines are touched because earlier matching text can
    come from user memory/context. This does not persist the rewritten prompt.
    """
    ...
    for label, value in (("Model", model), ("Provider", provider)):
        if not value:
            continue
        matches = list(re.finditer(rf"(?m)^{label}: .*$", system_prompt))
        if not matches:
            continue
        last = matches[-1]
        system_prompt = (
            f"{system_prompt[:last.start()]}{label}: {value}"
            f"{system_prompt[last.end():]}"
        )
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    if agent._fallback_index >= len(agent._fallback_chain):
        # Keep the current fallback active briefly after a non-capacity chain
        # exhaustion.  On the next turn restore_primary_runtime observes this
        # cooldown and leaves the exhausted index intact, preventing a fast
        # client re-submit from replaying every failed provider.
        if (
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in fan_cli/config.py).
        ...
        # Rebind pool recovery when fallback changes providers.  The primary
        # pool must never be used to recover a fallback request because a
        # rotated primary credential would overwrite the fallback endpoint.
        # Conversely, a configured pool for the fallback must remain usable
        # for its own 401/429 recovery.
        ...
        if getattr(agent, "_credential_pool", None) is None:
            try:
                ...
                fallback_pool_key = fb_provider
                if fb_provider == "custom":
                    fallback_pool_key = get_custom_provider_pool_key(fb_base_url) or fb_provider
                fallback_pool = load_pool(fallback_pool_key)
                if fallback_pool is not None and fallback_pool.has_credentials():
        ...
        # The fallback is now the runtime that will answer this retry.
        rewrite_prompt_model_identity(agent, fb_model, fb_provider)

        getattr(agent, "_buffer_fallback_status", agent._buffer_status)(
        ...
        # The stale streak belongs to the provider we just left.  Give the
        # freshly activated fallback a real attempt before the breaker applies.
        _reset_stale_streak(agent)
        return True
    except Exception as e:
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        history_overlays, history_messages = partition_privileged_messages(messages)
        api_messages = []
        for msg in history_messages:
            ...
            # replay-only timestamp metadata, and every Fan-internal
            # underscore-prefixed scaffolding key.
        ...
        # turns so strict providers do not reject the summary call.
        ...
        # Fan does not expose the upstream LM Studio provider.
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    Handles the supported API modes:
    ...
    def _call_chat_completions():
        ...
        # wins over the FAN_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("FAN_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # FAN_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("FAN_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode FAN_STREAM_READ_TIMEOUT.
        ...
        # Some OpenAI-compatible endpoints accept ``stream=True`` but return
        # a completed response object. Treat every object exposing ``choices``
        # as that non-streaming shape, including empty/None choices: iterating
        # it would otherwise raise and bypass the normal invalid-response
        # recovery path.
        ...
        # A text-only stream can also end mid-response without a protocol
        # terminator or finish_reason. Treating that clean socket close as
        # "stop" presents truncated text as a complete answer. Route it
        # through the existing partial-stream continuation path instead.
    ...
    def _call():
        ...
        _max_stream_retries = env_int("FAN_STREAM_RETRIES", 2)
        ...
        try:
            for _stream_attempt in range(_max_stream_retries + 1):
                ...
                try:
                    ...
                    if _is_timeout or _is_conn_err or _is_sse_conn_err or _is_empty_stream:
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("FAN_STREAM_STALE_TIMEOUT", 180.0)
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set FAN_STREAM_STALE_TIMEOUT.
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        _stream_stale_timeout = float("inf")
        logger.debug("Local provider detected (%s) — stale stream timeout disabled", agent.base_url)
    else:
        # Preserve an explicit provider/model or environment override.  With
        # Fan's implicit default, known reasoning models need a longer first
        # token window than ordinary chat models; otherwise a healthy thinking
        # phase becomes a client-created transport failure.
        _reasoning_floor = None
        if _cfg_stale is None and os.getenv("FAN_STREAM_STALE_TIMEOUT") is None:
            from agent.reasoning_timeouts import get_reasoning_stale_timeout_floor

            _reasoning_floor = get_reasoning_stale_timeout_floor(agent.model)
        # Scale the stale timeout for large contexts: slower models
        ...
        if _reasoning_floor is not None:
            _stream_stale_timeout = max(_stream_stale_timeout_base, _reasoning_floor)
    ...
    # The worker can exit after observing an interrupt before this polling
    # loop sees it. Re-check at the handoff so /stop never turns into a normal
    # partial/empty response merely because the thread finished first.
    if agent._interrupt_requested:
    ...
    if result["error"] is not None:
        if deltas_were_sent["yes"]:
            ...
            # The partial-stream wrapper normally hides the underlying error.
            # Preserve content-filter classification on the stub so the
            # conversation loop can try a configured fallback instead of
            # repeatedly continuing against a provider that will deterministically
            # stop the same output.
            content_filter_terminated = False
            try:
            ...
            if content_filter_terminated:
                stub._content_filter_terminated = True
            # A partial response proves the provider delivered data, even if
            # its stream later failed. Do not keep a stale-route latch here.
            _reset_stale_streak(agent)
            return stub
        raise result["error"]
    if result["response"] is not None:
