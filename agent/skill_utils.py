...
from fan_constants import get_config_path, get_skills_dir, is_termux
...
def is_skill_support_path(path) -> bool:
    """Return whether *path* is below a support directory of a real skill.

    A top-level category may itself be named ``scripts`` or ``references``;
    only a support directory directly below a directory containing SKILL.md is
    excluded from active discovery.
    """
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``FAN_KANBAN_TASK`` / ``FAN_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("FAN_KANBAN_TASK") or os.getenv("FAN_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Fan Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``fan_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit local interface name (e.g. ``"cron"``). When
            *None*, resolves from ``FAN_PLATFORM`` or
            ``FAN_SESSION_PLATFORM`` env vars.  Returns the global
            disabled list, unioned with the interface-specific list when an
            interface is resolved (a globally-disabled skill stays disabled
            everywhere).
...
# which becomes the dominant cost of ``fan`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.fan/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``fan`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against FAN_HOME, not cwd
        if not p.is_absolute():
            p = (fan_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.fan/skills/`` first, then external.
...
def _resolve_for_skill_ownership(path) -> Path:
    """Resolve a candidate without turning an unreadable path into a crash."""
    path_obj = path if isinstance(path, Path) else Path(str(path))
    try:
...
def is_external_skill_path(path) -> bool:
    """Whether ``path`` belongs to a configured externally-owned skills dir.

    Fan can discover external skills and foreground user-directed changes may
    still target them.  Autonomous curator work must treat them as read-only,
    so ownership is determined by this single path boundary rather than by a
    name or a fragile discovery order.
    """
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    fan = metadata.get("fan") or {}
    if not isinstance(fan, dict):
        fan = {}
    return {
        "fallback_for_toolsets": fan.get("fallback_for_toolsets", []),
        "requires_toolsets": fan.get("requires_toolsets", []),
        "fallback_for_tools": fan.get("fallback_for_tools", []),
        "requires_tools": fan.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Fan metadata, VCS, virtualenv/dependency, cache, and support
    directories inside actual skill packages. Supporting files remain
    accessible through ``skill_view(..., file_path=...)``.
    """
    matches = []
