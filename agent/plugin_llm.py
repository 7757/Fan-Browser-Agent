...
Plugins built on Fan Agent often need to make their own LLM calls
out-of-band — a hook that rewrites a tool error before the user sees
it, a command that summarises a paste, or a scheduled job that scores
yesterday's activity into a single line on a status board.

Today the only stable plugin surfaces extend an existing Fan
subsystem: ``register_tool``, ``register_memory_provider``,
...
:class:`~fan_cli.plugins.PluginContext`:
...
* async siblings ``acomplete()`` / ``acomplete_structured()`` for
  plugins running on asyncio loops and hooks.

Provider/model/agent_id/profile are explicit keyword arguments — no
embedded slugs, no shorthands. This mirrors Fan' main config
      ...
      entries:
        my-plugin:
          llm:
            ...
            allowed_providers: [alibaba]                 # optional
            allowed_models:    [qwen3-max]               # optional
...
handles every provider, fallback chain, and per-task override Fan
supports.
"""
...
def _extract_usage(response: Any) -> PluginLlmUsage:
    ...
    Tolerant of OpenAI-compatible provider differences, including alternate
    input/output token fields and ``cache_read_input_tokens``."""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`fan_cli.plugins.PluginContext`
