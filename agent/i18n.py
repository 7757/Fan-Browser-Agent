"""Lightweight internationalization (i18n) for Fan static user-facing messages.

Scope (thin slice, by design): only the static local CLI approval prompts
shown by Fan itself.  Agent-generated output, log lines, error tracebacks,
tool outputs, and command descriptions all stay in English.
...
Usage::
    ...
    print(t("approval.dangerous_header", description="rm -rf build"))
    print(t("approval.choose_long", lang="zh"))            # explicit override

Language resolution order:
    1. Explicit ``lang=`` argument passed to :func:`t`
    2. ``FAN_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``FAN_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("FAN_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "FAN_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a fan dependency
...
def _config_language_cached() -> str | None:
    ...
    Cached because ``t()`` is called in hot approval paths and re-reading
    YAML each call would be wasteful.
...
def reset_language_cache() -> None:
    ...
    Call after :func:`fan_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("FAN_LANGUAGE")
    if env_lang:
...
def t(key: str, lang: str | None = None, **format_kwargs: Any) -> str:
    ...
    **format_kwargs
        ``str.format`` substitution arguments (for example,
        ``t("approval.dangerous_header", description="...")``).
