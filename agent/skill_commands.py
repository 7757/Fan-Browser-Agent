...
Shared by local interactive surfaces so they can invoke skills via
/skill-name commands.
"""
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``FAN_PLATFORM`` env var and
    ``FAN_SESSION_PLATFORM`` from the local session context. Returns
    ``None`` when no platform scope is active (e.g. classic CLI, RL
    rollouts, standalone scripts).
    """
    resolved_platform = (
        os.getenv("FAN_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.fan/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.fan.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_fan_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.fan/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # non-alnum chars (e.g. +, /) to keep slash-command
                    # identifiers valid in every local client.
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Fan command; skipping "
                            "auto-registration. The skill remains available "
                            "through skill_view (name=%r).",
                            name,
                            cmd_name,
                            name,
                        )
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    Rescans when the active interface scope changes (for example, a backend
    serving desktop and CLI sessions concurrently) so each interface sees
    its own ``skills.platform_disabled`` view (#14536).
    """
    if (
        not _skill_commands
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.fan/skills/`` and any ``skills.external_dirs`` so the
