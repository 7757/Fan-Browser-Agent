...
# Persisted markers belong to the source session only. Compaction assembles a
# fresh transcript for a rotated child session, so copied messages must shed
# this marker before the child-session flush.
_DB_PERSISTED_MARKER = "_db_persisted"


def _fresh_compaction_message_copy(msg: Dict[str, Any]) -> Dict[str, Any]:
    """Copy a message for compaction without source-session DB markers."""
    fresh = msg.copy()
...
def _strip_persistence_markers(messages: List[Dict[str, Any]]) -> None:
    """Ensure no compaction-local message retains a source DB marker."""
    for msg in messages:
...
# dimensions (some vision models use roughly width×height/750 and others up to ~1700 for
...
# already exhausted. protect_last_n's default is high for small/light tails;
# using all of it as a hard floor here would bring back the old case where a run
# of bulky tool outputs can't be compacted.
_MAX_TAIL_MESSAGE_FLOOR = 8

# Smaller context windows need more headroom between compactions.  At the
# historical 50% trigger, the non-compressible system/tool/tail surface can
# consume most of the reclaimed room and make compression fire every few
# turns.  This is raise-only: an explicitly higher threshold still wins.
...
# Provider replay fields ride the wire but are invisible to ``content`` and
# ``tool_calls`` accounting.  Codex Responses reasoning blobs can be larger
# than the visible assistant text; this is accounting-only and never mutates
# or prunes those fields.
_REPLAY_BUDGET_KEYS = (
    "reasoning",
    "reasoning_content",
    "reasoning_details",
...
def _estimate_msg_budget_tokens(msg: Dict[str, Any]) -> int:
    """Estimate one message for the compressor's protected-tail walks."""
    tokens = _content_length_for_budget(msg.get("content") or "") // _CHARS_PER_TOKEN + 10
...
def _strip_image_parts_from_parts(parts: Any) -> Any:
    ...
    old multimodal screenshots and image-heavy tool results.
    """
...
def _is_image_part(part: Any) -> bool:
    ...
    Recognizes both shapes the agent handles:
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the fan compressor emits).
    """
    if not messages:
        return messages
...
def _str_arg(args: dict, key: str, default: str = "") -> str:
    """Return a tool argument as a string without leaking type errors."""
...
def _summarize_tool_result(tool_name: str, tool_args: str, tool_content: str) -> str:
    ...
    This function must never raise: persisted history can contain malformed
    model-generated argument types, and compression would otherwise retry the
    same bad history indefinitely.
    """
    try:
...
def _summarize_tool_result_unguarded(
    ...
    """Build a summary line; callers use the guarded public helper above."""
    try:
...
class ContextCompressor(ContextEngine):
    ...
    def on_session_end(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        """Clear all unsafe per-session state at a real session boundary.

        Do not delegate to ``on_session_reset()``: ``compression_count`` is
        lineage state and must survive compression-driven session rotation so
        ``protect_first_n`` can decay after the first handoff.
        """
        self._previous_summary = None
        self._last_summary_error = None
        self._last_summary_dropped_count = 0
    ...
    def bind_session_state(self, session_db: Any = None, session_id: str = "") -> None:
        """Bind durable compression cooldown state to the active session."""
    ...
    def on_session_start(self, session_id: str, **kwargs) -> None:
        """Restore compression state when a session is created or resumed."""
    ...
    def get_active_compression_failure_cooldown(self) -> Optional[Dict[str, Any]]:
        """Return the still-active, session-scoped summary failure cooldown."""
        ...
        try:
            ...
            logger.debug("compression cooldown lookup failed: %s", exc)
            return None
        except Exception:
            return None
        if not state:
    ...
    def _record_compression_failure_cooldown(
        self, cooldown_seconds: float, error: Optional[str]
    ) -> None:
        ...
        session_id = getattr(self, "_session_id", "")
        recorder = (
            getattr(session_db, "record_compression_failure_cooldown", None)
        ...
        try:
            ...
            logger.debug("compression cooldown persist failed: %s", exc)
        except Exception:
            pass
    ...
    def _clear_compression_failure_cooldown(self) -> None:
        ...
        session_id = getattr(self, "_session_id", "")
        clearer = (
            getattr(session_db, "clear_compression_failure_cooldown", None)
            if session_db and session_id
            else None
        )
        ...
        try:
            ...
            logger.debug("compression cooldown clear failed: %s", exc)
        except Exception:
            pass
    ...
    def _effective_threshold_percent(
        ...
        """Apply the raise-only 75% floor to context windows below 512K."""
    ...
    def _compute_threshold_tokens(
        ...
        """Compute a reachable compaction threshold for any context window."""
    ...
    def update_model(
        ...
        self.max_summary_tokens = int(context_length * 0.05)
        # Provider usage and anti-thrash calibration describe the previous
        # model only. Carrying a large model's "this prompt fit" evidence into
        # a smaller window can incorrectly suppress the new model's preflight
        # compression.
    ...
    def __init__(
        ...
        target_tokens = int(self.threshold_tokens * self.summary_target_ratio)
        self.tail_token_budget = target_tokens
        self.max_summary_tokens = int(self.context_length * 0.05)
        ...
        # Arm this only after a completed boundary. The next provider-reported
        # prompt count then decides whether compaction actually recovered room.
        self._verify_compaction_cleared_threshold: bool = False
        # Lets the boundary wrapper distinguish a real rewrite from an abort or
        # a no-op without inferring progress from a message count.
        ...
        # Authentication and terminal network failures must never rotate a
        # conversation into a degraded fallback-summary child session.
    ...
    def update_from_response(self, usage: Dict[str, Any]):
        ...
        if self.last_prompt_tokens > 0:
            ...
            if self.last_prompt_tokens < self.threshold_tokens:
                ...
                # A real provider reading below the trigger proves this episode
                # has enough room again, even if it was not the first response
                # after compression.
                self._ineffective_compression_count = 0
            else:
                self.last_rough_tokens_when_real_prompt_fit = 0
            # Only provider usage can answer whether the completed compaction
            # cleared the threshold. Rough message estimates omit the
            # incompressible system prompt and tool schemas, so using them here
            # can cause an endless compression loop.
                ...
                if self.last_prompt_tokens >= self.threshold_tokens:
                    ...
                    if not self.quiet_mode:
                        logger.warning(
                            ...
                            "tokens still >= %d; ineffective_compression_count=%d",
                    ...
                    self._ineffective_compression_count = 0
        # Consume the pending verdict even for a usage-less response so a later,
        # unrelated response is never charged to an old compaction attempt.
        self._verify_compaction_cleared_threshold = False
        self.awaiting_real_usage_after_compression = False
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so Fan compresses before a
    ...
    def should_compress(self, prompt_tokens: int = None) -> bool:
        ...
        if tokens < self.threshold_tokens:
            return False
        cooldown_remaining = self._summary_failure_cooldown_until - time.monotonic()
    ...
    def _prune_old_tool_results(
        ...
        for i in range(prune_boundary):
            ...
            # Without this, an old screenshot (~1MB base64 + ~1500 real
            # tokens) survives every compression pass forever.
            if isinstance(content, list):
    ...
    def _compute_summary_budget(self, turns_to_summarize: List[Dict[str, Any]]) -> int:
        ...
        so large-context models retain enough detail to avoid immediately
        compacting the same material again.
        """
    ...
    def _serialize_for_summary(self, turns: List[Dict[str, Any]]) -> str:
        ...
        """
        # Lazy import avoids pulling the agent runtime's transitive imports into
        # every short session that never reaches compression.
...
"Active Task" above as historical context only. Verify current state and continue
from the protected recent messages after this summary.
...
## Pending User Asks
None recoverable from deterministic fallback. The latest user ask above is not
automatically treated as an outstanding request.
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _fallback_to_main_for_compression(self, e: Exception, reason: str) -> None:
        ...
        self._clear_compression_failure_cooldown()  # retry main model immediately
    ...
    def _generate_summary(
        ...
        # Date-only granularity matches the user's configured timezone via
        # fan_time.now(). The compaction summary is a mid-conversation message
        # that is NOT part of the cached prefix, so a date here never affects
        # prompt-cache stability. Resolved defensively — a clock failure must
        # never block compaction.
        try:
            from fan_time import now as _fan_now

            _today_str = _fan_now().strftime("%Y-%m-%d")
        ...
        if _today_str:
            _temporal_anchoring_rule = (
                f"\nTemporal anchor: the current date is {_today_str}. When an action "
                "has already been completed, record it as a dated, completed fact in "
                "the past tense rather than as an open instruction that still needs "
                f'to be done. For example, rewrite "Email John about the proposal" as '
                f'"Emailed John about the proposal on {_today_str}." Never make a '
                "completed action sound unfinished, and never invent a date for work "
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
        ...
        try:
            ...
            # A context handoff is atomic.  An ordinary incoming-message
            # interrupt must not leave a half-generated summary, while the
            # auxiliary client's timeout and hard process termination remain
            # fully effective.
            ...
            # ``RuntimeError`` is also used for empty or malformed provider
            # responses.  Only the explicit no-provider message is a permanent
            # configuration failure; all other runtime errors get the normal
            # main-model fallback and transient cooldown handling below.
            if (
                ...
                logger.warning(
                    ...
                    "Middle turns will be preserved for %d seconds.",
                    _SUMMARY_FAILURE_COOLDOWN_SECONDS,
                )
                return None
    ...
    def _sanitize_tool_pairs(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        This method removes orphaned results and strips orphaned tool calls so
        the message list is always well-formed.  Inserting synthetic results
        is unsafe when Codex ``call_id`` and chat ``id`` differ: downstream
        repair can discard the stub and expose the orphan again.
        """
        ...
        # 2. Strip assistant tool_calls whose results were dropped.
    ...
    def _effective_protect_first_n(self) -> int:
        """Decay early-turn protection after the first handoff is available."""
    ...
    def _find_last_assistant_message_idx(
        ...
        for i in range(len(messages) - 1, head_end - 1, -1):
            ...
            if isinstance(content, str) and content.strip():
                return i
            if isinstance(content, list):
                # Multimodal content: look for any text block with non-empty
                # text.
                for part in content:
    ...
    def _ensure_last_user_message_in_tail(
        ...
        if adjusted > last_user_idx:
            # The clamp would put the user in the summarized region without
            # its reply.  Summarize the complete causal pair instead of
            # preserving a dangling request that may be executed twice.
            return max(
                self._find_turn_pair_end(messages, last_user_idx),
                head_end + 1,
            )
        return adjusted
    ...
    def _find_turn_pair_end(
        messages: List[Dict[str, Any]], user_idx: int,
    ) -> int:
        """Return the first index after user -> assistant -> tool results."""
        idx = user_idx + 1
    ...
    def _find_tail_cut_by_tokens(
        ...
        # protect_last_n is a minimum up to the cap; the cap avoids preserving a
        # whole run of bulky tool outputs on every compaction.
        ...
        # transcripts; else compression can replace a tiny middle with a summary
        # and save no messages at all.
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        if n_messages <= _min_for_compress:
            # A transcript that cannot expose a compressible window must count
            # as a no-op; otherwise every subsequent turn re-enters this branch.
            self._ineffective_compression_count += 1
            ...
            if not self.quiet_mode:
                logger.warning(
                    ...
                    n_messages, _min_for_compress, self._ineffective_compression_count,
                )
            return messages
        ...
        if not summary and (
            ...
            if not self.quiet_mode and self._last_summary_auth_failure:
                logger.warning(
                    "Summary generation failed with an authentication error — "
                    "aborting compression. %d message(s) preserved unchanged; "
                    "the session was not rotated.",
                    n_skipped,
                )
            elif not self.quiet_mode and self._last_summary_network_failure:
                logger.warning(
                    "Summary generation failed with a network error — aborting "
                    "compression. %d message(s) preserved unchanged; retry once "
        ...
        # Strict OpenAI-compatible chat templates (including Qwen/vLLM) reject
        # assistant/tool-only transcripts.  If compression would remove the
        # final genuine user turn, make the handoff itself the required user
        # turn instead of producing a poisoned history.
        force_user_summary = not (
            any(msg.get("role") == "user" for msg in messages[:compress_start])
            or any(msg.get("role") == "user" for msg in messages[compress_end:])
        )
        # Pick a role that avoids consecutive same-role with both neighbors.
        # Priority: avoid colliding with head (already committed), then tail.
        if last_head_role in {"assistant", "tool"} or force_user_summary:
            summary_role = "user"
        else:
            summary_role = "assistant"
        # If the chosen role collides with the tail AND flipping wouldn't
        # collide with the head, flip it.
        if summary_role == first_tail_role and not force_user_summary:
        ...
        # Compare only like with like. ``display_tokens`` can include a large
        # prompt/tool-schema floor that compaction cannot shrink, whereas both
        # estimates below describe the message list itself.
