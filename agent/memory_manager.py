...
def normalize_tool_schema(schema: Any) -> Optional[Dict[str, Any]]:
    """Normalize a bare or OpenAI-wrapped function schema.

    Providers should return a bare function schema, but some return an
    already-wrapped OpenAI tool entry. Rewrapping that shape produces a
    nameless nested function that strict providers reject for the entire
    request. Return ``None`` when no usable function name can be resolved.
    """
...
class MemoryManager:
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        """Queue work on the serialized worker and track its durability class."""
        ...
        try:
            ...
            # An already-complete future invokes callbacks synchronously, so
            # attach this after releasing the manager lock.
            future.add_done_callback(self._forget_background_future)
        except RuntimeError:
            if self._shutting_down:
                logger.warning("Memory manager shut down during %s submission", kind)
                return
            try:
                fn()
            except Exception as e:  # pragma: no cover - fn guards internally
    ...
    def _memory_tool_result_succeeded(result: Any) -> bool:
        """Fail closed unless the built-in memory write actually committed."""
    ...
    def notify_memory_tool_write(
        ...
        *,
        ...
        """Mirror only committed built-in memory writes to external providers."""
        ...
        for operation in raw_operations:
            ...
            try:
                ...
                if operation.get("old_text"):
                    metadata["old_text"] = str(operation["old_text"])
                ...
                logger.debug("notify_memory_tool_write failed for %s: %s", action, exc)
    ...
    def _drain_sync_executor(self) -> None:
        """Give serialized writes a bounded FIFO drain before abandoning them."""
        with self._sync_executor_lock:
        ...
        logger.warning(
            ...
            "write(s), %d queued prefetch(es); %d active task(s) remain detached",
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``fan_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_fan_home()`` themselves.
        """
        if "fan_home" not in kwargs:
            from fan_constants import get_fan_home
            kwargs["fan_home"] = str(get_fan_home())
        for provider in self._providers:
            try:
