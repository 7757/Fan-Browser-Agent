...
* :func:`try_shrink_image_parts_in_messages` — image-too-large recovery
  ...
  size so retries can fit under strict provider ceilings.
...
class _CompressionLockLeaseRefresher:
    """Keep a long-running compression's state-db lock from expiring."""
    ...
    def _run(self) -> None:
        ...
        while not self._stop.wait(self._refresh_interval_seconds):
            ...
            if failures >= self._max_consecutive_failures:
                logger.debug(
                    "compression lock refresh failed %d times; stopping for session %s",
                    failures,
                    self._session_id,
                )
                break
    ...
    *after* construction, so :func:`replay_compression_warning` re-sends
    ...
    try:
        ...
        if client is None or not aux_model:
            fb_client, fb_model, _fb_label = _try_configured_fallback_for_unavailable_client(
                "compression",
                _aux_cfg_provider,
            )
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `fan setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Fan "
...
def replay_compression_warning(agent: Any) -> None:
    ...
    During ``__init__`` the local client's ``status_callback`` is not yet
    wired, so ``_emit_status`` only reaches ``_vprint`` (CLI). This method
    is called once at the start of the first ``run_conversation()``; by then
    the client has set the callback and receives the warning.
    """
...
def _ensure_compressed_has_user_turn(
    ...
    """Keep a real user turn for strict OpenAI-compatible chat templates."""
    if any(
    ...
    for msg in reversed(original_messages):
        if not isinstance(msg, dict) or msg.get("role") != "user":
            continue
        restored = dict(msg)
        # Rotation writes into a different session.  Never carry a persistence
        # hint across that boundary, even though Fan's current marker also
        # validates the owning session and fingerprint.
    ...
    compressed.append({
        ...
        "content": (
            ...
            "This marker exists because no earlier user turn was available."
        ),
    })
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``fan_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `fan update`) to resync.",
    ...
    # Compare semantic state rather than object identity. Plugin compressors
    # may return an equal copy, or mutate the live list and return an unchanged
    # snapshot. Neither case made progress and neither may rotate the session.
    ...
    if agent._session_db:
        try:
            ...
            # Auto-compression can rotate the session in the middle of a turn,
            # before the normal end-of-turn persistence path runs. Flush the
            # complete pre-compression history while it still belongs to the
            # old session so those messages are not stranded or lost.
            try:
                agent._flush_messages_to_session_db(messages)
            except Exception as flush_error:
                # Persistence is best-effort here: a transient DB failure must
                # not prevent compression from relieving an oversized context.
                # The flush implementation only marks successfully committed
                # messages, so an uncommitted tail remains retryable.
                logger.warning(
                    "Could not flush session history before compression rotation: %s",
                    flush_error,
                )
            ...
            # The gateway/tools session context (env above) and the logging
            # session context are SEPARATE mechanisms. The line above moves the
            # former; the ``[session_id]`` tag on log lines comes from
            # ``fan_logging._session_context`` (set once per turn in
            ...
            try:
                agent._session_db.create_session(
                    ...
                    source=agent.platform or os.environ.get("FAN_SESSION_SOURCE", "cli"),
                    model=agent.model,
                    model_config=agent._session_init_model_config,
                    parent_session_id=old_session_id,
                )
                if agent._session_db.get_session(agent.session_id) is None:
                    raise RuntimeError("continuation session row was not persisted")
            except Exception as create_error:
                failed_child_id = agent.session_id
                logger.warning(
                    "Compression continuation %s could not be created (%s); "
                    "rolling back to indexed parent %s",
                    failed_child_id,
                    create_error,
                    old_session_id,
                )
                ...
                try:
                    ...
                    logger.warning(
                        "Could not reopen compression parent session %s: %s",
                        old_session_id,
                        reopen_error,
                    )
                agent._session_db_created = True
                old_session_id = None
                _rotation_rolled_back = True
                raise
            inherit_browser_context = getattr(
                agent._session_db,
                "inherit_session_browser_context",
                None,
            )
            if callable(inherit_browser_context):
                try:
                    inherit_browser_context(old_session_id, agent.session_id)
                except Exception:
                    # create_session() already anchors the child to the
                    # parent's stable workbench. Copying the optional browser
                    # snapshot is best-effort and must not roll a committed
                    # compression rotation back.
                    logger.warning(
                        "Could not inherit browser context after compression: "
                        "parent=%s child=%s",
                        old_session_id,
                        agent.session_id,
                        exc_info=True,
            ...
            agent._session_db_created = True
            try:
                from fan_cli.goals import migrate_goal_to_session
                ...
                logger.warning(
                    "Could not migrate persistent goal across compression: %s",
                    goal_error,
                )
            ...
            if _rotation_rolled_back:
                logger.warning(
                    "Compression rotation aborted; continuing on parent "
                    "session %s: %s",
                    agent.session_id or "?",
                    e,
                )
    ...
    # compression (not a fresh /new). Plugin engines (e.g. fan-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See fan-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    if _cc >= 2:
        ...
        agent._compression_warning = _cc_message
        agent._emit_status(_cc_message)
    ...
    if getattr(agent.context_compressor, "_last_compression_made_progress", False):
        # Arm only after the complete boundary rewrite. Aborts and no-op paths
        # return earlier, so later usage cannot be misattributed to them.
...
def try_shrink_image_parts_in_messages(
    ...
    *,
    ...
    image-too-large provider errors.
    ...
    size exceeds 4 MB (a conservative target with request headroom) or whose
    longest side exceeds
    ...
    # A 4 MB target leaves comfortable request headroom. Providers may accept
    # much larger images; shrinking here loses quality but only fires
    # after a confirmed provider rejection, so the alternative is failure.
    target_bytes = 4 * 1024 * 1024
    # Some providers enforce a per-side dimension cap independently of the
    # byte cap. In many-image requests, a provider can report a lower cap.
    # The caller passes that parsed ceiling
    ...
    def _decode_pixels(data_url: str) -> Optional[tuple]:
        """Return ``(width, height)`` for a base64 data URL when decodable."""
        try:
            import base64 as _b64_dim
            import io as _io_dim

            _header, _, data = data_url.partition(",")
            if not data or not data_url.startswith("data:"):
                return None
            from PIL import Image as _PILImage

            with _PILImage.open(_io_dim.BytesIO(_b64_dim.b64decode(data))) as image:
                return image.size
        except Exception:
            return None

    def _shrink_data_url(url: str) -> tuple[Optional[str], bool]:
        """Return ``(replacement, unshrinkable)`` for one image data URL."""
        if not isinstance(url, str) or not url.startswith("data:"):
            return None, False

        # Keep the triggering constraint explicit. Pixel downscaling can make
        # a PNG larger in bytes; a dimension-correct result is still valid in
        # that case and must not be discarded in favour of the rejected source.
        needs_shrink = len(url) > target_bytes  # over byte budget
        triggered_by = "bytes" if needs_shrink else None
        if not needs_shrink:
            dimensions = _decode_pixels(url)
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="fan_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
            ...
            if triggered_by == "bytes":
                if len(resized) >= len(url):
                    return None, True
                new_dimensions = _decode_pixels(resized)
                if (
                    new_dimensions is not None
                    and max(new_dimensions) > max_dimension
                ):
                    return None, True
                return resized, False

            # Dimension is the binding constraint. Accept a pixel-correct
            # result even when PNG re-encoding increased its byte length.
            new_dimensions = _decode_pixels(resized)
            if new_dimensions is not None:
                if max(new_dimensions) <= max_dimension:
                    return resized, False
                return None, True
            # If the output cannot be verified, retain the historical safer
            # byte gate rather than accepting a larger opaque payload.
            if len(resized) >= len(url):
                return None, True
            return resized, False
        except Exception as exc:
