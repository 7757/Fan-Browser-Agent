...
_SENSITIVE_FAN_DIRS: tuple[Path, ...] = ()
...
async def preprocess_context_references_async(
    ...
    *,
    ...
    # Each reference is independent during expansion. Resolve them together so
    # several @url references do not serialize network extraction, while
    # retaining the original order when assembling the injected context.
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    fan_home = get_fan_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(fan_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(fan_home / rel for rel in _SENSITIVE_FAN_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Fan path and cannot be attached")
