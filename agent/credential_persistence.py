...
``auth.json``.  They intentionally have no dependency on ``fan_cli.auth`` so
...
# Credential sources the local runtime may intentionally persist in auth.json. Everything
...
def _credential_secret_fingerprint(payload: Mapping[str, Any]) -> str | None:
    for key in ("access_token", "refresh_token", "api_key", "token", "secret"):
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and supported OAuth/device-code state)
