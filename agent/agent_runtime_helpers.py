...
* ``drop_thinking_only_and_merge_users`` — provider-neutral message cleanup
* ``restore_primary_runtime`` — un-do fallback activation
* ``extract_reasoning`` — pull reasoning fields out of API responses
* ``dump_api_request_debug`` — write bounded request-failure diagnostics
* ``prompt_cache_policy`` — decide whether to add cache_control breakpoints
...
from fan_cli.timeouts import get_provider_request_timeout
...
# A refreshed OAuth token can still be rejected upstream.  With a one-entry
# credential pool, treating every successful refresh as recoverable would keep
# the request loop on that same credential forever and prevent model fallback.
...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset(
    {"todo", "session_search", "memory", "collect", "delegate_task"}
)
    ...
    while message_index < len(messages):
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                # Codex Responses can expose a distinct response-item ``id``
                # and function ``call_id``.  Use the pipeline's canonical
                # id so an existing result is found instead of inserting a
                # duplicate orphan repair stub.
...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    Chat-completions providers expect strict alternation:
        ...
        if role == "assistant":
            ...
            for tc in (msg.get("tool_calls") or []):
                if not isinstance(tc, dict):
                    continue
                # Tool results may refer to either OpenAI's item id or
                # Responses' call_id; both identify a valid preceding call.
...
def recover_with_credential_pool(
    ...
    *,
    ...
    # An empty agent provider is not a safe wildcard: swapping a scoped
    # pool's credentials into it leaves provider/model identity unset while
    # base URL and key change underneath, corrupting later recovery. Only an
    # unscoped pool (empty ``pool_provider``) may operate without an exact
    # provider match.
    ...
    if effective_reason == FailoverReason.auth:
        ...
        if refreshed is not None:
            # Limit repeated refreshes of the same pool entry within one turn.
            # A successful token mint only means the refresh endpoint accepted
            # it; it does not prove that the actual model endpoint will accept
            # the token.  Once the cap is exceeded, return unrecovered so the
            # normal fallback path can take over.
            ...
            if refreshed_id is not None:
                ...
                if refresh_counts[refresh_key] > _MAX_AUTH_REFRESH_ATTEMPTS:
                    _ra().logger.warning(
                        ...
                        "pool entry %s; allowing fallback to activate.",
...
def try_recover_primary_transport(
    ...
    OpenAI, local models) where a TCP-level hiccup does not
    mean the provider is down.

    Skipped for OpenRouter, which already manages connection pools and retries
    server-side — if our retries through it are exhausted, one more rebuilt
    client won't help.
    """
...
def drop_thinking_only_and_merge_users(
    ...
    *,
    ...
    - Filtering orphaned reasoning-only turns and merging adjacent user
      messages preserves a valid wire sequence without inventing content.
    """
    if not messages:
        return messages
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        # A cross-provider fallback can leave the fallback provider's pool
        # attached.  Restore the primary pool before selecting a credential;
        # otherwise the mismatch guard disables rotation on the next primary
        # 401/429.  A custom endpoint pool is keyed ``custom:<name>``, not the
        # generic runtime label, so resolve it from the primary endpoint.
        primary_provider = str(rt.get("provider") or "").strip().lower()
        primary_pool_key = primary_provider
        ...
        if primary_provider == "custom":
            try:
                ...
                primary_pool_key = (
                    get_custom_provider_pool_key(str(rt.get("base_url") or ""))
                    or primary_provider
                )
                pool_matches_primary = bool(primary_pool_key) and pool_provider == primary_pool_key
            except Exception:
                pool_matches_primary = False

        if pool is not None and pool_provider and not pool_matches_primary:
            # A mismatched pool is worse than no pool: it prevents recovery or
            # can rotate a credential into the wrong endpoint.  Clear first so
            # a temporary auth-store failure cannot leave it attached.
            agent._credential_pool = None
            try:
                from agent.credential_pool import load_pool

                agent._credential_pool = load_pool(primary_pool_key)
        ...
        # The primary snapshot captures a construction-time key, but a pool
        # entry may have rotated or been exhausted during a prior turn. Select
        # the current viable entry before the next request so we do not revive
        # a known-bad credential and burn through the retry/fallback chain.
        pool = getattr(agent, "_credential_pool", None)
        if pool is not None and pool.has_available():
            entry = pool.select()
            if entry is not None:
                entry_provider = str(
                    getattr(entry, "provider", "") or ""
                ).strip().lower()
                ...
                if entry_key and entry_matches_primary:
                    ...
                    logger.info(
                        "Primary restore re-selected credential pool entry %s (%s)",
                        getattr(entry, "id", "?"),
                        getattr(entry, "label", "?"),
                    )
                elif entry_key:
                    logger.info(
                        "Primary restore skipped credential pool entry %s (%s): "
                        "provider %s does not match primary provider %s",
                        getattr(entry, "id", "?"),
                        getattr(entry, "label", "?"),
                        entry_provider or "?",
                        primary_provider or "?",
        ...
        # The streak measured the fallback route we are leaving.  A restored
        # primary must get one fresh request before the circuit breaker can
        # trip again.
...
def dump_api_request_debug(
    ...
    *,
    ...
    """
    Persist a bounded failure record for the active inference API.

    Normal failures store metadata only. Full prompts, conversation history,
    and tool schemas are written solely when ``FAN_DUMP_REQUESTS`` is
    explicitly enabled.
    """
    try:
        ...
        full_dump = env_var_enabled("FAN_DUMP_REQUESTS")
        ...
        request_record: Dict[str, Any] = {
            ...
            "model": body.get("model") or getattr(agent, "model", None),
            "body_keys": sorted(str(key) for key in body),
            "body_bytes": len(serialized_body),
        ...
        dump_payload: Dict[str, Any] = {
            ...
            "reason": reason,
            "contains_full_request": full_dump,
        ...
        if error is not None:
            ...
            if full_dump:
                ...
                encoded_error_message = error_message.encode(
                    "utf-8", errors="replace"
                )
                error_info["message_bytes"] = len(encoded_error_message)
                error_info["message_sha256"] = hashlib.sha256(
            ...
            if body_attr is not None:
                if full_dump:
                    ...
                    encoded_error_body = json.dumps(
                        body_attr,
                        ensure_ascii=False,
                        separators=(",", ":"),
                        default=str,
                    ).encode("utf-8", errors="replace")
                    error_info["body_bytes"] = len(encoded_error_body)
                    error_info["body_sha256"] = hashlib.sha256(
                        encoded_error_body
                    ).hexdigest()
            ...
            if response_obj is not None:
                try:
                    ...
                    response_text = str(response_obj.text or "")
                    if full_dump:
                        error_info["response_text"] = response_text
                    else:
                        encoded_response = response_text.encode(
                            "utf-8", errors="replace"
                        )
                        error_info["response_bytes"] = len(encoded_response)
                        error_info["response_sha256"] = hashlib.sha256(
                            encoded_response
                        ).hexdigest()
                except Exception as e:
                    _ra().logger.debug(
        ...
        safe_session_id = _ra()._safe_session_filename_component(agent.session_id)
        dump_file = (
            agent.logs_dir
            / f"request_dump_{safe_session_id}_{timestamp}.json"
        )
        from agent.redact import redact_sensitive_text

        serialized_dump = json.dumps(
            dump_payload,
            ensure_ascii=False,
            indent=2,
            default=str,
        )
        redacted_dump = redact_sensitive_text(serialized_dump, force=True)
        temp_file = dump_file.with_name(dump_file.name + ".tmp")
        temp_created = False
        try:
            flags = (
                os.O_WRONLY
            ...
            descriptor = os.open(temp_file, flags, 0o600)
            temp_created = True
            try:
                payload = redacted_dump.encode("utf-8", errors="replace")
                view = memoryview(payload)
                while view:
                    written = os.write(descriptor, view)
                    if written <= 0:
                        raise OSError("request dump write made no progress")
                    view = view[written:]
                try:
        ...
        # Keep enough recent breadcrumbs for support without accumulating full
        # request history for the lifetime of an installation.
        try:
            existing = []
            for candidate in agent.logs_dir.glob("request_dump_*.json"):
            ...
            for retained in existing[:10]:
                try:
                    os.chmod(retained, 0o600, follow_symlinks=False)
        ...
        label = "full request debug" if full_dump else "request failure metadata"
        agent._vprint(
            f"{agent.log_prefix}🧾 {label} written to: {dump_file}"
        )

        if env_var_enabled("FAN_DUMP_REQUEST_STDOUT"):
            print(redacted_dump)
...
def prompt_cache_policy(
    ...
    *,
    ...
    """Return whether this OpenAI-compatible endpoint supports cache markers.

    Fan currently enables explicit markers only for Qwen models on Alibaba
    family endpoints. Other providers may perform automatic prefix caching,
    but do not need request-body markers from Fan.
    """
    ...
    return provider_is_alibaba_family and model_is_qwen
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    ssl_verify = client_kwargs.pop("ssl_verify", None)
    ...
    # The conversation loop owns retry policy, including Retry-After and
    # interruption-aware backoff.  Leaving the SDK default here performs
    # hidden 1–2 second retries inside every outer retry and can repeatedly
    # hit a rate-limited provider before its advertised reset window.
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    Called by the /model command handlers (CLI and gateway) after the
    requested model/provider has been resolved and validated.  This
    method performs the actual runtime
    ...
    # HTTP 400s like "qwen3-vl-plus is not supported on openai-codex" on the
    ...
    # A provider change also changes which credentials may safely recover a
    # request.  Include the pool in the transactional snapshot so a client
    # construction failure cannot strand the old runtime with a new pool.
    ...
    try:
        ...
        # A genuine provider change must never retain the previous provider's
        # endpoint.  An empty resolver result is an incomplete switch, not
        # permission to pair the new provider with a stale base_url and persist
        # that broken pair into ``_primary_runtime``.  Re-selecting the same
        # provider may still omit the URL for credential-only refreshes.
        ...
        if base_url:
            ...
            raise ValueError(
                "switch_model: no base_url resolved for provider "
        ...
        # Rebind pool recovery to the selected provider.  Leaving the old
        # provider's pool attached makes 401/429 recovery either short-circuit
        # on a provider mismatch or rotate a credential into the wrong
        # endpoint.  Custom endpoints store pools under ``custom:<name>``, so
        # derive that key from the resolved endpoint rather than loading the
        # generic (and intentionally empty) ``custom`` pool.
        if old_norm_provider != new_norm_provider or getattr(agent, "_credential_pool", None) is None:
            # Do not retain the old provider's pool if reload fails: recovery
            # must fail closed instead of rotating a credential into the newly
            # selected endpoint.
            agent._credential_pool = None
            try:
                ...
                if new_norm_provider == "custom":
                    pool_key = get_custom_provider_pool_key(agent.base_url) or new_provider
                ...
                logger.warning(
                    ...
                    "continuing without pool recovery this turn",
                    new_provider,
                    pool_exc,
                )
        ...
        try:
            from fan_cli.config import apply_custom_provider_tls_to_client_kwargs
        ...
        # Reapply endpoint-specific attribution/auth headers after rebuilding
        # client kwargs.  A model switch otherwise drops OpenRouter and other
        # provider headers that were installed during initial construction.
        apply_client_headers = getattr(agent, "_apply_client_headers_for_base_url", None)
        if callable(apply_client_headers):
            apply_client_headers(str(effective_base or ""))
        agent.client = agent._create_openai_client(
    ...
    agent._use_prompt_caching = agent._prompt_cache_policy(
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from fan_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    # A user-initiated model switch is the recovery action surfaced by the
    # stale-call error. Reset only after the new client was constructed above.
    ...
    # Session usage and resume metadata must follow the live route, not the
    # provider active when this session was created.  The state DB update is
    # best-effort so a transient persistence failure never rolls back a valid
    # in-memory model switch.
    ...
    if session_db is not None and session_id:
        try:
            ...
            logger.warning(
                "switch_model: failed to persist billing route after model switch",
                exc_info=True,
            )
    ...
    # entries that target the OLD primary
    ...
    # (the user explicitly selected a different provider).
...
def repair_tool_call(agent, tool_name: str) -> str | None:
    ...
    4. Strip trailing ``_tool`` / ``-tool`` / ``tool`` suffix that
       some models tack on (TodoTool_tool ->
...
def looks_like_codex_intermediate_ack(
    ...
    The default preserves the conservative coding-workspace scope.  When the
    deployment enables intent-ack continuation for a model or every API mode,
    an action acknowledgement without a workspace reference is also enough to
    ...
    # Vision turns forwarded through the OpenAI-compatible API can retain
    # OpenAI-style multi-part content here.  Flatten it before workspace
    # matching so a list never reaches ``.strip()`` and aborts the turn.
...
def intent_ack_continuation_mode(agent) -> str:
    """Resolve whether intermediate intent acknowledgements should continue.

    ``auto`` retains Fan's historical behaviour for Responses API sessions.
    Explicit true or a matching model substring extends the continuation to
    every supported API mode; false disables it.  This lets chat-completions
    models recover from the common “I'll check that” then stop pattern without
    silently changing the default behaviour for all existing deployments.
    """
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    # Preserve explicit reasoning only for providers that require echo-back.
    # Strict OpenAI-compatible fallbacks reject the field altogether, even if
    # it is an empty string or a one-space placeholder from a prior provider.
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    """Reconcile reasoning_content for the currently active provider.
...
def apply_pending_steer_to_tool_results(agent, messages: list, num_tool_msgs: int) -> None:
    ...
    if not isinstance(existing_content, str):
        # Preserve structured multimodal content and append a text block.
        try:
