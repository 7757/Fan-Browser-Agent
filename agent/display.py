...
# Browser text entry has one current single-field tool. Multi-field form input
# is rendered separately because its payload is a list of fields.
_BROWSER_TEXT_INPUT_TOOLS = frozenset({"browser_type"})


def redact_browser_typed_text_for_display(value: Any, typed_text: Any) -> Any:
    """Compatibility hook: browser results are displayed unchanged."""
    del typed_text
    return value


def redact_tool_args_for_display(tool_name: str, args: dict | None) -> dict | None:
    """Compatibility hook: tool arguments are displayed unchanged."""
    del tool_name
    return args
...
def _build_tool_preview_unguarded(
    tool_name: str,
    args: dict,
    ...
    *max_len* controls truncation.  ``None`` (default) defers to the global
    ...
    primary_args = {
        ...
        "browser_run": "intent", "browser_handoff": "reason",
        "browser_click": "index", "browser_type": "text",
        "vision_analyze": "question", "mixture_of_agents": "user_prompt",
        "skill_view": "name", "skills_list": "category",
        "cronjob": "action",
        "execute_code": "code", "delegate_task": "goal",
        "collect": "question", "skill_manage": "name",
    }
    ...
    if tool_name == "delegate_task":
        ...
        if tasks and isinstance(tasks, list):
            ...
            return f"{len(tasks)} 个任务: " + " | ".join(goals) if goals else f"{len(tasks)} 个并行任务"
        goal = args.get("goal", "")
    ...
    if tool_name == "browser_fill_form":
        ...
        return f"{len(fields)} 个字段" if isinstance(fields, list) else None
...
def build_tool_preview(
    ...
    """Build a best-effort preview without letting display data crash callers."""
    try:
        return _build_tool_preview_unguarded(tool_name, args, max_len=max_len)
    except Exception as exc:
        logger.debug("Tool preview failed for %s: %s", tool_name, exc)
        return None
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Fan' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("FAN_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def _detect_tool_failure(tool_name: str, result: str | None) -> tuple[bool, str]:
    ...
    # Browser-program results have a trusted top-level transaction status.
    # Never scan the program value or page snapshot for words such as
    # "failed": they are untrusted task data and do not describe tool health.
    if tool_name in {
        "browser_snapshot",
        "browser_run",
        "browser_handoff",
        ...
        if status in {"completed", "needs_replan", "needs_human"}:
            return False, ""
        if status in {
            ...
            if error:
                return True, f" [{_trim_error(str(error))}]"
...
def _get_cute_tool_message_unguarded(
    ...
    if tool_name == "browser_observe":
        return _wrap(f"┊ 👁️  observe   current page  {dur}")
    if tool_name == "browser_click":
        return _wrap(f"┊ 👆 click     #{args.get('index', '?')}  {dur}")
    if tool_name in _BROWSER_TEXT_INPUT_TOOLS:
        return _wrap(f"┊ ⌨️  type      \"{_trunc(args.get('text', ''), 30)}\"  {dur}")
    if tool_name == "browser_fill_form":
        fields = args.get("fields")
        count = len(fields) if isinstance(fields, list) else 0
        return _wrap(f"┊ ⌨️  fill      {count} field(s)  {dur}")
    ...
    if tool_name == "browser_send_keys":
        return _wrap(f"┊ ⌨️  keys      {args.get('keys', '?')}  {dur}")
    if tool_name == "browser_screenshot":
        return _wrap(f"┊ 📸 screenshot current page  {dur}")
    if tool_name == "browser_find_visual":
        return _wrap(f"┊ 👁️  visual    {_trunc(args.get('description', ''), 30)}  {dur}")
    if tool_name == "todo":
        todos_arg = args.get("todos")
        merge = args.get("merge", False)
    ...
    if tool_name == "delegate_task":
        ...
        if tasks and isinstance(tasks, list):
            ...
            detail = " | ".join(goals) if goals else "并行"
...
def get_cute_tool_message(
    ...
    """Render a best-effort completion line that never breaks tool delivery."""
    try:
        return _get_cute_tool_message_unguarded(
            tool_name,
            args,
            duration,
            result=result,
        ...
        logger.debug("Tool completion display failed for %s: %s", tool_name, exc)
        try:
            duration_text = f"{float(duration):.1f}s"
        except (TypeError, ValueError):
            duration_text = ""
        return f"┊ ⚡ {str(tool_name)[:9]:9} {duration_text}".rstrip()
