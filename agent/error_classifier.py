...
class FailoverReason(enum.Enum):
    ...
    # A broken certificate chain is deterministic for the host/proxy; retrying
    # it burns the recovery budget without changing the TLS handshake.
    ...
    input_moderation_blocked = "input_moderation_blocked"  # Provider's pre-inference INPUT content-safety filter rejected an image (e.g. DashScope/Bailian data_inspection_failed) — strip the screenshot, downgrade to text for the session, retry
...
# Provider-wide saturation is different from a per-credential rate limit.
# Retrying the same credential after backoff is correct; rotating credentials
# would only exhaust the pool while the endpoint remains busy.  A few
# OpenAI-compatible providers report this condition as HTTP 429, so inspect
# their body before applying the normal rate-limit recovery path.
...
# ``content`` to be a string. Some (Codex Responses,
# Gemini native, first-party OpenAI) extend this to accept a content-parts
# list (text + image_url) so screenshot-bearing tool results survive. Others
...
# Provider INPUT content-safety / moderation rejection of image data. Distinct
# from _CONTENT_POLICY_BLOCKED_PATTERNS (the model's per-prompt safety refusal):
# DashScope/Bailian runs a MANDATORY pre-inference "green-net" data inspection
# and returns a hard 400 "DataInspectionFailed" when an input image trips it
# (common on Chinese web content / public figures). The model never ran, so
# there is no graceful refusal to surface. Recovery is the same shape as the
# multimodal downgrade: strip the offending screenshot from the tool result,
# keep its DOM text, record the (provider, model) so subsequent screenshots
# auto-downgrade for the session, and retry. Patterns are narrow, verbatim
# DashScope shapes — NOT a generic word like "inappropriate".
_INPUT_MODERATION_PATTERNS = [
    "data_inspection_failed",                # DashScope error code
    "datainspectionfailed",                  # DashScope message: InternalError.Algo.DataInspectionFailed
    "data inspection failed",                # spaced variant
    "input data may contain inappropriate",  # DashScope message tail
]
...
_MODEL_NOT_FOUND_PATTERNS = [
    ...
    "unknown model",
    "unsupported model",
    # OpenRouter uses this deterministic 404 when no candidate endpoint for a
    # model supports tools.  Let fallback select a compatible route rather
    # than retrying the same rejected tool-call request.
...
_CONTENT_POLICY_BLOCKED_PATTERNS = [
    ...
    # MiniMax uses this narrow marker when its output safety layer truncates a
    # response/tool call. Treat it as a deterministic policy block rather than
...
# Certificate verification differs from a transient TLS alert: a corporate
# TLS proxy, stale CA store, self-signed endpoint or hostname mismatch will
# fail identically until the environment is corrected.  Check these before
# the broad ``[ssl:`` retry patterns above.
...
def classify_api_error(
    ...
    *,
    ...
    Args:
        ...
        provider: Current provider name (e.g. "alibaba", "custom").
    ...
    # ── 5. Certificate verification failures → fail fast ──────────────
    ...
    if is_disconnect and not status_code:
        # A transport close from a known reasoning model is normally an
        # upstream proxy idle timeout, even when the prompt is large.  Do not
        # misclassify it as context overflow: that would compact a healthy
        # conversation solely because the provider dropped a long thinking
...
def _classify_by_status(
    ...
    *,
    ...
    if status_code == 401:
        ...
        # third-party provider refresh runs before
    ...
    if status_code == 404:
        # Some providers return plan or credit depletion as 404 rather than
        # 402. Treat explicit billing-shaped messages as billing exhaustion,
        # not a missing model, so the loop can show credit/top-up guidance.
    ...
    if status_code in {500, 502}:
        ...
        # Local OpenAI-compatible servers can surface an explicit context
        # overflow as 500/502 instead of 400/413. Compress rather than
        # exhaust blind server-error retries.
    ...
    # 408 Request Timeout is a transient timing failure, often emitted by a
    # reverse proxy in front of a local/OpenAI-compatible backend. It is safe
    # to rebuild and retry; treating it as a generic 4xx aborts a live turn.
    if status_code == 408:
...
def _classify_400(
    ...
    *,
    ...
    # Provider INPUT moderation rejected an image (DashScope DataInspectionFailed).
    # Same recovery shape as multimodal (strip image parts from tool messages,
    # downgrade to text for the session, retry) but a distinct reason so logging
    # and the user-facing message are accurate. Checked before context_overflow
    # because the wording must not be mis-routed to a compress/retry-unchanged path.
    if any(p in error_msg for p in _INPUT_MODERATION_PATTERNS):
        return result_fn(
            FailoverReason.input_moderation_blocked,
            retryable=True,
        )

    # Some providers report image-size failures as HTTP 400.
...
def _classify_by_message(
    ...
    *,
    ...
    # Input moderation patterns (from message text when no status_code)
    if any(p in error_msg for p in _INPUT_MODERATION_PATTERNS):
        return result_fn(
            FailoverReason.input_moderation_blocked,
            retryable=True,
    ...
    # Message-only overloads occur on some proxy / Anthropic-compatible
    # endpoints.  Handle them before rate-limit and billing matching so a
    # valid credential is not rotated merely because the server is busy.
    if any(p in error_msg for p in _OVERLOADED_PATTERNS):
        return result_fn(
            FailoverReason.overloaded,
            retryable=True,
...
def _extract_error_body(error: Exception) -> dict:
    """Extract a structured error body, including wrapped exception causes."""
    current = error
    for _ in range(5):
