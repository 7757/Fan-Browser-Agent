...
Two modes:
  ...
  native  — attach images as OpenAI-style ``image_url`` content parts on the
            user turn. The active transport sends them in its supported
            multimodal format.
...
# Image extensions used by extract_image_refs(). Kept tight on purpose — only
# media the model can actually see is treated as a vision input.
_IMAGE_EXTS = (
...
# Absolute / home-relative local image path: anchors to ``~/`` or ``/``, ignores
# matches inside URLs, and is case-insensitive on the extension.
...
def extract_image_refs(text: str) -> Tuple[List[str], List[str]]:
    ...
    reference aren't mistaken for live attachments.
...
def _supports_vision_override(
    ...
    Resolution order, first hit wins:
      ...
      2. ``providers.<provider>.models.<model>.supports_vision``
         ...
         the stripped ``<name>`` provider key is tried as well.)
    ...
    # (fan_cli/runtime_provider.py:_resolve_named_custom_runtime), so the
    # config still holds the user-declared name under model.provider. Try
    # both as candidate provider keys, plus the stripped custom:<name> suffix.
    config_provider = str(model_cfg.get("provider") or "").strip()
    stripped_suffix = (
        config_provider[len("custom:"):]
...
def _explicit_aux_vision_override(cfg: Optional[Dict[str, Any]]) -> bool:
    ...
    In auto mode this is a fallback for text-only main models; native vision
    remains preferred when the active main model can accept the original image.
    """
    if not isinstance(cfg, dict):
        return False
...
def decide_image_input_mode(
    ...
    Args:
      provider: active inference provider ID (e.g. ``"alibaba"``).
    ...
    # auto: native vision preserves pixels and wins whenever the main model
    # supports it. An explicit auxiliary backend is the fallback for models
    # that do not accept images.
...
# the provider rejects the request because an image exceeds its limit.
#
...
def _sniff_mime_from_bytes(raw: bytes) -> Optional[str]:
    ...
    an upstream source reports the wrong content type. Strict multimodal APIs
    validate that the declared media type matches the actual bytes and return
    ...
    if raw.startswith(b"BM"):
        return "image/bmp"
    # ISO-BMFF image family: AVIF / HEIC / HEIF.
    ...
    if raw[:4] == b"\x00\x00\x01\x00":
        return "image/x-icon"
    # SVG is vector data and cannot be rasterized by Pillow without an
    # additional renderer. Recognize it so the compatibility path skips it
    # instead of declaring it as JPEG.
...
_UNIVERSALLY_SUPPORTED_IMAGE_MIMES = frozenset({
    "image/png", "image/jpeg", "image/gif", "image/webp",
})


def _transcode_image_to_png(raw: bytes) -> Optional[bytes]:
    """Convert a non-standard raster image into a provider-safe PNG.

    Pillow (and HEIC/AVIF plugins, where installed) is optional.  A missing
    decoder must not fail the entire user turn: callers treat ``None`` as a
    skipped native attachment and can continue with other input.
    """
    try:
        from PIL import Image
        from io import BytesIO
    except ImportError:
        logger.info("image_routing: Pillow unavailable; cannot transcode non-standard image")
        return None
    try:
        # Plugins register decoders at import time.  They are optional, so a
        # missing dependency simply means that particular format is skipped.
        try:
        ...
        logger.info("image_routing: cannot transcode image to PNG: %s", exc)
        return None
...
def _file_to_data_url(path: Path) -> Optional[str]:
    ...
    Formats outside PNG/JPEG/GIF/WEBP are converted to PNG first because
    common model providers reject raw BMP/TIFF/HEIC/AVIF/ICO payloads.  SVG
    and undecodable input are skipped rather than poisoning the whole turn.
    """
    try:
        ...
        # Guard availability must not disable ordinary image attachments.
        pass
    ...
    if mime not in _UNIVERSALLY_SUPPORTED_IMAGE_MIMES:
        transcoded = _transcode_image_to_png(raw)
        if transcoded is None:
            logger.warning(
                "image_routing: skipping unsupported native image %s (%s)",
                path.name,
                mime,
            )
            return None
...
def build_native_content_parts(
    ...
    request because an image is too large, the agent's retry loop
    transparently shrinks and retries
