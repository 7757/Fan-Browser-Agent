...
class LSPClient:
    ...
    async def _spawn(self) -> None:
        # Language servers execute third-party project tooling. Preserve
        # ordinary project/runtime variables and explicit LSP overrides, but
        # never hand the server Fan's provider, tool, skill, auxiliary-model,
        # desktop-runtime, or control-plane credentials.
        ...
        try:
            self._proc = await asyncio.create_subprocess_exec(
                ...
                *cmd[1:],
                ...
                env=env,
                cwd=self._cwd,
                # Keep language servers out of Fan's process group. A child
                # snapshot racing with MCP spawn can otherwise misclassify an
                # LSP PID, and group cleanup would include the gateway itself.
            ...
            while True:
                ...
                if text:
                    ...
                    logger.debug(
                        "[%s] stderr: %s",
                        self.server_id,
                        redact_sensitive_text(text, force=True)[:1000],
                    )
