...
# Language servers parse repository-controlled source, so their diagnostic
# fields are untrusted before being embedded in the model-visible block.
...
def _sanitize_field(value: Any, *, limit: int) -> str:
    """Make one LSP diagnostic field safe for a tool-result block."""
    if value is None:
        return ""
    raw = str(value)
    # Keep each field on one physical line and discard control characters that
    # could synthesize formatting inside the diagnostics block.
    raw = raw.replace("\r", " ").replace("\n", " ")
    raw = "".join(ch for ch in raw if ch == " " or ch.isprintable())
    raw = raw.strip()[:limit]
    # Escape tag delimiters so diagnostic text cannot close or forge blocks.
    return html.escape(raw, quote=False)


def format_diagnostic(d: Dict[str, Any]) -> str:
    """One-line representation of a sanitized single diagnostic."""
...
def report_for_file(
    ...
    *,
    ...
    if extra > 0:
        body += f"\n... and {extra} more"
    # Quote attribute delimiters too, so a crafted path cannot escape the
    # diagnostics file attribute and introduce model-visible markup.
