...
appropriate.  All installs go to a Fan-owned bin staging dir,
``<FAN_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``fan lsp
  status``.
...
def _install_subprocess_env(extra_env: Optional[dict] = None) -> dict[str, str]:
    """Environment for package-manager children that execute third-party code."""
    from tools.environments.local import fan_subprocess_env

    return fan_subprocess_env(
        inherit_provider_credentials=False,
        extra_env=extra_env,
    )


def _safe_install_detail(value: object) -> str:
...
# ``<FAN_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def fan_lsp_bin_dir() -> Path:
    """Return the Fan-owned bin staging dir for LSP servers."""
    home = os.environ.get("FAN_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".fan")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(fan_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = fan_lsp_bin_dir().parent  # <FAN_HOME>/lsp/
    ...
    try:
        ...
        if proc.returncode != 0:
            logger.warning(
                ...
                pkg,
                _safe_install_detail(proc.stderr)[:500],
            )
            return None
    except (subprocess.TimeoutExpired, OSError) as e:
        logger.warning(
            "[install] npm install errored for %s: %s",
            pkg,
            _safe_install_detail(e),
        )
        return None
...
def _install_go(pkg: str, bin_name: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            "[install] go install errored for %s: %s",
            pkg,
            _safe_install_detail(e),
        )
        return None
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a fan-owned target dir.
    ...
    console script.
    """
    pip_target = fan_lsp_bin_dir().parent / "python-packages"
    ...
    try:
        ...
        logger.warning(
            "[install] pip install errored for %s: %s",
            pkg,
            _safe_install_detail(e),
        )
        return None
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = fan_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``fan lsp status`` CLI to give users a quick
