"""Language Server Protocol (LSP) integration for Fan Agent.

Fan runs full language servers (pyright, gopls, rust-analyzer,
...
checks. This keeps ad-hoc sessions on user-home cwd's from spawning daemons
they don't need.
...
def get_service() -> Optional[LSPService]:
    ...
    with _service_lock:
        ...
        if not _atexit_registered:
            ...
            # ``fan chat`` exit would leak pyright processes that
