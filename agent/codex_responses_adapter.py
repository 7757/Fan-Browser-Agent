...
# Responses rejects replayed assistant item ids above this cap.  Codex can
# emit server-side/base64 ids hundreds of characters long, while Fan's short
# ``msg_…`` ids are still safe and useful for cache affinity.
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Fan threading encrypted reasoning back across
            ...
            if role == "assistant":
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Fan-side metadata key and not part
                ...
                if isinstance(codex_message_items, list):
                    for raw_item in codex_message_items:
                        ...
                        # Copilot binds assistant item ids to a short-lived
                        # backend connection.  They become invalid after pool
                        # rotation/restart, so never replay them there.  Other
                        # providers retain only ids within the API's 64-char
                        # input limit.
                        if (
    ...
    *,
    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            if message_text:
                # Codex commentary/analysis is mid-turn protocol narration,
                # not the user-visible final answer. Keep it for replay below
                # but route the text through the reasoning channel.
