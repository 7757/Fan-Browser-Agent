...
def _merge_request_overrides(
    ...
    """Combine resolver metadata with the curator slot's extra request body."""
...
# The LLM umbrella-building pass is opinionated, potentially destructive, and
# can be expensive. Keep deterministic stale/archive transitions enabled while
# requiring an explicit opt-in for consolidation.
...
def _state_file() -> Path:
    return get_fan_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.fan/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `fan update` re-seeds.
    External-directory skills are never pruned regardless of this flag.
    """
    cfg = _load_config()
...
def get_consolidate() -> bool:
    """Whether the curator may run the LLM umbrella-building pass."""
    cfg = _load_config()
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``fan update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``fan curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `fan curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Fan' background skill CURATOR. This is an "
    ...
    "1. DO NOT touch bundled or external-dir skills "
    "(`skills.external_dirs`). The candidate list below is filtered to "
    "local curator-managed skills only.\n"
    "2. DO NOT delete any skill. Archiving (moving the skill's directory "
    "into ~/.fan/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: fan-config-*, fan-dashboard-*, gateway-*, codex-*, "
    "ollama-*, gemini-*, mcp-*, salvage-*, pr-*, "
    ...
    "~/.fan/skills/<umbrella>/references/ && mv ... <umbrella>/"
    ...
    "  - terminal                       — only move LOCAL candidate content "
    "when package integrity requires it; never rewrite external-dir skills\n\n"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.fan/logs/curator/``)
    ...
    authored skill data in ``~/.fan/skills/``.

    ``ensure_fan_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_fan_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every fan install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: fan curator status
        keep an umbrella stable: fan curator pin document-tools
    ...
    lines.append("full report: fan curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: fan curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.fan/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.fan/skills/.archive/` for "
            "safety and can be restored via `fan curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.fan/skills/.archive/`. "
            "Restore any via `fan curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `fan curator restore <name>`")
    lines.append("- All archives live under `~/.fan/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    Steps:
      ...
      2. When consolidation is explicitly enabled and there are agent-created
         skills, spawn a forked AIAgent for the umbrella-building review.
    ...
    *consolidate* defaults to ``curator.consolidate`` (off by default). When
    false, the run remains deterministic and never creates an auxiliary Agent.
    ...
    # real pass out. We still record a summary so `fan curator status`
    ...
    def _llm_pass():
        ...
        if not consolidate:
            ...
            try:
                ...
                logger.debug("Curator report write failed: %s", exc, exc_info=True)
            save_state(state2)
            if on_summary:
                try:
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `fan update` only if "
        ...
        # recorded in state so `fan curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``fan model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `fan model` → auxiliary
    ...
    try:
        ...
        # Bind this fork to the autonomous-curator write origin.  The skill
        # manager's external/bundled/hub write guards intentionally apply only
        # to background maintenance, not to a foreground user-directed edit.
