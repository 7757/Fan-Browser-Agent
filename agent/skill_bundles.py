...
Storage
-------
Bundles live in ``~/.fan/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``FAN_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``fan bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under FAN_HOME.

    Honors ``FAN_BUNDLES_DIR`` for tests; falls back to
    ``<FAN_HOME>/skill-bundles``.
    """
    override = os.environ.get("FAN_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_fan_home() / "skill-bundles"
...
def resolve_bundle_command_key(command: str) -> Optional[str]:
    ...
    Hyphens and underscores are treated interchangeably so command names
    remain stable across local clients.
    """
    if not command:
        return None
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `fan bundles` CLI subcommand.
# ---------------------------------------------------------------------------
