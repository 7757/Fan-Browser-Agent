...
See the ``fan-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Fan, e.g. 'fan-agent').\n"
    "Pinned skills (marked via 'fan curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("FAN_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Fan' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Fan
            ...
            # Match the parent's toolset config so the request prefix remains
            # byte-identical for providers that support prompt caching.
            # (The runtime whitelist below still restricts dispatch.)
            _fork_kwargs: Dict[str, Any] = {}
            _parent_max_tokens = getattr(agent, "max_tokens", None)
            if isinstance(_parent_max_tokens, int):
                _fork_kwargs["max_tokens"] = _parent_max_tokens
            _parent_acp_command = getattr(agent, "acp_command", None)
            if isinstance(_parent_acp_command, str) and _parent_acp_command:
                _fork_kwargs["acp_command"] = _parent_acp_command
                _fork_kwargs["acp_args"] = list(
                    getattr(agent, "acp_args", []) or []
                )
            review_agent = AIAgent(
                model=agent.model,
                max_iterations=16,
                quiet_mode=True,
                # Internal review forks remain durable for diagnostics but are
                # not user-visible conversations. Tag them as tool sessions so
                # history/resume/search never surfaces them as top-level chats.
                platform="tool",
            ...
            # This short-lived fork is intentionally restricted to memory and
            # skill review tools. Refreshing the foreground Agent's late MCP
            # snapshot here adds work and can expose tools the review harness
            # is not allowed to dispatch.
            review_agent._skip_mcp_refresh = True
            # The review fork has no desktop workbench of its own. Keep the
            # foreground browser out of this memory/skill-only lifecycle.
            review_agent._skip_browser_grounding = True
            review_agent._memory_store = agent._memory_store
            review_agent._memory_enabled = agent._memory_enabled
            review_agent._user_profile_enabled = agent._user_profile_enabled
            review_agent._memory_nudge_interval = 0
            review_agent._skill_nudge_interval = 0
            # The fork reuses the foreground session id for prompt-cache
            # parity. Never let its review harness or reply persist into that
            ...
            # the review fork's outbound HTTP request hits the same provider
            # prefix cache the parent warmed.
            # Without this, the fork rebuilds the system prompt from
            # scratch (fresh _fan_now() timestamp, fresh
            ...
            # This fork shares the foreground agent's live session id for
            # prompt-cache parity; closing it must never end the parent row.
            ...
            # parent into a NEW child that the gateway never adopts (the fork is
            # single-lifecycle and dies right after this run_conversation). The
            # foreground turn would then start from the stale parent and compress
            # it again, leaving the same parent with two sibling children. Review
            # also needs full context for a good memory/skill summary. Both
            # compression triggers in conversation_loop.py gate on
            # agent.compression_enabled, so this short-circuits both.
