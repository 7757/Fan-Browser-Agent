...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.fan/models_dev_cache.json)
...
class ModelInfo:
    ...
    name: str
    family: str
    provider_id: str        # models.dev provider ID (e.g. "alibaba")
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Fan ↔ models.dev
# ---------------------------------------------------------------------------

# Local provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    return get_fan_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``fan config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a local provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Fan agent traffic.
...
def get_model_info(
    ...
    Accepts Fan or models.dev provider ID.  Tries exact match then
