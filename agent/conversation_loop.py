...
from agent.browser_state_note import (
    escape_control_envelope_tags,
    merge_into_ephemeral,
    render_browser_grounding_unavailable_note,
)
from agent.browser_tool_protocol import coalesce_browser_type_calls
...
from fan_logging import set_session_context
from tools.skill_provenance import set_current_write_origin
from utils import base_url_host_matches, env_var_enabled

logger = logging.getLogger(__name__)

# Lazily-built, process-wide RPC client for the read-only liveState pull used
# to refresh the embedded-browser ground-truth note each LLM iteration. Reused
# across turns/sessions; cheap to keep around (just a URL + token holder).
_iter_browser_state_client = None
_iter_browser_observe_client = None

# A truncated response may be recoverable, but recovery must stay bounded. Keep
# text continuations and tool-call retries on the same contract so their logs,
# error messages, and actual number of follow-up requests cannot drift apart.
MAX_TRUNCATION_RECOVERY_RETRIES = 3
...
def _prepare_tool_turn_fallback_state(agent: Any, tool_calls: Any) -> bool:
    """Classify a tool turn and invalidate stale housekeeping state."""
    calls = list(tool_calls or ())
    all_housekeeping = bool(calls) and all(
        call.function.name in _HOUSEKEEPING_TOOLS for call in calls
    )
    if calls and not all_housekeeping:
        agent._last_content_with_tools = None
        agent._last_content_tools_all_housekeeping = False
        # A prior housekeeping turn may have muted progress. Substantive
        # tools in this turn must remain visible while they execute.
        agent._mute_post_response = False
    return all_housekeeping


def _browser_guardrail_is_blocked(
    decision: Any,
    navigation_failure: Any,
) -> bool:
    """Classify a browser circuit-break stop as a failed/blocked turn."""

    if decision is None:
        return False
    code = str(getattr(decision, "code", "") or "")
    return code.startswith("browser_no_progress") or code.startswith(
...
def _finalize_turn_status(
    *,
    ...
    """Return ``(completed, failed, blocked)`` without conflating a halt with success."""

    guardrail_halted = guardrail_decision is not None
    browser_blocked = _browser_guardrail_is_blocked(
        guardrail_decision,
        navigation_failure,
    )
    effective_failed = bool(failed or browser_blocked)
    normal_text_response = str(turn_exit_reason).startswith("text_response(")
    completed = (
        final_response is not None
        and not effective_failed
        and not unresolved_browser_action
        and not guardrail_halted
        and (api_call_count < max_iterations or normal_text_response)
    )
    return completed, effective_failed, browser_blocked


def _browser_guardrail_blocked_response(
    decision: Any,
    navigation_failure: Any,
) -> str:
    """Build a truthful terminal message for a browser no-progress halt."""

    if not isinstance(navigation_failure, dict):
        return _browser_guardrail_takeover_response(decision)
    ...
    description = _one_line(
        navigation_failure.get("errorDescription")
        or navigation_failure.get("error")
        or navigation_failure.get("code")
    )
    target = _one_line(
        navigation_failure.get("requestedUrl")
        or navigation_failure.get("validatedUrl")
    )
    network_code = navigation_failure.get("networkErrorCode")
    has_network_code = (
        network_code is not None and bool(str(network_code).strip())
    )
    code_suffix = (
        f" (network error code {network_code})"
        if has_network_code
        else ""
    )
    friendly_navigation_errors = {
        "ERR_CONNECTION_TIMED_OUT": "The connection to the target website timed out",
        "ERR_PROXY_CONNECTION_FAILED": "The proxy server could not be reached",
        "ERR_NAME_NOT_RESOLVED": "The target website address could not be resolved",
        "ERR_INTERNET_DISCONNECTED": "The device is currently offline",
        "ERR_CONNECTION_REFUSED": "The target website refused the connection",
    }
    friendly_description = friendly_navigation_errors.get(description)
    if friendly_description:
        network_detail = f", network error code {network_code}" if has_network_code else ""
        problem_text = (
            f"{friendly_description} ({description}{network_detail})"
        )
    else:
        problem_text = f"Website navigation failed: {description}{code_suffix}"
    target_text = f" {target}" if target else " the target page"
    decision_code = str(getattr(decision, "code", "") or "")
    if decision_code.startswith("browser_navigation_failure"):
        middle = "Waiting on the current error page will not restore access, so I stopped this path."
    else:
        middle = "The page remained in an error state, and further automatic retries could not recover it."
    return (
        f"I could not open{target_text}, so the current task is not complete.\n\n"
        f"Problem: {problem_text}.\n"
        f"{middle} Check the network or proxy configuration. After connectivity is "
        "restored, return to the chat and type \"continue\"; I will read the page "
        "again and continue from the current state."
    )


def _browser_guardrail_takeover_response(decision: Any) -> str:
    """Explain a browser halt as an actionable handoff, without tool jargon."""

    raw_context = getattr(decision, "user_context", None)
    ...
    attempted_action = _one_line(
        context.get("attempted_action")
        or "complete the previous unsuccessful page operation",
        limit=180,
    )
    page_url = _one_line(context.get("page_url"), limit=240)
    status = _one_line(context.get("status"), limit=80).lower()
    error_code = _one_line(context.get("error_code"), limit=120)
    error_message = _one_line(context.get("error_message"), limit=240)
    count = int(getattr(decision, "count", 0) or 0)

    diagnostic = f"{error_code} {error_message}".lower()
    uncertain_effect = status == "unknown_after_effect"
    if (
        uncertain_effect
        or "timeout" in diagnostic
        or "timed out" in diagnostic
        or "still-settling" in diagnostic
    ):
        problem = (
            "the page operation timed out while the page may still be loading or "
            "processing it, so I cannot confirm whether it took effect"
        )
        check_first = True
    elif "stale" in diagnostic:
        problem = "the page updated and invalidated the control located earlier"
        check_first = False
    elif (
        status == "needs_replan"
        or "replan" in diagnostic
        or "cannot find" in diagnostic
        or "not found" in diagnostic
        or "target_mismatch" in diagnostic
    ):
        problem = "the page did not contain the control or content needed for this step"
        check_first = False
    elif (
        "unavailable" in diagnostic
        or "queue_closed" in diagnostic
        or "webcontentsview" in diagnostic
    ):
        problem = "automated browser control is temporarily unavailable"
        check_first = False
    elif count > 0:
        problem = (
            f"after {count} no-progress attempts, the page still showed no change that "
            "could confirm the operation succeeded"
        )
        check_first = False
    else:
        problem = "the browser did not return enough information to confirm success"
        check_first = False

    page_line = f"\nCurrent page: {page_url}" if page_url else ""
    if check_first:
        takeover_steps = (
            f"1. Check the browser on the left to see whether \"{attempted_action}\" "
            "already took effect.\n"
            "2. If it did not, complete this step manually. Do not repeat an operation "
            "that already succeeded.\n"
            "3. When the target page or result appears, return to the chat and type "
            "\"continue\"."
        )
    else:
        takeover_steps = (
            f"1. Complete \"{attempted_action}\" manually in the browser on the left.\n"
            "2. Handle any popup, verification challenge, or overlay first.\n"
            "3. When the target page or result appears, return to the chat and type "
            "\"continue\"."
        )

    return (
        "I paused browser automation because the current task is not complete.\n\n"
        f"Problem: while trying to \"{attempted_action}\", {problem}."
        f"{page_line}\n"
        "To avoid duplicate clicks, duplicate submissions, or an endless loop, I cannot "
        "continue automatically along the same path.\n\n"
        "You can take over this step now:\n"
        f"{takeover_steps}\n\n"
        "After you send \"continue\", I will read the current page again and continue "
        "from where you stopped without repeating an operation that already took effect. "
        "If you do not want to take over manually, reply \"try a different approach\"."
    )


def _attach_browser_state_note_to_request(
    api_messages: list[dict[str, Any]],
    note: str | None,
) -> str | None:
    """Attach request-local browser state without inventing a user turn.

    On the first model call the real user message is the natural owner of the
    request-local state. During a tool loop the latest tool result is newer and
    keeps the state next to the browser evidence it describes. In either case
    the canonical conversation is untouched and the provider never sees a
    synthetic ``role=user`` message that it could mistake for a new request.

    Returns the role that received the note, or ``None`` when no safe target
    exists.
    """
    if not api_messages or not isinstance(note, str) or not note.strip():
        return None

    last_user_idx = None
    for idx in range(len(api_messages) - 1, -1, -1):
        if str(api_messages[idx].get("role") or "").strip().lower() == "user":
            last_user_idx = idx
            break

    target_idx = None
    lower_bound = last_user_idx if last_user_idx is not None else -1
    for idx in range(len(api_messages) - 1, lower_bound, -1):
        if str(api_messages[idx].get("role") or "").strip().lower() == "tool":
            target_idx = idx
            break
    ...
    target = dict(api_messages[target_idx])
    content = target.get("content")
    if isinstance(content, str):
        content = escape_control_envelope_tags(content)
        separator = "\n\n" if content else ""
        target["content"] = f"{content}{separator}{block}"
        ...
        for part in content:
            if (
                ...
                escaped_part = dict(part)
                escaped_part["text"] = escape_control_envelope_tags(part["text"])
        ...
        target["content"] = (
            f"{escape_control_envelope_tags(str(content))}\n\n{block}"
        )
    api_messages[target_idx] = target
    return str(target.get("role") or "")


def _refresh_iter_browser_state_note(
    agent: Any,
    workbench_id: str,
    messages: list[dict[str, Any]] | None = None,
) -> None:
    """Refresh browser ground truth and bind this model request to its DOM.

    The decision token is correctness-critical: every state-dependent browser
    RPC emitted by the resulting assistant message is checked against it.  It is
    deliberately refreshed before *every* API request rather than throttled; a
    skipped refresh could bind a new model response to an older page.  Failures
    clear the token so the executor fails closed instead of reusing stale state.
    """
    force_observe = bool(getattr(agent, "_browser_force_observe", False))

    # The note is request-local truth, not a cache. Clear it before every
    # refresh so no early return or RPC failure can attach the previous
    # request's DOM as though it were current.
    merge_into_ephemeral(agent, None)
    agent._browser_decision_token = None
    if force_observe:
        # A forced refresh is a correctness boundary (for example after human
        # interaction). The previous observation identity must stop being
        # reusable before any liveState/observe RPC is attempted.
        agent._browser_model_visible_observation_token = None
        agent._browser_model_visible_observation_fingerprint = None

    def clear_visible_observation_identity() -> None:
        agent._browser_model_visible_observation_token = None
        agent._browser_model_visible_observation_fingerprint = None

    def grounding_unavailable(reason: str, error: Exception | None = None) -> None:
        clear_visible_observation_identity()
        agent._browser_decision_token = None
        agent._browser_force_observe = True
        merge_into_ephemeral(agent, render_browser_grounding_unavailable_note())
        event_logger = getattr(agent, "logger", None) or logger
        event_logger.warning(
            "[grounding] status=BROWSER_GROUNDING_UNAVAILABLE "
            "reason=%s workbench=%s error_type=%s force_observe=true",
            reason,
            str(workbench_id or "-"),
            type(error).__name__ if error is not None else "-",
        )

    if getattr(agent, "_skip_browser_grounding", False):
        clear_visible_observation_identity()
        return
    try:
        # enabled_toolsets=None means every toolset is enabled. With an
        # explicit list, refresh browser grounding only for the program
        # interface or the deliberately selected legacy compatibility set.
        toolsets = getattr(agent, "enabled_toolsets", None)
        if toolsets is not None and not {
            "browser_program",
            "electron_browser",
        }.intersection(toolsets):
            clear_visible_observation_identity()
            return
        if not workbench_id:
            grounding_unavailable("missing_workbench_id")
            return
        global _iter_browser_state_client
        if _iter_browser_state_client is None:
            from agent.electron_browser_client import ElectronBrowserClient

            # liveState normally completes in milliseconds.  Keep the bound
            # small, but leave enough headroom for a busy Electron main thread.
            _iter_browser_state_client = ElectronBrowserClient(timeout=1.0)
        if not getattr(_iter_browser_state_client, "available", False):
            # 不可用就别缓存这个"定格了空 URL/token"的实例——置回 None,待 env 到位后下次重建。
            _iter_browser_state_client = None
            grounding_unavailable("live_state_client_unavailable")
            return
        ...
        cursor = getattr(agent, "_browser_state_cursor", None)
        if not isinstance(cursor, dict):
            cursor = {}
            agent._browser_state_cursor = cursor
        state = None
        live_token = None
        visible_token = getattr(
            agent, "_browser_model_visible_observation_token", None
        )
        visible_fingerprint = getattr(
            agent, "_browser_model_visible_observation_fingerprint", None
        )
        history_has_observation = False
        history_has_bound_observation = False

        if not force_observe:
            state = _iter_browser_state_client.call(
                "liveState", workbench_id=workbench_id
            )
            live_token = decision_token_from_live_state(state, workbench_id)
            if not isinstance(live_token, dict):
                grounding_unavailable("live_state_token_unavailable")
                return
            history_has_observation = any(
                isinstance(message, dict)
                and message.get("role") == "tool"
                and browser_result_contains_page_observation(message.get("content"))
                for message in (messages or [])
            )
            history_has_bound_observation = bool(
                isinstance(visible_fingerprint, str)
                and visible_fingerprint
                and any(
                    isinstance(message, dict)
                    and message.get("role") == "tool"
                    and browser_observation_content_fingerprint(
                        message.get("content")
                    )
                    == visible_fingerprint
                    for message in (messages or [])
            ...
            token_mismatch = (
                isinstance(visible_token, dict)
                and visible_token != live_token
            ) or (visible_token is None and history_has_observation)
            observation_missing = (
                isinstance(visible_token, dict)
                and not history_has_bound_observation
            )
            if token_mismatch or observation_missing:
                # Persist the barrier before the observe call. Any RPC failure
                # below must leave the next iteration fail-closed as well.
                force_observe = True
                agent._browser_force_observe = True
                clear_visible_observation_identity()
        ...
        if force_observe:
            global _iter_browser_observe_client
            if _iter_browser_observe_client is None:
                from agent.electron_browser_client import ElectronBrowserClient

                _iter_browser_observe_client = ElectronBrowserClient(timeout=8.0)
            if not getattr(_iter_browser_observe_client, "available", False):
                _iter_browser_observe_client = None
                grounding_unavailable("observe_client_unavailable")
                return
            observed = _iter_browser_observe_client.call(
                "observe",
                workbench_id=workbench_id,
                params={"_fanPassiveRead": True},
            )
            if not isinstance(observed, dict) or observed.get("__error__"):
                grounding_unavailable("observe_result_unavailable")
                return
            observe_succeeded = True
            raw_token = observed.get("__fanDecisionToken")
            if isinstance(raw_token, dict):
                observation_token = dict(raw_token)
            from tools.electron_browser_serialization import (
                format_observation_for_model,
            )

            forced_dom = format_observation_for_model(observed)

        # Read liveState after a forced observe. The observe rebuilds the
        # selector map, so a token captured before it would immediately be
        # stale. The response token must also match this post-observe state;
        # otherwise the page raced again between the two RPCs.
        if force_observe:
            state = _iter_browser_state_client.call(
                "liveState", workbench_id=workbench_id
            )
        ...
        observation_bound = bool(
            force_observe
            and observe_succeeded
            and isinstance(observation_token, dict)
            and observation_token == live_token
        )
        if force_observe and not observation_bound:
            grounding_unavailable("post_observe_token_mismatch")
            return
        observation_current = bool(
            observation_bound
            if force_observe
            else (
                history_has_observation
                and history_has_bound_observation
                and isinstance(visible_token, dict)
                and visible_token == live_token
            )
        )

        agent._browser_decision_token = live_token
        note = render_live_state_note(
            state,
            cursor,
            observe_fn=(lambda: forced_dom) if forced_dom else None,
            force_observe=force_observe,
            observation_current=observation_current,
        )
        merge_into_ephemeral(agent, note)
        if observation_bound:
            # ``forced_dom`` is request-local ephemeral context, not canonical
            # history.  Only tool_executor may mark a snapshot reusable after
            # that same complete observation has been appended to ``messages``.
            agent._browser_model_visible_observation_token = None
            agent._browser_model_visible_observation_fingerprint = None
            agent._browser_force_observe = True
    except Exception as exc:
        grounding_unavailable("refresh_exception", exc)
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Fan tool use: "
    ...
    return (
        ...
        f"context, but Fan needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Fan config, set `model.ollama_num_ctx: 65536` "
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt:
        ...
        # previous turn so provider cache prefixes remain stable.
...
def _claim_truncation_retry(retries_started: int) -> Optional[int]:
    """Return the next 1-based retry number, or ``None`` when exhausted."""
    current = max(0, int(retries_started or 0))
    if current >= MAX_TRUNCATION_RECOVERY_RETRIES:
        return None
    return current + 1
...
_CONTENT_POLICY_RECOVERY_HINT = (
    "Try rephrasing the request or narrowing the context."
)
...
def _sync_failover_system_message(
    ...
    history_overlays=None,
):
    """Refresh this in-flight call block after a fallback runtime switch."""
    system_prompt = getattr(agent, "_cached_system_prompt", None)
    if not isinstance(system_prompt, str) or not system_prompt:
        return active_system_prompt
    if api_messages and api_messages[0].get("role") == "system":
        prefill_overlays, _ = partition_privileged_messages(agent.prefill_messages)
...
def run_conversation(
    ...
    # Keep the credential-refresh circuit breaker scoped to one user turn.
    # Otherwise a stale OAuth entry can consume every retry forever, while a
    # successful later turn would inherit an old failure count.
    ...
    # ``fan logs --session <id>`` can filter a single conversation.
    ...
    # MCP discovery is intentionally backgrounded so a slow server cannot
    # delay desktop/CLI startup. A server may therefore register after this
    # Agent took its build-time tool snapshot. Refresh only at the boundary
    # before a new turn starts: this makes late tools callable without
    # mutating an in-flight provider request or its cached tool prefix.
    try:
        # Avoid importing the optional MCP SDK on the no-MCP first-turn path.
        # Registered MCP tools imply discovery already imported this module.
        import sys as _sys

        if (
            not getattr(agent, "_skip_mcp_refresh", False)
            and "tools.mcp_tool" in _sys.modules
        ):
        ...
        logger.debug("Between-turn MCP tool refresh skipped", exc_info=True)
    ...
    # producing a different system prompt and breaking provider prefix caches.
    if agent._cached_system_prompt is None:
        _restore_or_build_system_prompt(agent, system_message, conversation_history)

    active_system_prompt = agent._cached_system_prompt

    # Create the DB row only after the cached system prompt exists. Otherwise
    # a first gateway/API turn persists system_prompt=NULL and a later resume
    # has to rebuild its prefix instead of reusing the stable snapshot.
    ...
    if (
        ...
        and (
            len(messages) > agent.context_compressor.protect_first_n
        ...
        if _preflight_deferred:
            ...
            for _pass in range(3):
                ...
                _made_progress = (
                    len(messages) < _orig_len
                    or (_orig_tokens > 0 and _new_tokens < _orig_tokens * 0.95)
                )
                if not _made_progress:
                    break  # Cannot compress materially further
    ...
    # are reused.  The system prompt is Fan's territory; plugins
    ...
    try:
        ...
        _ctx_parts: list[str] = []
        # Hook output is injected on every model request in this turn. Spill
        # each oversized contribution before aggregation so one runaway hook
        # cannot repeatedly consume the request context window.
        try:
            from tools.hook_output_spill import (
                get_spill_config as _get_spill_config,
                spill_if_oversized as _spill_if_oversized,
            )

            _spill_config = _get_spill_config()
        except Exception:
    ...
    last_outer_error_signature = None
    last_outer_error_api_call = None
    consecutive_outer_error_count = 0
    ...
    agent._turn_unresolved_browser_action: Dict[str, Any] = {}
    # Populated by tool_executor only for a structured
    # NAVIGATION_FAILED/NAVIGATION_TIMEOUT result. It is evidence for a later
    # circuit-break message, not a reason to halt after the first failure.
    agent._turn_browser_navigation_failure: Dict[str, Any] | None = None
    ...
    # all run inside Codex). Default Fan path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Per-iteration embedded-browser ground truth. The gateway already
        # refreshes once at turn start, but a multi-iteration turn can navigate
        # / crash / trip a native dialog between model calls, so we re-pull the
        # read-only liveState before every API request and rewrite the
        # [实时浏览器状态] block. This pull is also the correctness boundary
        # that captures the DOM decision token, so it is never throttled or
        # reused across API requests. Failures clear the token and browser
        # actions fail closed until an explicit observe establishes fresh state.
        _refresh_iter_browser_state_note(agent, effective_task_id, messages)

        history_overlays, _ = partition_privileged_messages(messages)
        api_messages = []
        for idx, msg in enumerate(messages):
            if str(msg.get("role") or "").strip().lower() in {
                "system",
                "developer",
            }:
                continue
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                ...
                # 实时浏览器状态在 API 消息完成清洗后单独处理：首轮附到
                # 真实 user 消息，工具循环附到最新 tool 结果。绝不创建一条
                # 新的 user 消息，否则模型可能把运行时状态误认为新请求。
                if _injections:
        ...
        # Build the final system message: cached prompt + ephemeral overlay +
        # privileged prefill overlays + the mandatory security boundary. The
        # boundary is always last inside the system message. Ephemeral additions
        # are API-call-time only (not persisted to session DB).
        ...
        # cache prefix.  The system prompt is reserved for Fan internals.
        #
        # Fan invariant: the system prompt is built ONCE per session
        # (cached on ``_cached_system_prompt``) and replayed verbatim on
        # every turn.  We send it as a single content string so the
        # bytes are byte-stable across turns and upstream prompt caches
        # stay warm.
        prefill_overlays, conversation_prefills = partition_privileged_messages(
        ...
        if conversation_prefills:
            sys_offset = 1 if (api_messages and api_messages[0].get("role") == "system") else 0
            for idx, pfm in enumerate(conversation_prefills):
                api_messages.insert(sys_offset + idx, pfm.copy())

        # Explicit cache markers are enabled only for compatible Qwen/Alibaba
        # endpoints. Other providers may cache prefixes automatically.
        ...
        # left behind. Some gateways cannot replay a thinking-only turn.
        # Runs on the per-call copy only — the
        ...
        # Browser ground truth stays request-local and recent without becoming
        # a fake user turn. On the initial request it is appended to the real
        # user message; after tools it is appended to the latest tool result.
        # Tool messages are never cache breakpoints, so the stable prefix stays
        # intact while the current state remains adjacent to current evidence.
        _bs_note = getattr(agent, "_browser_state_note", None)
        _attach_browser_state_note_to_request(api_messages, _bs_note)

        # Proactively strip any surrogate characters before the API call.
        # Models served via Ollama (Kimi K2.5, GLM-5, Qwen) can return
        # lone surrogates (U+D800-U+DFFF) that crash json.dumps() inside
        # the OpenAI SDK. Sanitizing after browser-state attachment also covers
        # page-controlled title and URL text inside that request-local note.
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Fan tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        input_moderation_retry_attempted = False
            ...
            try:
                ...
                try:
                    ...
                    if has_hook("pre_api_request"):
                        ...
                        # kwargs below are pre-existing raw passthroughs for
                        # hook consumers. They predate ``request`` and are intentionally NOT
                ...
                if env_var_enabled("FAN_DUMP_REQUESTS"):
                ...
                def _perform_api_call(next_api_kwargs):
                    # Request middleware may mutate the payload after the
                    # initial preflight.  Reapply endpoint-aware sanitization
                    # at the final dispatch boundary so a Copilot connection-
                    # scoped message id cannot be reintroduced.
                    if agent.api_mode == "codex_responses":
                ...
                if response_invalid:
                    ...
                    if agent._try_activate_fallback():
                        active_system_prompt = _sync_failover_system_message(
                            agent, api_messages, active_system_prompt, history_overlays
                        )
                        retry_count = 0
                        compression_attempts = 0
                        primary_recovery_attempted = False
                ...
                # OpenAI-compatible ``finish_reason="content_filter"`` or a
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Fan/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if finish_reason == "length":
                    ...
                    if agent.api_mode in {"chat_completions", "bedrock_converse"}:
                        assistant_message = _trunc_msg
                        # A stream can end after delivering partial content
                        # because the provider's output safety policy blocked
                        # it.  The helper marks that otherwise-hidden error on
                        # the stub.  Retrying a continuation on the same model
                        # simply repeats the block, so hand the clean message
                        # state to the configured fallback first.
                        if (
                            getattr(response, "_content_filter_terminated", False)
                        ...
                        if assistant_message is not None and not _trunc_has_tool_calls:
                            ...
                            _next_length_retry = _claim_truncation_retry(length_continue_retries)
                            if _next_length_retry is not None:
                                length_continue_retries = _next_length_retry
                                ...
                                if _is_partial_stream_stub and _dropped_tools:
                                    ...
                                    agent._vprint(
                                        ...
                                        f"chunked retry "
                                        f"({length_continue_retries}/{MAX_TRUNCATION_RECOVERY_RETRIES})..."
                                    )
                            ...
                            return {
                                ...
                                "error": (
                                    ...
                                    f"{MAX_TRUNCATION_RECOVERY_RETRIES} continuation attempts"
                                ),
                            }
                    ...
                    if agent.api_mode in {"chat_completions", "bedrock_converse"}:
                        ...
                        if assistant_message is not None and _trunc_has_tool_calls:
                            ...
                            _next_tool_retry = _claim_truncation_retry(truncated_tool_call_retries)
                            if _next_tool_retry is not None:
                                truncated_tool_call_retries = _next_tool_retry
                                if _is_stub_stall:
                                    ...
                                    agent._buffer_vprint(
                                        ...
                                        f"retrying ({truncated_tool_call_retries}/{MAX_TRUNCATION_RECOVERY_RETRIES})..."
                                    )
                                else:
                                    agent._buffer_vprint(
                                        f"⚠️  Truncated tool call detected — "
                                        f"retrying API call "
                                        f"({truncated_tool_call_retries}/{MAX_TRUNCATION_RECOVERY_RETRIES})..."
                                    )
                            ...
                            if _is_stub_stall:
                                agent._vprint(
                                    f"{agent.log_prefix}⚠️  Stream kept dropping mid tool-call after "
                                    f"{MAX_TRUNCATION_RECOVERY_RETRIES} retries — the action was not executed.",
                                    force=True,
                                )
                            else:
                                agent._vprint(
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # A usage-less response cannot judge the preceding
                    # compaction. Consume its marker now so preflight does not
                    # remain latched and unrelated future usage is not charged.
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # line was previously gated on explicit marker injection.
                    # ``canonical_usage`` is already normalised across the
                ...
                interrupted = True
                # The UI may already have received streamed output when the
                # user stops the call. Preserve that visible fragment in
                # durable history so the next turn does not lose context.
                ...
                _IMAGE_REJECTION_PHRASES = (
                    ...
                    "unknown variant `image_url`, expected `text`",
                    "unknown variant image_url, expected text",
                    # OpenRouter returns this 404 when none of the selected
                    # route's endpoints accept image input. Treat it as a
                    # capability rejection, strip the images once, and let
                    # the same user turn continue as text instead of
                    # repeatedly resubmitting the rejected payload.
                ...
                # Image-too-large recovery: shrink oversized image parts
                # in-place and retry once when a provider reports an image
                # size limit. If shrink fails or a second attempt still
                ...
                # Input-moderation recovery: the provider's pre-inference
                # content-safety net (e.g. DashScope/Bailian "DataInspectionFailed")
                # rejected an input image with a hard 400 — the model never ran, so
                # there is no graceful refusal to surface. Reuse the multimodal
                # downgrade: strip image parts from tool messages, record the
                # (provider, model) so subsequent screenshots auto-downgrade to text
                # for the rest of this session, and retry once. This turns a
                # session-killing raw 400 into "continue with page text".
                if (
                    classified.reason == FailoverReason.input_moderation_blocked
                    and not input_moderation_retry_attempted
                ):
                    input_moderation_retry_attempted = True
                    if agent._try_strip_image_parts_from_tool_messages(api_messages):
                        agent._vprint(
                            f"{agent.log_prefix}🛡️ 页面截图被内容安全审核拦截 — "
                            f"已改用页面文本继续（本会话后续截图将只发文本）...",
                            force=True,
                        )
                        continue
                    else:
                        logger.info(
                            "input-moderation recovery: no list-type tool messages "
                            "with image parts found; surfacing original error."
                        )
                ...
                # (413 payload-too-large and context-overflow). Without this
                # guard the proactive
                ...
                # Output-cap errors are not input overflow. Their recovery only
                # lowers this request's max_tokens and retries, so it remains
                # valid when automatic context compression is disabled. Some
                # vLLM/LM Studio messages are not recognized by the generic
                # classifier but still expose a parseable available-token cap.
                ...
                # Eager fallback for capacity and persistent transport errors.
                # Quota/billing failures switch immediately; a timeout or
                # overloaded provider gets one genuine retry first, then moves
                # to the configured fallback rather than consuming the full
                # retry budget against a dead primary.
                ...
                # Z.AI Coding Plan reports GLM-5.2 saturation as HTTP 429
                # code 1305, correctly classified as an overloaded service
                # rather than a credential rate limit. Give this narrow case
                # enough retry budget to reach its adaptive long-backoff tier.
                ...
                should_eager_fallback = (
                    is_rate_limited
                    or (is_transport_failure and retry_count >= 2)
                )
                if should_eager_fallback and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    if not pool_may_recover:
                        if classified.reason == FailoverReason.billing:
                            ...
                            agent._buffer_status(
                                "⚠️ Provider is unreachable — switching to fallback provider..."
                            )
                        else:
                ...
                # which Fan' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Fan' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`fan",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_payload_too_large:
                    ...
                    if len(messages) < original_len or (
                        original_tokens > 0 and new_tokens < original_tokens * 0.95
                    ):
                ...
                if is_context_length_error:
                    ...
                    # The provider says the requested output cap itself is
                    # invalid, but did not expose a numeric limit we can parse.
                    # Compression cannot change max_tokens, so entering the
                    # context-recovery loop would deterministically repeat the
                    # same 400 until the session is exhausted.
                    if is_output_cap_error(error_msg):
                        agent._flush_status_buffer()
                        agent._vprint(
                            f"{agent.log_prefix}❌ The provider rejected max_tokens "
                            "because it exceeds this model's output cap.",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}   💡 Lower model.max_tokens in "
                            "config.yaml. This is an output-cap error; context "
                            "compression cannot fix it.",
                            force=True,
                        )
                        logger.error(
                            "%sOutput-cap error not routed into compression: %s",
                            agent.log_prefix,
                            error_msg[:200],
                        )
                        agent._persist_session(messages, conversation_history)
                        return {
                            ...
                            "api_calls": api_call_count,
                            "error": (
                                "max_tokens exceeds the provider's output cap "
                                "for this model. Lower model.max_tokens."
                            ),
                            "partial": True,
                            "failed": True,
                        }
                    ...
                    if (
                        ...
                        or (original_tokens > 0 and new_tokens < original_tokens * 0.95)
                        or (new_ctx and new_ctx < old_ctx)
                    ):
                        if len(messages) < original_len:
                            agent._buffer_status(f"🗜️ Compressed {original_len} → {len(messages)} messages, retrying...")
                        elif original_tokens > 0 and new_tokens < original_tokens * 0.95:
                            agent._buffer_status(
                                f"🗜️ Compressed ~{original_tokens:,} → ~{new_tokens:,} tokens, retrying..."
                            )
                        time.sleep(2)  # Brief pause between compression retries
                ...
                is_local_validation_error = (
                    ...
                    # as non-retryable (which left users staring at a bare
                    # "Non-retryable error" with no recovery).
                    and not (
                ...
                if is_client_error:
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._emit_status(
                            f"❌ TLS certificate verification failed: {_error_summary}"
                        )
                    else:
                        agent._emit_status(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Restart Fan after refreshing the Codex CLI login.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `fan model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: fan setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        fan fallback add   (interactive picker — same as `fan model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Fan/gateway failure).\n\n"
                            f"Provider message: {_error_summary}\n\n"
                            f"{_CONTENT_POLICY_RECOVERY_HINT}"
                        )
                        return _content_policy_blocked_result(
                            messages,
                ...
                if retry_count >= max_retries:
                    ...
                    if not primary_recovery_attempted and agent._try_recover_primary_transport(
                        ...
                        retry_count = 0
                        # A rebuilt primary client starts a fresh attempt cycle.
                        # Re-open the fallback chain too: a failed eager
                        # fallback before recovery may have advanced its index,
                        # which would otherwise prevent a later 429 from
                        # reaching the configured backup model.
                        has_retried_429 = False
                    ...
                    # A long reasoning phase can be killed by an upstream
                    # proxy before the first visible token.  This is distinct
                    # from the large-tool-call stream-drop case below: telling
                    # a reasoning-model user to split file writes is actively
                    # misleading, while classifying it as overflow would have
                    # incorrectly compacted the session.
                    from agent.thinking_timeout_guidance import (
                        build_thinking_timeout_guidance,
                        is_thinking_timeout,
                    )

                    _is_thinking_timeout = (
                        not getattr(api_error, "status_code", None)
                        and is_thinking_timeout(classified, _model, error_msg)
                    )
                    ...
                    if _is_thinking_timeout:
                        agent._vprint(
                            f"{agent.log_prefix}   💡 推理阶段可能被上游网关超时中断，"
                            "并非上下文溢出。",
                            force=True,
                        )
                    ...
                    if _is_thinking_timeout:
                        _final_response += build_thinking_timeout_guidance(_provider, _model)
                    elif _is_stream_drop:
                        _final_response += (
                ...
                # For rate limits and the narrow Z.AI overload response,
                # respect Retry-After when the service provides one.
                _retry_after = None
                if is_rate_limited or _is_zai_coding_overload:
                    ...
                    if _resp_headers and hasattr(_resp_headers, "get"):
                        ...
                        if _ra_raw:
                            try:
                                # Providers can legitimately advertise several
                                # minutes of token-bucket reset time.  Respect
                                # it (while still bounding pathological values)
                                # instead of waking early and immediately
                                # consuming another failed request.
                ...
                if (is_rate_limited or _is_zai_coding_overload) and not _retry_after:
                    wait_time, backoff_policy = adaptive_rate_limit_backoff(
                        retry_count,
                        base_url=str(_base),
                        model=_model,
                        error=api_error,
                ...
                if is_rate_limited or _is_zai_coding_overload:
                    policy_note = ""
                    if backoff_policy == "zai_coding_overload_long":
                        policy_note = " (Z.AI Coding overload adaptive long backoff)"
                    elif backoff_policy == "zai_coding_overload_short":
                        policy_note = " (Z.AI Coding overload short retry)"
                    wait_reason = (
                        "Provider overloaded"
                    ...
                    agent._buffer_status(
                        f"⏱️ {wait_reason}. Waiting {wait_time:.1f}s "
                        f"(attempt {retry_count + 1}/{max_retries}){policy_note}..."
                    )
                else:
        ...
        if restart_with_rebuilt_messages:
            # The streamed response was stopped by a deterministic content
            # filter, then rolled back before switching providers.  Do not
            # spend an iteration on that unusable partial response.
            api_call_count -= 1
            agent.iteration_budget.refund()
            restart_with_rebuilt_messages = False
            continue
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                if invalid_tool_calls:
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if tool_name not in agent.valid_tool_names:
                            if not (tool_name or "").strip():
                                ...
                                content = f"Tool '{tool_name}' does not exist. Available tools: {available}"
                        else:
                ...
                original_tool_call_count = len(assistant_message.tool_calls)
                assistant_message.tool_calls = coalesce_browser_type_calls(
                    assistant_message.tool_calls,
                    available_tool_names=frozenset(
                        getattr(agent, "valid_tool_names", None) or ()
                    ),
                )
                normalized_tool_call_count = len(assistant_message.tool_calls)
                if normalized_tool_call_count < original_tool_call_count:
                    logger.info(
                        "Normalized adjacent browser_type batches (%d -> %d tool calls)",
                ...
                # Classify every tool turn, including tool-only turns. A stale
                # housekeeping fallback must not survive a later substantive
                # action and masquerade as that action's final response.
                _all_housekeeping = _prepare_tool_turn_fallback_state(
                    agent,
                    assistant_message.tool_calls,
                )
                ...
                try:
                    # Commit the exact assistant tool-call block before any
                    # side effect can restart or terminate the current process.
                ...
                if agent._tool_guardrail_halt_decision is not None:
                    ...
                    _navigation_failure = getattr(
                        agent,
                        "_turn_browser_navigation_failure",
                        None,
                    )
                    if _browser_guardrail_is_blocked(
                        decision,
                        _navigation_failure,
                    ):
                        final_response = _browser_guardrail_blocked_response(
                            decision,
                            _navigation_failure,
                        )
                    else:
                ...
                if not agent._has_content_after_think_block(final_response):
                    ...
                    if agent._has_content_after_think_block(_partial_streamed):
                        ...
                        # The renderer received only a stream fragment, not a
                        # confirmed final reply. Keep this false so the gateway
                        # can surface the recovery explanation with the text.
                    ...
                    if _truly_empty and agent._fallback_chain:
                        ...
                        if agent._try_activate_fallback():
                            ...
                            agent._empty_content_retries = 0
                            getattr(
                                agent, "_buffer_fallback_status", agent._buffer_status
                            )(
                ...
                try:
                    agent._clear_status_buffer(preserve_fallback_notice=True)
                except TypeError:
                    # Lightweight embedders that supply the historical
                    # zero-argument helper keep their existing behavior.
            ...
            outer_error_signature = (
                type(e).__module__,
                type(e).__qualname__,
                str(e),
            )
            if (
                outer_error_signature == last_outer_error_signature
                and last_outer_error_api_call is not None
                and api_call_count == last_outer_error_api_call + 1
            ):
                consecutive_outer_error_count += 1
            else:
                consecutive_outer_error_count = 1
            last_outer_error_signature = outer_error_signature
            last_outer_error_api_call = api_call_count
            try:
                print(f"❌ {error_msg}")
            ...
            # A deterministic internal failure cannot be repaired by asking the
            # model again. Stop after three consecutive identical failures so a
            # broken or mixed runtime cannot spin until the iteration budget is
            # exhausted and inflate memory/cost.
            if consecutive_outer_error_count >= 3:
                _turn_exit_reason = (
                    "repeated_outer_error("
                    f"{type(e).__qualname__}:{str(e)[:80]})"
                )
                failed = True
                final_response = (
                    "内部处理连续失败，已停止重试以避免循环。"
                ...
                messages.append({"role": "assistant", "content": final_response})
                agent._emit_status("⛔ 内部异常重复发生，已停止自动重试")
                logger.critical(
                    "Stopped turn after %d consecutive identical outer-loop errors",
                    consecutive_outer_error_count,
                )
                break
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("FAN_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    # Determine if conversation completed successfully. A controlled guardrail
    # halt still has a user-facing response, but that response explains why the
    # work stopped; it must never be counted as successful completion.
    unresolved_browser_action = dict(
        getattr(agent, "_turn_unresolved_browser_action", None) or {}
    )
    ...
    completed, failed, browser_blocked = _finalize_turn_status(
        final_response=final_response,
        failed=failed,
        unresolved_browser_action=unresolved_browser_action,
        api_call_count=api_call_count,
        max_iterations=agent.max_iterations,
        turn_exit_reason=_turn_exit_reason,
        guardrail_decision=_guardrail_decision,
        navigation_failure=_navigation_failure,
    )

    # Cleanup runs after the model has already produced the response. File
    # I/O, resource teardown, or SQLite failures must be reported without
    # throwing away that response, and one failed step must not skip the rest.
    ...
    try:
        ...
        # Recovery paths can produce a real final response without appending
        # the corresponding assistant turn to the transcript. Persisting that
        # shape leaves a user/tool tail even though the response was delivered,
        # causing the next resumed turn to re-answer apparent backlog. Keep
        # the durable conversation closed at this common finalization point.
    ...
    # A read-only observation cannot turn a failed click/navigation/input into
    # success. Surface any browser action that remained unresolved at turn end,
    # even if the model's prose incorrectly claimed completion.
    if final_response and not interrupted:
        try:
            from agent.browser_action_outcome import browser_action_failure_footer

            footer = browser_action_failure_footer(
                getattr(agent, "_turn_unresolved_browser_action", None)
            )
            if footer:
                final_response = final_response.rstrip() + "\n\n" + footer
        except Exception as _browser_ver_err:
            logger.debug("browser-action verifier footer failed: %s", _browser_ver_err)
    ...
    if not interrupted:
        try:
            if agent._turn_completion_explainer_enabled():
                ...
                # A recovered stream fragment can look like a complete long
                # response.  It still ended abnormally, so always disclose
                # that recovery and offer the user a safe continuation rather
                # than silently presenting it as a confirmed final answer.
    ...
    # providers (reasoning models and Codex Responses) emit
    ...
    result = {
        ...
        "browser_action_unresolved": unresolved_browser_action or None,
    ...
    if browser_blocked:
        result["error"] = (
            f"{getattr(_guardrail_decision, 'code', 'browser_no_progress_halt')}: "
            f"{final_response or 'browser task stopped before completion'}"
        )
    if _cleanup_errors:
