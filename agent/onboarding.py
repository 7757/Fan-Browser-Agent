...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Fan starts and finds ``~/.openclaw/``.

    Points users at ``fan claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``fan claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Fan, run "
        "`fan claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `fan claw cleanup` (renames it to ~/.openclaw.pre-migration — "
...
def profile_build_mode(config: Mapping[str, Any]) -> str:
    ...
    Returns ``"ask"`` (on first contact, OFFER to build a profile — default) or
    ``"off"`` (never offer). Read from ``config.onboarding.profile_build``.
    Unknown / missing values fall back to ``"ask"``.
    """
...
def profile_build_directive() -> str:
    ...
    lookup and never silently reads connected accounts.
    """
    return (
        "\n\n[System note: this is the user's first-ever message. After introducing "
        "yourself in one sentence and mentioning that /help shows available commands, "
        "proactively offer—but do not begin without permission—to build a brief user "
        "profile so you can assist them better in the future. Explain that they may "
        "decline or do it later. Only if they explicitly accept:\n"
        "  1. Ask what they are comfortable sharing, such as their preferred name, what "
        "they do, and how they want you to collaborate with them. Prefer facts they "
        "volunteer directly.\n"
        "  2. Before any external lookup, explain what you intend to look up and obtain "
        "explicit consent for that step. Never silently read connected accounts such as "
        "email or calendars; ask first every time.\n"
        "  3. With consent, you may use web_search to verify public details based on the "
        "information they supplied, such as an employer or public profile.\n"
        "  4. Save each confirmed, durable fact with the memory tool using "
        "target=\"user\". Keep entries compact and information-dense.\n"
        "If the user declines at any point, stop immediately and continue normally. Keep "
        "the process light and conversational, not like an interrogation.]"
    )
