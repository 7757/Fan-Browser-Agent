...
Owns one Codex thread per Fan session. Drives `turn/start`, consumes
...
# Fan' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_FAN_TO_CODEX_PERMISSION_PROFILE = {
...
def _combine_developer_instructions(
    configured_instructions: Any,
    mandatory_instructions: str,
) -> str:
    """Preserve the user's Codex workflow and keep Fan's boundary last."""

    configured = (
        configured_instructions.strip()
        if isinstance(configured_instructions, str)
        else ""
    )
    mandatory = mandatory_instructions.strip()
    if not configured:
        return mandatory
    if not mandatory or configured.endswith(mandatory):
        return configured
    return f"{configured}\n\n{mandatory}"
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Fan/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Fan session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _FAN_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("FAN_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
        self._approval_callback = approval_callback
        self._developer_instructions = str(developer_instructions or "").strip()
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="fan",
            client_title="Fan Agent",
            client_version=_get_fan_version(),
        )
        ...
        if self._developer_instructions:
            # Supplying thread/start.developerInstructions replaces (rather
            # than appends to) the effective Codex config value. Read the
            # already-layered value first so Fan does not discard the user's
            # global or project-specific workflow instructions.
            configured_instructions = ""
            try:
                config_result = self._client.request(
                    "config/read",
                    {"cwd": self._cwd, "includeLayers": False},
                    timeout=10,
                )
                effective_config = (
                    config_result.get("config") or {}
                    if isinstance(config_result, dict)
                    else {}
                )
                if isinstance(effective_config, dict):
                    configured_instructions = effective_config.get(
                        "developer_instructions"
                    )
            except Exception:
                # Keep the mandatory boundary fail-closed if effective config
                # lookup fails. All other Codex config still loads normally in
                # thread/start.
                logger.warning(
                    "codex app-server config/read unavailable; "
                    "using Fan developer instructions without config merge"
                )

            combined_instructions = _combine_developer_instructions(
                configured_instructions,
                self._developer_instructions,
            )
            # Native app-server protocol field (ThreadStartParams). Keep Fan's
            # mandatory boundary at developer priority instead of weakening it
            # into user_input. Codex's own base instructions and tool abilities
            # remain intact.
            params["developerInstructions"] = combined_instructions
            # Also place the same value in the request config layer. Native
            # subagents rebuild Config when applying a role; keeping the value
            # in that layer lets ordinary/default roles inherit the boundary
            # instead of retaining it only as an ephemeral thread override.
            params["config"] = {
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Fan' text path is the common case).
        try:
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            if method == "turn/completed":
                ...
                if turn_status == "interrupted":
                    ...
                    # the error into a re-auth hint and retire this session.
                    stderr_blob = "\n".join(self._client.stderr_tail(40))
        ...
        # Some app-server builds emit a completed assistant message but never
        # follow it with ``turn/completed``.  Preserve that usable final text
        # instead of turning a successful answer into a timeout/retry loop.
        if (
            not turn_complete
            ...
            logger.warning(
                ...
                "assistant message; accepting the assistant text"
            )
            turn_complete = True
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Fan' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own fan-tools callback we
            # auto-accept — the user already approved Fan' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Fan approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Fan returns 'once', 'session', 'always', or 'deny'.
...
def _get_fan_version() -> str:
    """Best-effort Fan version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("fan-agent")
