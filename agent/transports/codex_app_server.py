...
projection into Fan' display, approval bridging, transcript projection into
...
"codex_app_server"`. Fan' default tool dispatch is unchanged when this
...
def _redact_error_payload(value: Any) -> Any:
    """Preserve JSON-RPC error shape while removing secrets recursively."""
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # Codex may authenticate with a configured LLM provider, but has no
        # reason to inherit Fan's tool/skill, auxiliary-model, Bitwarden,
        # Electron-runtime, or server-control credentials.
        spawn_env = fan_subprocess_env(
        ...
        if spawn_env.get("FAN_KANBAN_TASK"):
            kanban_db = spawn_env.get("FAN_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "FAN_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("FAN_HOME", os.path.expanduser("~/.fan")),
                        "kanban",
                    ),
                )
            )
        ...
        if "error" in msg:
            ...
            raise CodexAppServerError(
                ...
                message=redact_sensitive_text(
                    str(err.get("message", "")), force=True
                ),
                data=_redact_error_payload(err.get("data")),
            )
    ...
    def _read_stdout(self) -> None:
        ...
        try:
            ...
            with self._stderr_lock:
                self._stderr_lines.append(redact_sensitive_text(
                    f"<stdout reader error> {exc}", force=True
                ))
...
def check_codex_binary(
    ...
    if proc.returncode != 0:
        return False, (
            ...
            f"{redact_sensitive_text(proc.stderr.strip(), force=True)}"
        )
    version = parse_codex_version(proc.stdout)
    if version is None:
        return False, (
            "could not parse codex version from: "
            f"{redact_sensitive_text(proc.stdout, force=True)!r}"
        )
