...
def _content_cache_key(
    ...
    """Derive a stable cache key from the static request prefix.

    Cron sessions intentionally carry a fresh timestamped session id for each
    fire. Using that id as the provider cache key makes an otherwise identical
    system prompt and tool schema cache-cold every time. The cache key is only
    a routing hint, so hashing those static inputs safely shares the warm
    prefix while session identity remains unchanged elsewhere.
    """
    ...
    if tools:
        sorted_tools = sorted(
            ...
            key=lambda tool: str(tool.get("name") or tool.get("type") or ""),
        )
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if "gpt-5.6" in (model or "").lower():
            # Ultra is a product tier; the Responses wire value tops out at max.
            _effort_clamp["ultra"] = "max"
        if is_xai_responses:
            # xAI Responses tops out at high, but users can keep one shared
            # stronger-effort setting across supported models.
        ...
        # Forward per-request timeout to the SDK so provider clients
