...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - fan_home (str): The active FAN_HOME directory path. Use this
            for Fan-home-scoped storage instead of hardcoding ``~/.fan``.
          - platform (str): "cli", "desktop", "api_server", "cron", etc.

        kwargs may also include:
          - agent_context (str): "primary", "subagent", "cron", or "flush".
            Providers should skip writes for non-primary contexts (cron system
            prompts would corrupt user representations).
          - agent_identity (str): Agent identity name. Use for
            per-agent provider identity scoping.
          - agent_workspace (str): Shared workspace name (e.g. "fan").
          - parent_session_id (str): For subagents, the parent's session_id.
          - user_id (str): Optional stable user identifier.
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        (desktop requests, cached agents). Providers that don't need
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        (CLI exit, /reset, or local session expiry).
        """
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'fan memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], fan_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'fan memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``fan_home`` is the active FAN_HOME directory path.
