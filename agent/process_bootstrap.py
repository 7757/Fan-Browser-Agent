...
class _SafeWriter:
    ...
    When fan-agent runs as a systemd service, Docker container, or headless
...
def build_keepalive_http_client(
    ...
    *,
    ...
    """Build an OpenAI-SDK HTTP client using only explicit proxy env vars.

    Supplying a transport deliberately bypasses httpx's macOS system-proxy
    discovery, whose exceptions list is not reliably honoured.  We retain
    HTTPS_PROXY/HTTP_PROXY/ALL_PROXY and NO_PROXY through the explicit helper
    above, and apply the same TCP keepalive policy as Fan's primary client.
    """
    try:
        import httpx
        import socket
        ...
        return client_cls(
            transport=transport_cls(socket_options=socket_options, verify=verify),
