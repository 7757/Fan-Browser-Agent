...
def _codex_note_to_tool_progress(note: dict) -> tuple[str, str, dict] | None:
    """Map Codex app-server item starts to Fan tool-progress events."""
    ...
    if item_type == "fileChange":
        ...
        preview = ", ".join(paths[:3]) or "file changes"
        if len(paths) > 3:
    ...
    if item_type == "mcpToolCall":
        ...
        if not isinstance(args, dict):
            args = {"arguments": args}
        return f"mcp.{server}.{tool}", str(tool), args
    if item_type == "dynamicToolCall":
        tool = item.get("tool") or "unknown"
        args = item.get("arguments") or {}
        if not isinstance(args, dict):
            args = {"arguments": args}
        return str(tool), str(tool), args
    return None
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Fan'
    ...
    from agent.prompt_builder import FAN_SECURITY_GUARDRAIL
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Fan' standard prompt flow if a
        ...
        # Gateway and cron turns have no interactive surface for Codex's
        # app-server approval requests.  Keep their default fail-closed
        # behaviour, but honor an explicit user approval bypass and leave
        # Codex's own sandbox profile as the remaining policy gate.
        ...
        def _on_codex_event(note: dict) -> None:
            ...
            try:
                callback("tool.started", tool_name, preview, args)
            except Exception:
    ...
    if turn.projected_messages:
        ...
        # This early-return runtime bypasses conversation_loop's normal
        # per-step persistence. The inbound user message is already marked,
        # so the intrinsic marker makes this flush idempotent while retaining
        # projected assistant/tool rows for session search and resume.
        if getattr(agent, "_session_db", None) is not None:
            try:
                agent._flush_messages_to_session_db(messages)
            except Exception:
                logger.debug("codex projected-message flush failed", exc_info=True)
...
def _item_field(item: Any, name: str, default: Any = None) -> Any:
    """Read a nested Responses item from either SDK or raw JSON shape."""
...
def _consume_codex_event_stream(
    ...
    * ``interrupt_check()`` — returns True to break the loop early.
    ...
    for event in event_iter:
        ...
        # Commentary/analysis message text is protocol progress, not an
        # assistant final answer. Track the current output item so its deltas
        # can use the reasoning channel instead of leaking into the chat.
