"""Shared utility functions for fan-agent."""
...
def pid_exists(pid: int) -> bool:
    ...
    POSIX uses ``os.kill(pid, 0)`` (EPERM still means alive). Windows opens a
    SYNCHRONIZE handle — ``os.kill(pid, 0)`` on Windows is NOT a probe (it
    calls TerminateProcess-adjacent semantics via GenerateConsoleCtrlEvent).
    """
    try:
        pid = int(pid)
...
def _preserve_file_owner(path: Path) -> "tuple[int, int] | None":
    """Capture a POSIX file's owning uid/gid before an atomic replace."""
    if os.name != "posix":
        return None
    try:
        st = path.stat()
...
def _restore_file_owner(path: Path, owner: "tuple[int, int] | None") -> None:
    """Best-effort restore of an existing file's owner after replacement."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.fan/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
class IndentDumper(yaml.SafeDumper):
    """Keep sequences indented beneath mapping keys across YAML writers."""
...
def safe_json_loads(text: str, default: Any = None) -> Any:
    ...
    pattern duplicated across display.py, auxiliary_client.py, and others.
    """
    try:
