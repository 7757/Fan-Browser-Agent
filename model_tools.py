...
# the first user message, freezing event processing for up to 120 seconds
# whenever any configured MCP server was slow or unreachable.
#
# Each entry point now runs discovery explicitly at its own startup:
#   - gateway/run.py            -> start_gateway() uses run_in_executor
#   - cli.py, fan_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    # user-visible config edits that affect dynamic schemas (for example
    # execute_code mode) without needing an explicit
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("FAN_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("FAN_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by FAN_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like fan-cli)
        ...
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                resolved = (
                    ...
                    if toolset_name.startswith("fan-")
                    else resolve_toolset(toolset_name)
                )
    ...
    # window). Core Fan tools (toolsets._FAN_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from fan_cli.config import load_config as _load
        cfg = _load() or {}
        model_cfg = cfg.get("model") if isinstance(cfg.get("model"), dict) else {}
        if not isinstance(model_cfg, dict):
            model_cfg = {}
        configured_context = model_cfg.get(
            "context_length", cfg.get("model_context_length")
        )
        try:
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in list(args.items()):
        ...
        if not isinstance(value, str):
            # Native containers may still contain JSON-encoded elements or
            # sub-fields. Normalize those recursively against their schema.
        ...
        if coerced is not value:
            args[key] = coerced
            # A top-level JSON string may itself decode to a container with
            # nested JSON-encoded values; normalize those too.
...
def _schema_accepts_kind(schema: Any, kind: str) -> bool:
    """Return whether a JSON Schema fragment permits ``kind``.

    Checks direct types plus the common union forms used by tool schemas.
    ``kind`` is either ``"array"`` or ``"object"``.
    """
    if not isinstance(schema, dict):
        return False
    schema_type = schema.get("type")
    if schema_type == kind or (isinstance(schema_type, list) and kind in schema_type):
        return True
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    """Recursively parse schema-expected object/array JSON strings.

    Models can emit a structured field, an array element, or a nested object
    property as an encoded JSON string. Parsing stays schema-guided so normal
    string fields that merely look like JSON are preserved verbatim.
    """
    if not isinstance(schema, dict):
        return value
    ...
    if isinstance(value, list):
        ...
        normalized = []
        for item in value:
            next_item = _normalize_json_strings_for_schema(item, items_schema)
    ...
    if isinstance(value, dict):
        ...
        for key, prop_schema in properties.items():
            if key not in value or not isinstance(prop_schema, dict):
                continue
            next_value = _normalize_json_strings_for_schema(value[key], prop_schema)
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from fan_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    Args:
        ...
        user_task: The user's original task (for browser tool context).
        ...
        result_mode: Internal consumer contract. ``"structured"`` prevents
                       model-facing output compaction for nested APIs such as
                       execute_code's ``fan_tools`` module.
    ...
    try:
        ...
        # (Removed: ACP/Zed editor edit-approval gate — the acp_adapter frontend was
        # pruned. It only applied to write_file/patch inside ACP sessions, neither of
        # which exists in the desktop browser agent. A browser-action approval gate, if
        # wanted, is a separate Phase-D feature.)
