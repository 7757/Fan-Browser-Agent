#!/usr/bin/env bash
# Canonical test runner for fan-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.fan/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.fan/fan-agent/venv"; do
...
if [ -f "$HOME/.fan/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.fan"
