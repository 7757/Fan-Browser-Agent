"""Windows UTF-8 bootstrap for Fan entry points.
...
should be imported at the very top of every Fan entry point
(``fan``, ``fan-agent``, ``fan-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import fan_bootstrap``
# (or ``from fan_bootstrap import apply_windows_utf8_bootstrap``) at
