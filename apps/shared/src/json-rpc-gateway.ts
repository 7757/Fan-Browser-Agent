...
export class JsonRpcGatewayError extends Error {
  ...
  constructor(payload: JsonRpcErrorPayload) {
    super(payload.message || 'Fan RPC failed')
...
// keeps the composer disabled and stuck on "Starting Fan..."). If the open
...
export class JsonRpcGatewayClient {
  ...
  async connect(wsUrl: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onClose = () => {
        // The general close handler runs first and clears this.socket. Allow
        // that null state for the current attempt, while still ignoring a
        // delayed close from an old socket after a replacement connected.
        if (settled || (this.socket !== null && this.socket !== socket)) {
          return
        }
  ...
  close(): void {
    ...
    try {
      ...
      // Local teardown is already complete; a socket close failure must not
      // leave the client open or keep pending calls alive.
    }
  }
