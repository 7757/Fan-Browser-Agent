...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'fan:saveImageFromUrl'")
}
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      if (window.fanDesktop?.saveImageFromUrl) {
        const saved = await window.fanDesktop.saveImageFromUrl(src)

        if (saved) {
          notify({ kind: 'success', title: '图片已保存', message: imageFilename(src) })
        }
      ...
      if (isMissingIpcHandler(error)) {
        try {
          ...
          notify({
            kind: 'info',
            title: '下载已开始',
            message: '重启 Fan Desktop 以使用"保存图片"功能。'
          })
        } catch (fallbackError) {
          notifyError(fallbackError, '请重启 Fan Desktop 以保存图片')
        }

        return
      }

      notifyError(error, '图片下载失败')
    } finally {
      setSaving(false)
    }
  }
