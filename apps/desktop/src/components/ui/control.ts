...
export const controlVariants = cva(
  // Literal radius (0.5rem = bwa-radius-sm) so the global --radius-scalar can't
  // collapse it. Chrome (bg/border/hover/focus glow) lives in
  // `.desktop-input-chrome`, restyled to the bwa form spec.
  'desktop-input-chrome w-full min-w-0 rounded-[0.5rem] border text-xs leading-4 text-foreground outline-none placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
  {
    variants: {
      size: {
