...
type CopyButtonAppearance = 'button' | 'context-menu-item' | 'icon' | 'inline' | 'menu-item' | 'tool-row'
...
export async function writeClipboardText(text: string) {
  ...
  if (window.fanDesktop?.writeClipboard) {
    await window.fanDesktop.writeClipboard(text)
...
export function CopyButton({
  ...
  const feedbackLabel = status === 'copied' ? '已复制' : status === 'error' ? errorMessage : (title ?? label)
  ...
  if (appearance === 'context-menu-item') {
    return (
      <ContextMenuItem
        className={className}
        disabled={disabled}
    ...
    return (
      <Tip label={feedbackLabel}>
        <button
          aria-label={ariaLabel}
          className={cn(
            'lg-icon-btn size-6 text-muted-foreground/70 opacity-0 hover:text-foreground focus-visible:opacity-100 group-hover/tool-row:opacity-100 disabled:opacity-40',
            className
          )}
          disabled={disabled}
          onClick={event => void copy(event)}
