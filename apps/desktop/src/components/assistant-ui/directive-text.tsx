...
const FAN_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type FanRefType = (typeof FAN_REF_TYPES)[number]
...
const ICON_PATHS: Record<FanRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as FanRefType] ?? ICON_FALLBACK
...
const FAN_DIRECTIVE_RE = new RegExp(
...
export const fanDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(FAN_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as FanRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: FanRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
  ...
  // `@session:<id>` — show a short id; the composer chip carries the friendly
  // title, but once sent the wire form only has the id.
  if (type === 'session') {
    const sid = id
...
/**
 * Renders text containing Fan directives (`@file:...`, `@image:...`) as
 * inline chips. Embedded MEDIA images render below as a thumbnail row.
 */
export function DirectiveContent({ text }: { text: string }) {
  const { cleanedText, images } = useMemo(() => extractEmbeddedImagesSafely(text ?? ''), [text])
  const segments = useMemo(() => parseDirectiveTextSafely(cleanedText), [cleanedText])
...
export function extractEmbeddedImagesSafely(
  text: string,
  extract: typeof extractEmbeddedImages = extractEmbeddedImages
): ReturnType<typeof extractEmbeddedImages> {
  try {
    return extract(text)
  } catch {
...
export function parseDirectiveTextSafely(
  text: string,
  parse: typeof fanDirectiveFormatter.parse = fanDirectiveFormatter.parse
