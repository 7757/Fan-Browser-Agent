...
export function splitFencesSafely(
  text: string,
  split: (value: string) => TopSegment[] = splitFences
): TopSegment[] {
  try {
    return split(text)
  } catch {
...
export function splitInlineCodeSafely(
  text: string,
  split: (value: string) => InlineNode[] = splitInlineCode
): InlineNode[] {
  try {
    return split(text)
  } catch {
...
export const UserMessageText: FC<UserMessageTextProps> = ({ className, text }) => {
  ...
  const top = useMemo(() => (oversized ? [] : splitFencesSafely(text)), [oversized, text])
...
const InlineSegmentView: FC<{ text: string }> = ({ text }) => {
  const nodes = useMemo(() => splitInlineCodeSafely(text), [text])
