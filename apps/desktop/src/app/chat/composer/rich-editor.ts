...
const REF_RE = /@(file|folder|url|image|tool|line|terminal|session):(`[^`\n]+`|"[^"\n]+"|'[^'\n]+'|\S+)/g
...
function quoteRefValue(value: string) {
  if (!value.includes('`')) {
    return `\`${value}\``
  }
...
function appendComposerContents(target: DocumentFragment | HTMLElement, text: string) {
  let cursor = 0
...
/** Backspace at a collapsed caret immediately after a chip: delete the chip
 * and the single auto-inserted trailing space as one atomic edit. */
...
function isBlankNode(node: ChildNode | null): boolean {
  ...
  if (node.nodeType === Node.ELEMENT_NODE) {
    ...
    return !element.dataset.refText && Array.from(element.childNodes).every(isBlankNode)
  }

  return false
}

/** Remove Chromium contenteditable wrappers/breaks that serialize as a
 * phantom newline after editing around a contenteditable=false ref chip. */
export function normalizeComposerEditorDom(editor: HTMLElement) {
  ...
  if (last?.nodeName === 'BR') {
    let previous: ChildNode | null = last.previousSibling

    while (previous?.nodeType === Node.TEXT_NODE && !(previous.textContent || '').trim()) {
      previous = previous.previousSibling
    }

    if (!previous || (previous as HTMLElement).dataset?.refText) {
