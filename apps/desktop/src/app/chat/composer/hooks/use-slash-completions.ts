...
import { type FanGateway, getFanConfig } from '@/fan'
import {
  type CommandsCatalogLike,
  desktopSlashDescription,
  filterDesktopCommandsCatalog,
  isDesktopSlashSuggestion,
  setDesktopSlashAllowlist
...
export function useSlashCompletions(options: { gateway: FanGateway | null }): {
  ...
  // Apply an optional local desktop.slash_commands override. Missing or
  // unreadable config keeps the built-in desktop commands enabled.
  useEffect(() => {
    if (!enabled) {
      return
    }
    ...
    void getFanConfig()
      .then(cfg => {
        ...
        const list = (cfg as { desktop?: { slash_commands?: unknown } }).desktop?.slash_commands
        setDesktopSlashAllowlist(Array.isArray(list) ? list : null)
      })
      .catch(() => setDesktopSlashAllowlist(null))
    return () => {
      cancelled = true
    }
  }, [enabled])
  ...
  const toItem = useCallback((entry: CompletionEntry, index: number): Unstable_TriggerItem => {
    ...
    const metadata: SlashItemMetadata = {
      ...
      display,
      meta,
      // Provide rawText so fanDirectiveFormatter.serialize uses the
