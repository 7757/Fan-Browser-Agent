...
// Slash commands only execute when they begin the message, so typing a path or
// prose such as "see /foo" never opens a command picker. `@` references remain
// valid when inserted by an explicit workflow, but typing `@` no longer opens
...
export function detectTrigger(textBefore: string): TriggerState | null {
  ...
  return { kind: '/', query: match[2], tokenLength: 1 + match[2].length }
}
