    ...
    <PreviewEmptyState
        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.fanDesktop?.openExternal(error.url)
            }}
          >
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Fan 正在重启...' : '让 Fan 重启服务器',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Fan to restart the server" looked like it did nothing —
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `Fan 正在寻找需要重启的预览服务器 (${taskId})`
      })

      notify({
        kind: 'info',
        title: '正在重启预览服务器',
        message: 'Fan 正在后台运行，请查看预览控制台了解进度。',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
        level: 2,
        message: `无法启动服务器重启：${error instanceof Error ? error.message : String(error)}`
      })
      notifyError(error, '服务器重启失败')
    }
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ...
            : `服务器重启失败：${previewServerRestart.message || '未知错误'}`
    })
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        kind: 'success',
        title: '预览服务器已重启',
        message: previewServerRestart.message?.slice(0, 160) || '正在重新加载预览。',
        durationMs: 3500
      })
      ...
      notify({
        kind: 'warning',
        title: '预览重启失败',
        message: userFacingErrorMessage(previewServerRestart.message, 'Fan 无法重启服务器，请重试。'),
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'Fan 仍在处理中，但尚未收到重启结果。服务器命令可能正在前台运行。'
      )
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.fanDesktop?.watchPreviewFile ||
      !window.fanDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const flushReload = () => {
      ...
      appendConsoleEntry({
        ...
        message:
          changedCount === 1
            ? `文件已变更，正在重新加载预览：${compactUrl(changedUrl)}`
            : `${changedCount} 个文件已变更，正在重新加载预览：${compactUrl(changedUrl)}`
      })

      reloadPreview()
    }

    const unsubscribe = window.fanDesktop.onPreviewFileChanged(payload => {
    ...
    void window.fanDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.fanDesktop?.stopPreviewFileWatch?.(watch.id)
      ...
      .catch(error => {
        appendConsoleEntry({
          level: 2,
          message: `无法监视预览文件：${error instanceof Error ? error.message : String(error)}`
        })
      })
    ...
    return () => {
      ...
      if (watchId) {
        void window.fanDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:fan-preview')
    ...
    const onConsole = (event: Event) => {
      ...
      if ((detail.level ?? 0) >= 3 && isModuleMimeError(message)) {
        setLoadError({
          description:
            'Module 脚本以错误的 MIME 类型提供。这通常意味着静态文件服务器正在代替项目开发服务器提供 Vite/React 应用。',
    ...
    const onFail = (event: Event) => {
      ...
      appendConsoleEntry({
        level: 3,
        message: `加载失败${errorCode ? ` (${errorCode})` : ''}：${
          detail.errorDescription || detail.validatedURL || '未知错误'
        }`
      })
      setLoadError({
        code: errorCode,
        description: detail.errorDescription || '预览页面无法访问。',
