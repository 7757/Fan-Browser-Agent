...
import { FAN_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
const TONE_STYLES: Record<EmptyStateTone, { cube: string; primary: string }> = {
  ...
  warning: {
    ...
    primary:
      'border-(--ui-yellow)/40 bg-(--ui-yellow)/10 text-(--ui-yellow) hover:bg-(--ui-yellow)/15'
  }
}
...
async function readTextPreview(filePath: string) {
  if (window.fanDesktop.readFileText) {
    try {
      return await window.fanDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'fan:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.fanDesktop.readFileDataUrl(filePath)
...
function MarkdownCode({ className, children, ...props }: ComponentProps<'code'>) {
  ...
  if (!language) {
    return (
      <code
        className={cn(
          'rounded bg-muted px-1 py-0.5 font-mono text-[0.86em] text-(--ui-purple)',
          className
        )}
        {...props}
      >
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(FAN_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
    ...
    <div className="grid min-w-max grid-cols-[auto_minmax(0,1fr)] font-mono text-xs leading-relaxed">
          ...
          return (
            <div
              className={cn(
                ...
                selected
                  ? 'bg-(--ui-yellow)/20 text-(--ui-yellow)'
      ...
      <div className="relative [&_pre]:m-0 [&_pre]:px-3 [&_pre]:py-3 [&_pre]:bg-transparent!">
        {selection && (
          <div
            aria-hidden
            className="pointer-events-none absolute inset-x-0 bg-(--ui-yellow)/15"
            style={{
...
export function LocalFilePreview({ reloadKey, target }: { reloadKey: number; target: PreviewTarget }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) {
          console.error('[preview] File preview failed', error)
          setState({
            error: userFacingErrorMessage(error, '文件预览加载失败，请重试。'),
            loading: false
          })
        }
      }
  ...
  if (state.loading) {
    return <PageLoader label="加载预览中" />
  }

  if (state.error) {
    return <PreviewEmptyState body={state.error} title="预览不可用" />
  }
  ...
  if (
    ...
    return (
      <PreviewEmptyState
        body={
          binary
            ? `预览 ${target.label} 可能显示乱码。`
            : `${target.label} 大小为 ${formatBytes(size)}，Fan 仅显示前 512 KB。`
        }
        primaryAction={{ label: '仍然预览', onClick: () => setForcePreview(true) }}
        title={binary ? '这看起来是一个二进制文件' : '此文件较大'}
        tone="warning"
      />
    )
  }
  ...
  return (
    <PreviewEmptyState
      body={`${target.mimeType || '此文件类型'}仍可作为上下文附加。`}
      title="无内联预览"
    />
  )
}
