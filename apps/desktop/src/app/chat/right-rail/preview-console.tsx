...
export function PreviewConsoleTitlebarIcon({ consoleState }: { consoleState: PreviewConsoleState }) {
  ...
  return (
    <>
      <PanelBottom />
      {logCount > 0 && <span className="sr-only">{logCount} 条控制台消息</span>}
    </>
  )
}
...
export function PreviewConsolePanel({
  ...
  function sendLogsToComposer(entries: ConsoleEntry[]) {
    ...
    notify({
      kind: 'success',
      title: '已发送到对话',
      message: `已向输入框添加 ${entries.length} 条日志`
    })
  }
    ...
    >
      ...
      <div className="flex h-8 shrink-0 items-center justify-between border-b border-border/50 px-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <CopyButton
            ...
            errorMessage="无法复制控制台输出"
            iconClassName="size-3"
            label={visibleSelection.length > 0 ? '复制所选到剪贴板' : '复制全部到剪贴板'}
      ...
      >
        {logs.length > 0 ? (
          ...
          <div className="py-2 text-muted-foreground/70">暂无控制台消息。</div>
        )}
      </div>
    </div>
  )
