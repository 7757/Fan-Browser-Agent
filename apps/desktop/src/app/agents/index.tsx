...
function statusGlyph(status: SubagentStatus): ReactNode {
  ...
  if (status === 'failed' || status === 'interrupted') {
    return <AlertCircle aria-label="失败" className="size-3.5 shrink-0 text-destructive" />
  }
...
export function AgentsView({ embedded = false, onClose }: AgentsViewProps) {
  ...
  const mainContent = (
    <>
      <header className="mb-3 shrink-0">
        <h2 className="text-sm font-semibold text-foreground">任务协作</h2>
        <p className="text-xs text-muted-foreground/80">复杂任务会拆给多个助手同时处理，进度实时显示在这里。</p>
      </header>
  ...
  if (embedded) {
    return <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">{mainContent}</div>
  }
...
function groupDelegations(roots: readonly SubagentNode[]): RootGroup[] {
  ...
  for (const node of roots) {
    ...
    if (node.taskCount > 1) {
      n += 1
      groups.push({ id: `delegation-${n}`, label: `协作 ${n}`, nodes: [node], taskCount: node.taskCount })
  ...
  if (tree.length === 0) {
    return (
      <div className="grid place-items-center gap-3 py-12 text-center">
        ...
        <p className="text-sm font-medium text-foreground/90">暂无正在协作的任务</p>
        <p className="max-w-md text-xs leading-relaxed text-muted-foreground/75">
          当 Fan 把一个复杂任务拆给多个助手同时处理时，这里会实时显示每一步的进度。
        </p>
      </div>
    )
  }
  ...
  return (
    <section className="grid min-w-0 gap-3">
      <p className="text-[0.66rem] font-medium uppercase tracking-wider text-muted-foreground/70">
        {group.label} <span className="text-muted-foreground/50">·</span> {group.nodes.length} 个工作者
        {activeWorkers > 0 ? <span className="text-primary/85"> · {activeWorkers} 活跃</span> : null}
      </p>
...
function SubagentRow({ node, depth = 0, nowMs }: { node: SubagentNode; depth?: number; nowMs: number }) {
  ...
  const subtitle = [
    ...
    node.toolCount ? `${node.toolCount} 个工具` : '',
    fmtTokens((node.inputTokens ?? 0) + (node.outputTokens ?? 0)),
    `更新于 ${fmtAge(node.updatedAt, nowMs)}`
  ].filter(Boolean)
    ...
    <div className={cn('grid min-w-0 max-w-full gap-2', depth > 0 && 'pl-4')} data-slot="tool-block" ref={enterRef}>
      ...
      {open && fileLines.length > 0 ? (
        <div className="grid min-w-0 gap-0.5 pl-6" data-selectable-text="true">
          <p className="text-[0.58rem] font-medium tracking-wider text-muted-foreground/60 uppercase">文件</p>
