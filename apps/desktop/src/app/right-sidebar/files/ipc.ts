...
import type { FanReadDirEntry, FanReadDirResult } from '@/global'
...
async function gitRootFor(start: string) {
  ...
  if (!cached) {
    cached = window.fanDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.fanDesktop?.readDir || !window.fanDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.fanDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.fanDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: FanReadDirEntry) {
...
async function filterIgnored(entries: FanReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<FanReadDirResult> {
  if (!window.fanDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.fanDesktop.readDir(dirPath)
