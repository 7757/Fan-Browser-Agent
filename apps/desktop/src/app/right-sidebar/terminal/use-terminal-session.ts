...
const FAN_PATHS_MIME = 'application/x-fan-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(FAN_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(FAN_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.fanDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      convertEol: true,
      cursorBlink: true,
      fontFamily: "'SF Mono', 'JetBrains Mono Variable', Menlo, Monaco, 'Cascadia Code', Consolas, monospace",
      fontSize: 11,
      lineHeight: 1.12,
    ...
    try {
      ...
      console.warn('[fan-terminal] WebGL unavailable; falling back to DOM', err)
    }
      ...
      .then(session => {
        ...
        cleanup.push(
          ...
          terminalApi.onExit(session.id, sessionExit => {
            ...
            setStatus('closed')
            term.write(`\r\n[终端已退出${signal ? `：${signal}` : code !== null ? `：${code}` : ''}]\r\n`)
          })
        )
