...
const RIGHT_SIDEBAR_TABS: readonly RightSidebarTab[] = [
  { id: 'files', label: '文件系统', icon: 'list-tree' },
...
export function RightSidebarPane({ onActivateFile, onActivateFolder, onChangeCwd }: RightSidebarPaneProps) {
  ...
  const chooseFolder = async () => {
    const selected = await window.fanDesktop?.selectPaths({
  ...
  return (
    <aside
      aria-label="右侧边栏"
      className="before:pointer-events-none relative flex h-full w-full min-w-0 flex-col overflow-hidden border-l border-(--ui-stroke-secondary) bg-(--ui-sidebar-surface-background) pt-(--titlebar-height) text-(--ui-text-tertiary)"
    >
  ...
  return (
    <header className="shrink-0 bg-transparent text-[0.75rem]">
      <div className="flex items-center gap-2 px-2.5 py-1">
        <nav aria-label="右侧边栏面板" className="flex min-w-0 items-center gap-1">
          {tabs.map(tab => (
  ...
  return (
    <div className="group/project-header flex min-h-0 flex-1 flex-col">
      <RightSidebarSectionHeader>
        <Tip label={hasCwd ? `${cwd} — 点击更改文件夹` : '打开文件夹'}>
          <button
...
function FileTreeBody({
  ...
  if (!cwd) {
    return <EmptyState body="请从状态栏设置工作目录以浏览文件。" title="无项目" />
  }

  if (error) {
    return <EmptyState body={`无法读取此文件夹（${error}）。`} title="无法读取" />
  }
  ...
  if (data.length === 0) {
    return <EmptyState body="此文件夹为空。" title="空文件夹" />
  }
    ...
    <ErrorBoundary
      fallback={({ reset }) => (
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-2 px-4 text-center">
          <EmptyState body="文件树渲染此文件夹时出错。" title="树错误" />
          <button
...
function FileTreeLoadingState() {
  return (
    <div aria-label="正在加载文件树" className="grid min-h-0 flex-1 place-items-center px-3" role="status">
      <Loader
        aria-hidden="true"
