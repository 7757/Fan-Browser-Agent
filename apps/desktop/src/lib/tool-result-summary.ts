...
const pluralize = (n: number, noun: string) => `${n} ${noun}`
...
function summarizeRecordInline(record: Json, depth: number): string {
  if (depth > 3) {
    return pluralize(Object.keys(record).length, '个字段')
  }
  ...
  return pairs.length ? pairs.join(' · ') : pluralize(Object.keys(record).length, '个字段')
}
...
function formatFieldValue(value: unknown, depth: number): string {
  ...
  if (Array.isArray(v)) {
    ...
    return first ? `${pluralize(v.length, '个条目')} (${first})` : pluralize(v.length, '个条目')
  }
