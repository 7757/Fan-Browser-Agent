...
import type { SessionMessage, UsageStats } from '@/types/fan'
...
export type GatewayEventPayload = {
  ...
  command?: string
  description?: string
  // false when the backend won't honor a permanent allow (tirith warning),
  // so the approval bar hides "Always allow".
  allow_permanent?: boolean
  // secret.request (skill credential capture)
  env_var?: string
  prompt?: string
  // verification.request (captcha) / control.request (user takeover) — _block-style
  captcha_type?: string
  challenge_id?: string
  document_revision?: number
  url?: string
  // control.request: 'tab' when a tab-strip op (not a page click) triggered the
  // takeover; anchor/user tab ids for the restore-on-继续 diagnostics.
  tabKind?: string
  anchorTabId?: string
  userTabId?: string
  settling?: boolean
...
// `message.complete` carries only the final no-tool response, while the live
// stream can already contain useful assistant commentary before one or more
// tool cards. Preserve every text segment before the last tool and replace
// only the trailing post-tool stream with the authoritative final response.
// A tool-free response keeps the original full-replacement behaviour.
export function finalizeAssistantTextParts(parts: ChatMessagePart[], text: string): ChatMessagePart[] {
  ...
  const replaceTextFrom = lastToolIndex >= 0 ? lastToolIndex + 1 : 0

  const kept = parts.filter((part, index) => {
    if (part.type === 'text') {
      return index < replaceTextFrom
    }
    ...
    const reasoningText = normalize(part.text)

    return !(reasoningText && (dedupeReference.startsWith(reasoningText) || reasoningText.startsWith(dedupeReference)))
  })
  ...
  return preservedText && preservedText === dedupeReference ? kept : [...kept, assistantTextPart(finalText)]
}
...
export function dismissAssistantMessageError(messages: ChatMessage[], messageId: string): ChatMessage[] {
  return messages.flatMap(message => {
    if (message.id !== messageId || message.role !== 'assistant' || !message.error) {
      return [message]
    }

    // An error raised before any visible output leaves only a renderer-local
    // placeholder. Remove it completely; otherwise keep partial text, tools,
    // reasoning, and attachments while clearing the stale failure state.
...
const INTERRUPTED_TRANSCRIPT_MARKER = '_[interrupted]_'
const LEGACY_INTERRUPTED_TEXT_RE = /^(?:_?\[interrupted\]_?|Operation interrupted\.)$/i
...
export type StreamDeltaKind = 'assistant' | 'reasoning'

export interface QueuedStreamSegment {
  kind: StreamDeltaKind
  text: string
}

// Append a streaming delta to an ordered segment buffer, coalescing
// consecutive same-kind deltas into one segment. The gateway sends reasoning
// (`reasoning.delta`) and visible text (`message.delta`) on two channels but in
// true arrival order; preserving that order here — instead of bucketing all
// reasoning vs all text and flushing text-before-reasoning — stops a content
// token that lands in the same flush window from being hoisted in front of the
// trailing reasoning, which used to split one continuous thought into two
// "Thinking" disclosures (and reorder it). Mutates `segments` in place.
export function pushStreamSegment(segments: QueuedStreamSegment[], kind: StreamDeltaKind, delta: string): void {
...
// Replay buffered stream segments onto the message parts in arrival order.
// Same-type adjacency merging in append*Part means a leading reasoning segment
// coalesces into an existing trailing reasoning part (e.g. across a flush
// boundary) rather than starting a new one.
export function applyStreamSegments(
  parts: ChatMessagePart[],
  segments: readonly QueuedStreamSegment[]
): ChatMessagePart[] {
  return segments.reduce<ChatMessagePart[]>(
    (acc, segment) =>
      segment.kind === 'assistant'
        ? appendAssistantTextPart(acc, segment.text)
        : appendReasoningPart(acc, segment.text),
    parts
  )
}
...
function toolPayloadMatchValues(payload: GatewayEventPayload | undefined): string[] {
  ...
  const query = firstStringField(payloadArgs, ['search_term', 'query', 'url', 'href'])
...
function toolPartMatchValues(part: ChatMessagePart): string[] {
  ...
  const query = firstStringField(args, ['search_term', 'query', 'url', 'href'])
...
function compactShadowToolParts(parts: ChatMessagePart[]): ChatMessagePart[] {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    const values = toolPartMatchValues(part)

    for (let candidateIndex = 0; candidateIndex < parts.length; candidateIndex += 1) {
      ...
      if (candidate.type !== 'tool-call' || candidate.result === undefined || candidate.toolName !== part.toolName) {
        continue
      }

      const sameStableId = Boolean(part.toolCallId && candidate.toolCallId && part.toolCallId === candidate.toolCallId)
      const sameTarget = values.length > 0 && hasToolMatchOverlap(values, toolPartMatchValues(candidate))
      const adjacentCompletion = !values.length && candidateIndex === index + 1

      if (sameStableId || sameTarget || adjacentCompletion) {
        shadowed.add(index)
  ...
  return shadowed.size ? parts.filter((_part, index) => !shadowed.has(index)) : parts
}
...
export function upsertToolPart(
  ...
  if (index === -1) {
    return compactShadowToolParts([...next, base])
  }

  next[index] = { ...next[index], ...base }

  return compactShadowToolParts(next)
}
...
const UNTRUSTED_TOOL_RESULT_OPEN_RE = /^\s*<untrusted_tool_result\b[^>]*>\s*/i
const UNTRUSTED_TOOL_RESULT_CLOSE_RE = /\s*<\/untrusted_tool_result>\s*$/i

function unwrapStoredToolResultText(content: string): string {
  if (!UNTRUSTED_TOOL_RESULT_OPEN_RE.test(content) || !UNTRUSTED_TOOL_RESULT_CLOSE_RE.test(content)) {
    return content
  }

  const body = content.replace(UNTRUSTED_TOOL_RESULT_OPEN_RE, '').replace(UNTRUSTED_TOOL_RESULT_CLOSE_RE, '')
  const payloadBoundary = body.indexOf('\n\n')

  // The first paragraph is Fan's fixed untrusted-data warning. The remainder
  // is the original tool result and is safe to parse as display data.
  return (payloadBoundary >= 0 ? body.slice(payloadBoundary + 2) : body).trim()
}
...
function parseStoredToolResult(content: unknown): unknown {
  ...
  const textContent = unwrapStoredToolResultText(textFromUnknown(content))
...
function storedToolResultIsError(result: unknown): boolean {
  ...
  const browserActionStatus = recordFromUnknown(record.browser_action_status)
  ...
  return Boolean(
    hasError ||
      ...
      browserActionStatus?.state === 'failed'
  )
}
...
function applyStoredToolResult(messages: ChatMessage[], toolMessage: SessionMessage): boolean {
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    parts[partIndex] = {
      ...existing,
      result,
      isError: storedToolResultIsError(result)
    } as ChatMessagePart
...
function toolResultRichness(part: Extract<ChatMessagePart, { type: 'tool-call' }>): number {
  if (part.result === undefined) {
    return 0
  }

  if (part.result && typeof part.result === 'object' && !Array.isArray(part.result)) {
    return 100 + Object.keys(part.result as Record<string, unknown>).length
  }

  return 1
}

function mergeSameToolCall(
  first: Extract<ChatMessagePart, { type: 'tool-call' }>,
  duplicate: Extract<ChatMessagePart, { type: 'tool-call' }>
): ChatMessagePart {
  const firstArgs = recordFromUnknown(first.args) ?? {}
  const duplicateArgs = recordFromUnknown(duplicate.args) ?? {}
  const args = { ...firstArgs, ...duplicateArgs }

  const resultSource =
    toolResultRichness(duplicate) > toolResultRichness(first) ? duplicate : first
  ...
  return {
    ...
    args: args as never,
    argsText: Object.keys(args).length ? JSON.stringify(args) : first.argsText || duplicate.argsText,
    ...(resultSource.result !== undefined && {
      result: resultSource.result,
      isError: resultSource.isError
    })
  } as ChatMessagePart
}
...
function withUniqueToolCallIds(messages: ChatMessage[]): ChatMessage[] {
  ...
  const seen = new Map<string, { messageIndex: number; partIndex: number }>()
  ...
  next.forEach((message, messageIndex) => {
    ...
    message.parts.forEach((part, partIndex) => {
      ...
      const sourceId = part.toolCallId || `${message.id}-tool-${partIndex}`
      const scopedId = `${turnIndex}\u0000${sourceId}`
      const previous = seen.get(scopedId)

      if (previous) {
        const canonical = next[previous.messageIndex]?.parts[previous.partIndex]

        if (canonical?.type === 'tool-call') {
          next[previous.messageIndex].parts[previous.partIndex] = mergeSameToolCall(canonical, part)
          duplicates.add(`${messageIndex}:${partIndex}`)
        }

        return
      }

      const id = globallyUsedIds.has(sourceId)
        ? `${sourceId}-${message.id}-${partIndex}`
        : sourceId

      const normalizedPart: Extract<ChatMessagePart, { type: 'tool-call' }> =
        part.toolCallId ? part : { ...part, toolCallId: id }

      if (normalizedPart.toolCallId !== id) {
        message.parts[partIndex] = { ...normalizedPart, toolCallId: id }
      } else {
        message.parts[partIndex] = normalizedPart
      }

      globallyUsedIds.add(id)
      seen.set(scopedId, { messageIndex, partIndex })
    })
  })
  ...
  return next
    .map((message, messageIndex) => ({
      ...message,
      parts: message.parts.filter((_part, partIndex) => !duplicates.has(`${messageIndex}:${partIndex}`))
    }))
...
export function toChatMessages(messages: SessionMessage[]): ChatMessage[] {
  ...
  messages.forEach((message, index) => {
    ...
    const exactInterruptedText =
      message.role === 'assistant' && LEGACY_INTERRUPTED_TEXT_RE.test(displayContent.trim())

    const interrupted =
      message.role === 'assistant' &&
      (message.finish_reason === 'interrupted' || exactInterruptedText)
  ...
  return withUniqueToolCallIds(
    result
      .map(message => ({ ...message, parts: compactShadowToolParts(message.parts) }))
      .filter(m => chatMessageText(m).trim() || m.parts.some(part => part.type !== 'text'))
  )
}
