...
function localPreviewTarget(rawTarget: string, cwd?: string | null): PreviewTarget | null {
...
export async function normalizeOrLocalPreviewTarget(
  ...
  try {
    const normalized = await window.fanDesktop?.normalizePreviewTarget?.(rawTarget, cwd || undefined)
