...
import { $activeBrowserWorkbenchId, $activeSessionId } from '@/store/session'
...
function cachedTitle(key: string): string | undefined {
  const value = titleCache.get(key)
...
function cacheTitle(key: string, value: string): void {
  titleCache.delete(key)
  titleCache.set(key, value)
...
// surrounding text is full of filename-shaped tokens (e.g. `agent.log` in a
// /debug report) that the bare-domain branch of URL_RE would mistake for
// domains and linkify.
const EXPLICIT_URL_RE = /(?:https?:\/\/|www\.)[^\s<>"'`]+[^\s<>"'`.,;:!?)]/gi

const DOMAIN_RE = /^(?:www\.)?[a-z0-9](?:[a-z0-9-]*\.)+[a-z]{2,}(?::\d+)?(?:[/?#][^\s]*)?$/i
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|fan):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  if (titleCache.has(key)) {
    return Promise.resolve(cachedTitle(key) ?? '')
  }
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.fanDesktop?.fetchLinkTitle
...
export function useLinkTitle(url?: null | string): string {
  ...
  const [title, setTitle] = useState(() => (key ? (cachedTitle(key) ?? '') : ''))

  useEffect(() => {
    setTitle(key ? (cachedTitle(key) ?? '') : '')
    ...
    // 防抖:助手回复流式渲染时,显式 URL 会逐 token 增长成一串中途前缀(https://yiyan.ba →
    // https://yiyan.baidu → https://yiyan.baidu.com),remark-gfm 对每个前缀都 autolink。若立即抓标题,
    // 就会对不存在的半截 host 发一串无谓的失败请求(ERR_CONNECTION_CLOSED 刷屏)。等 href 稳定 ~500ms 再抓:
    // 中途前缀在抖动窗口内被后续变化清掉,只有最终完整 URL 真正发请求。已缓存的标题仍会即时命中(上面 setTitle)。
...
export function openExternalLink(href: string): void {
  ...
  // We ARE a browser agent: open http(s) links as a new tab in the current
  // session's embedded browser instead of bouncing to the OS browser picker.
  // Fall back to the OS opener for non-http schemes, or when no session browser
  // is available / the tab open fails.
  const browser = window.fanDesktop?.browser
  const workbenchId = $activeBrowserWorkbenchId.get() ?? $activeSessionId.get()

  if (/^https?:\/\//i.test(href) && browser?.newTab && workbenchId) {
    void browser.newTab(workbenchId, href).then(
      result => {
        if (!result?.ok) {
          void window.fanDesktop?.openExternal?.(href)
        }
      },
      () => void window.fanDesktop?.openExternal?.(href)
    )
