...
export function appendInterruptedMarker(parts: ChatMessagePart[]): ChatMessagePart[] {
  const marker = INTERRUPTED_MARKER.trim()

  const settledParts = parts.map(part => {
    if (part.type !== 'tool-call' || part.toolName !== 'collect' || part.result !== undefined) {
      return part
    }

    // Stop is terminal for a pending collect interaction. Mark the tool part
    // before the collect request store is cleared so it cannot fall back to
    // the "正在准备表单…" state while the backend worker is unwinding.
    return {
      ...part,
  ...
  const last = settledParts.at(-1)

  if (last?.type === 'text' && last.text.trim() === marker) {
    return settledParts.every((part, index) => part === parts[index]) ? parts : settledParts
  }

  // Keep the marker in its own part. The renderer recognizes this exact part
  // and turns it into the localized quiet-status pill; merging it into the
  // preceding answer makes Markdown render a raw italic "[interrupted]".
  return [...settledParts, textPart(marker)]
}
...
export function createClientSessionState(
  ...
  return {
    storedSessionId,
    browserWorkbenchId: storedSessionId,
    messages,
    branch: '',
    cwd: '',
    busy: false,
 ...
 *
 * Only the leading status prefix (e.g. "Fan is thinking...") and the
