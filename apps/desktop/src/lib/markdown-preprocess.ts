...
import { splitUrlFromProse } from '@/lib/url-text'
...
// `*` excluded from both classes so a URL abutting markdown emphasis with no
// space (e.g. `**label: https://x**`, a common LLM pattern) doesn't swallow the
// trailing `**` into the href. `*` is never meaningful in a real URL path.
...
const INDEXED_BROWSER_ELEMENT_CODE_RE =
  /(\[\d+\])\s+`<(?:a|button|canvas|input|select|span|textarea)\b[^`\n]*(?:`|$)/gim

const INLINE_INDEXED_BROWSER_ELEMENT_CODE_RE =
  /`(\[\d+\])\s*<(?:a|button|canvas|input|select|span|textarea)\b[^`\n]*(?:`|$)/gim
...
function autoLinkRawUrls(text: string): string {
  return text.replace(RAW_URL_RE, (candidate: string, index: number) => {
...
export function stripInternalMessageMarkers(text: string): string {
  try {
    return text.replace(REASONING_BLOCK_RE, '').replace(PREVIEW_MARKER_RE, '')
  } catch {
    return text
  }
}
...
export function preprocessMarkdown(text: string): string {
  const cleaned = stripInternalMessageMarkers(text)
...
/**
 * Browser observations use compact element references such as
 * `[100]<input ... />`. Models occasionally repeat the full serialized
 * element inside inline code while reasoning, which creates a long code pill
 * that wraps beneath an otherwise short list item. The element metadata is
 * useful to the model but redundant in the visible reasoning UI: the numeric
 * reference already identifies the target.
 *
 * Run this after normal Markdown cleanup and only for the reasoning surface.
 * Complete and currently-streaming fenced code blocks are kept intact so a
 * genuine code example is never rewritten.
 */
function compactReasoningBrowserElements(text: string): string {
  ...
  return lines
    .map(line => {
      ...
      if (fenceMatch) {
        ...
        if (!activeFence) {
          activeFence = { length: marker.length, marker: marker[0] || '`' }
        } else if (!info && marker[0] === activeFence.marker && marker.length >= activeFence.length) {
          activeFence = null
        }
      ...
      return line
        .replace(INDEXED_BROWSER_ELEMENT_CODE_RE, '$1')
        .replace(INLINE_INDEXED_BROWSER_ELEMENT_CODE_RE, '$1')
    })
    .join('\n')
}

export function preprocessReasoningMarkdown(text: string): string {
  return compactReasoningBrowserElements(preprocessMarkdown(text))
}

export function preprocessMarkdownSafely(
  text: string,
  transform: (value: string) => string = preprocessMarkdown
): string {
  try {
    return transform(text)
  } catch {
    return stripInternalMessageMarkers(text)
  }
}

export function preprocessReasoningMarkdownSafely(text: string): string {
  return preprocessMarkdownSafely(text, preprocessReasoningMarkdown)
}
