...
import { badworkGlassTheme, BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY } from './presets'
import type { DesktopTheme, DesktopThemeColors } from './types'

const SKIN_KEY = 'fan-desktop-theme-v2'
const MODE_KEY = 'fan-desktop-mode-v1'
...
function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = BUILTIN_THEMES[skinName] ?? badworkGlassTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...badworkGlassTheme.typography, ...theme.typography }
  ...
  root.dataset.fanTheme = skinName
  root.dataset.fanMode = rendered
  ...
  window.fanDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.fanThemeFont = 'true'
