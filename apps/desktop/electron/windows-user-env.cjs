...
// even though a fresh shell — and the Fan CLI — sees it immediately. The
// desktop's FAN_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\fan. Reading
