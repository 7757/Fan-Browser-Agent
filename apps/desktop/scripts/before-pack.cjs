 ...
 * product name (`Fan`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Fan'
 *
 * This is a hard failure with no obvious cause for the user — `fan desktop`
 ...
 * (the mac-unpacked Fan.app bundle) and Windows (win-unpacked), so we
...
exports.default = async function beforePack(context) {
  ...
  const bundledPython = path.join(desktopRoot, 'build', 'python')
  const bundledSource = path.join(desktopRoot, 'build', 'fan-src')
  if (!fs.existsSync(bundledPython) || !fs.existsSync(bundledSource)) {
    throw new Error(
      'Bundled Fan backend is missing. Run `npm run build:bundled-backend` before packaging a desktop installer.'
    )
  }
