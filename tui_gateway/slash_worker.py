"""Persistent slash-command worker — one FanSession per TUI session.
...
_WATCHDOG_POLL_S = float(os.environ.get("FAN_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = float(os.environ.get("FAN_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Populate this child process's MCP registry before its CLI snapshots tools.

    Slash workers are separate processes, so a discovery thread in the desktop
    backend cannot populate their in-memory registry.
    """
    import logging
...
def _run(cli: FanSession, command: str) -> str:
...
def main():
    ...
    os.environ["FAN_SESSION_KEY"] = args.session_key
    os.environ["FAN_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) FanSession build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = FanSession(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
