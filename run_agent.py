...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``fan -c`` / ``--resume`` (relaunch
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    Session IDs arrive from the desktop RPC surface and must never be
    interpolated directly into a path.  Preserve ordinary IDs unchanged; for
    anything containing separators or other unsafe characters, append a short
    hash so distinct IDs cannot collapse onto one session artifact.
    """
...
_loaded_env_paths = load_fan_dotenv(fan_home=_fan_home, project_env=_project_env)
...
def cleanup_browser(*args, **kwargs):  # noqa: D401
    """No-op: the legacy browser stack (tools/browser_tool.py) was removed; the
    fused agent drives the Electron-native browser runtime, which is owned by
    the desktop main process. Kept as a run_agent attribute for callers."""
    return None
...
# Intrinsic marker stamped on a message only after its SQLite append commits.
# A retained ``id(msg)`` set is unsafe across turns because CPython may recycle
# an address after the old dict is collected, causing an unrelated new message
# to be mistaken for one that is already durable.  The marker binds the target
# session plus a digest of the sanitized row, so compression children and later
# in-memory edits remain eligible for persistence.  Every model-wire adapter
# strips top-level underscore-prefixed fields before dispatch.
...
def _routermint_headers() -> dict:
    ...
    from fan_cli import __version__ as _FAN_VERSION

    return {
        "User-Agent": f"FanAgent/{_FAN_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[fan-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        if self._session_db_created or not self._session_db:
            return
        source = self.platform or os.environ.get("FAN_SESSION_SOURCE", "cli")
        try:
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("FAN_SESSION_SOURCE", "cli"),
    ...
    def reset_session_state(
        ...
        # Some reset-only paths change ``session_id`` without the richer
        # transition context. Rebind the built-in compressor so an old
        # session's persisted retry cooldown cannot leak into the new one.
        engine = getattr(self, "context_compressor", None)
        target_session_id = getattr(self, "session_id", "") or ""
        bound_session_id = getattr(engine, "_session_id", "") if engine else ""
        if (
            engine is not None
            and target_session_id
            ...
            try:
                ...
                logger.debug("context compressor session rebind failed: %s", exc)

    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """No-op runtime hook retained for the unconditional model-switch path.

        The LM Studio runtime-preload feature was removed along with its model
        catalog; the active providers (alibaba family, custom, openai-codex)
        never required it.  Kept callers in the switch_model/fallback paths
        invoke this for every provider, so the method must continue to exist
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``fan chat -q``. In that mode,
    ...
    def _buffer_fallback_status(self, message: str) -> None:
        """Buffer the durable notice that the serving model changed."""
        try:
            buf = getattr(self, "_retry_status_buffer", None)
            if buf is None:
                buf = []
                self._retry_status_buffer = buf
            buf.append(("fallback", message))
        except Exception:
            pass

    def _clear_status_buffer(self, *, preserve_fallback_notice: bool = False) -> None:
        """Drop transient retry messages, optionally emitting a model-switch notice."""
        try:
            buf = getattr(self, "_retry_status_buffer", None)
            if buf:
                fallback_notices = (
                    [message for kind, message in buf if kind == "fallback"]
                    if preserve_fallback_notice
                    else []
                )
                buf.clear()
                for message in fallback_notices:
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``FAN_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``FAN_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("FAN_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``FAN_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("FAN_API_CALL_STALE_TIMEOUT")
        if env_timeout is not None:
            return float(env_timeout), False

        # Known reasoning models may legitimately take minutes before a first
        # visible token.  Treat their floor as an explicit effective policy so
        # the local-endpoint implicit-default shortcut below cannot turn a
        # broken local/NIM reasoning request into an unbounded hang.
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        fan-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See fan-agent#21444 for symptom history."
        )
    ...
    def _prompt_cache_policy(
        ...
        *,
        ...
        """Forwarder — see ``agent.agent_runtime_helpers.prompt_cache_policy``."""
        from agent.agent_runtime_helpers import prompt_cache_policy
        return prompt_cache_policy(self, provider=provider, base_url=base_url, api_mode=api_mode, model=model)
    ...
    def _is_ollama_glm_backend(self) -> bool:
        """Detect the narrow Ollama/GLM stop-misreporting backend family.

        Other local OpenAI-compatible servers report finish reasons correctly;
        treating every local endpoint as Ollama causes false continuation
        requests on sglang, vLLM, LM Studio, and private proxies.
        """
    ...
    def _apply_persist_user_message_override(self, messages: List[Dict]) -> None:
        ...
        history stay clean. Structured multimodal content is left intact because
        early persistence runs before the same list is dispatched to the model.
        """
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        Successful writes are recorded on the message dict itself with a
        session-scoped private marker.  Repeated exit-path flushes therefore
        remain idempotent without retaining ``id(msg)`` values across turns,
        where CPython address reuse could silently suppress a real message.

        A marker is added only after ``append_message`` returns.  If a batch
        fails part-way through, the durable prefix remains marked while the
        failed message and its tail remain unmarked and are retried on the next
        persistence point.  ``_last_flushed_db_idx`` remains for legacy cursor
        repair callers, but is never treated as evidence that a row committed.
        """
        if getattr(self, "_persist_disabled", False):
            return
        if not self._session_db:
            return
        override_idx = getattr(self, "_persist_user_message_idx", None)
        override_content = getattr(self, "_persist_user_message_override", None)
        try:
            ...
            #
            # A legacy caller may still leave stale ids in this attribute.
            # Clear them before inspecting any message so no id can outlive the
            # object whose write it represented.  Intrinsic markers below are
            # the sole durable deduplication state.
            self._flushed_db_message_ids = set()

            current_session_id = str(getattr(self, "session_id", None) or "")
            ...
            for msg_idx, msg in enumerate(messages):
                ...
                # The API-facing user message can carry synthetic context that
                # must not be retained in the durable transcript. Apply the
                # clean persistence override only to the row being written;
                # mutating the shared live dict before the provider call can
                # silently remove that context from the model request.
                if (
                    msg_idx == override_idx
                    and role == "user"
                    and override_content is not None
                    and not isinstance(row_content, list)
                ):
                    row_content = override_content
                # Strip image bytes before fingerprinting or persistence. The
                # original in-memory content remains untouched for model use.
                content = sanitize_db_message_content(row_content)
                ...
                if db_persisted_marker_matches(msg, current_session_id, row):
                    continue
                self._session_db.append_message(
                    session_id=self.session_id,
                    **row,
                )
                # Stamp only after append_message has committed.  On failure,
                # this message and every later one remain retryable.
                msg[_DB_PERSISTED_MARKER] = make_db_persisted_marker(
                    current_session_id, row
                )
            self._last_flushed_db_idx = len(messages)
            previous_write_error = getattr(
                self, "_session_db_last_write_error", None
            )
            self._session_db_last_write_error = None
            if previous_write_error:
                try:
                    self._emit_status(
                        "Session history saving recovered; pending messages "
                        "were persisted."
                    )
                except Exception:
                    pass
        except Exception as e:
            # Keep failure observable to embedding frontends as well as logs;
            # the unmarked tail is intentionally preserved for a later retry.
            previous_write_error = getattr(
                self, "_session_db_last_write_error", None
            )
            self._session_db_last_write_error = str(e)
            self._flushed_db_message_ids = set()
            logger.warning("Session DB append_message failed: %s", e)
            if previous_write_error != self._session_db_last_write_error:
                try:
                    self._emit_warning(
                        "Session history could not be saved; Fan will retry "
                        f"automatically. Error: {e}"
                    )
                except Exception:
                    pass
    ...
    def _is_entitlement_failure(
        ...
        Extend here for new providers only when an entitlement error is
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Fan against xai-oauth.  Lots of
    ...
    def _coerce_api_error_detail(value: Any) -> str:
        """Normalize structured provider error fields into display-safe text."""
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        # Structured JSON errors from provider SDKs
        ...
        # Some OpenAI-compatible SDK errors expose an empty parsed ``body``
        # while the underlying response still contains the provider's actual
        # diagnostic.  Surface it safely instead of leaving the user with a
        # useless bare HTTP status.
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("FAN_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("fan_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from fan_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Handles both plain-string content and structured multimodal
        ...
        Respects ``FAN_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.fan/sessions/session_{sid}.json``
        ...
        try:
            safe_session_id = _safe_session_filename_component(self.session_id)
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects FAN_REDACT_SECRETS via
    ...
    def clear_interrupt(self) -> None:
        ...
        # Only an actual hard interrupt supersedes pending steer guidance.
        # Normal turn completion also calls clear_interrupt(); clearing steer
        # there creates a race where guidance accepted during final-response
        # teardown disappears before the gateway can hand it off.
        hard_interrupt_was_pending = bool(self._interrupt_requested)
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``FAN_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        as a method so tests can patch a single seam without reaching into
        the private ``_turn_failed_file_mutations`` state dict.
        """
        try:
            import os as _os
            env = _os.environ.get("FAN_FILE_MUTATION_VERIFIER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so desktop and CLI share
    ...
    # Wrap them in inline-code backticks so clients render paths as text and
    # never mistake protected credential paths for attachments.
    ...
    def _neutralize_footer_paths(cls, text: str) -> str:
        """Wrap bare file paths in backticks for safe, literal display."""
        if not text:
            return text
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        Every path in the user-facing text is backtick-wrapped via
        ``_neutralize_footer_paths`` so protected files such as
        ``~/.fan/config.yaml`` are always rendered literally.
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``FAN_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("FAN_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (FAN_KANBAN_TASK set),
    ...
    def commit_memory_session(self, messages: list = None) -> None:
        ...
        if self._memory_manager:
            try:
                ...
                logger.warning(
                    "Memory provider on_session_end failed during session commit: %s",
                    exc,
                    exc_info=True,
                )
    ...
    def close(self) -> None:
        ...
        # 7. Clear session-scoped human-input state owned by this root agent.
        # Shared temporary forks opt out of session ownership and must not
        # invalidate refs or pending approvals still used by their parent.
        if getattr(self, "_end_session_on_close", True) and task_id:
            try:
                from tools.approval import clear_session

                clear_session(task_id)
            except Exception:
                pass
            try:
                from agent.human_interaction_state import clear_human_interaction_state

                clear_human_interaction_state(task_id)
            except Exception:
                pass

        # 8. Finalize the owned SQLite row. end_session() is idempotent and
        # first-reason-wins, so an earlier compression/cron/gateway reason is
        # preserved. Shared temporary forks explicitly opt out.
        try:
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        Hydration only accepts results paired with an earlier assistant
        ``todo`` tool call. Conversation history may be caller-supplied, so a
        bare ``role: tool`` payload must not be able to seed todo state.
        """
    ...
    def _tool_response_matches_todo_call(
        ...
        """Return whether a tool result belongs to a prior assistant todo call."""
    ...
    def _assistant_has_todo_tool_call(
        ...
        """Return whether an assistant message issued the matching todo call."""
    ...
    def _get_tool_call_name_static(tc) -> str:
        ...
        message to carry the matching function name. Other providers
    ...
    def _is_thinking_only_assistant(
        ...
        *,
        ...
        ``reasoning_content``) but no visible text and no tool calls. Some
        gateways reject that shape when it is replayed. We drop the whole turn
        from the API copy rather than fabricating stub text; the stored transcript
        keeps the reasoning block.
        """
        if not isinstance(msg, dict) or msg.get("role") != "assistant":
            return False
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        """Build an HTTP pool that reaps idle connections before proxy FINs.

        Socket-level keepalive options conflict with some reverse proxies'
        streaming behavior.  Let httpx manage the pool lifetime instead while
        retaining explicit proxy/NO_PROXY routing.
        """
        try:
            ...
            # A mounted transport owns scheme behavior.  On a direct route,
            # prevent ambient proxy variables from undoing the NO_PROXY result.
            mounts = None
            if _proxy is None:
                mounts = {
    ...
    def _create_request_openai_client(self, *, reason: str, api_kwargs: Optional[dict] = None) -> Any:
        ...
        # Shared/primary clients and Bedrock are unaffected.
    ...
    def _apply_user_default_headers(self) -> None:
        ...
        No-op for Bedrock, which does not use the OpenAI client, and when no
    ...
    # A data URL is decoded into memory before vision routing.  Cap the base64
    # payload so an oversized pasted image cannot exhaust the shared desktop
    # agent process (20 MiB encoded is roughly 15 MiB decoded).
    ...
    def _materialize_data_url_for_vision(image_url: str) -> tuple[str, Optional[Path]]:
        ...
        tmp = tempfile.NamedTemporaryFile(prefix="fan_image_", suffix=suffix, delete=False)
        try:
            with tmp:
    ...
    def _describe_image_for_text_fallback(self, image_url: str, role: str) -> str:
        ...
        cached = self._image_fallback_cache.get(cache_key)
        if cached:
            return cached
        ...
        self._remember_image_note(cache_key, note)
        return note

    def _describe_browser_screenshot(self, image_url: str, element_list: str) -> str:
        """Qwen-vision GROUNDING for a TEXT brain on a browser observe.

        An explicitly annotated browser_snapshot screenshot, with the same
        numbered boxes as the [index] element list, is sent to the fixed Qwen
        vision model so the brain can locate icon-only controls that carry no
        text. Returns a note to append to the observation, or '' when vision
        fails (the caller then keeps the DOM-only summary). Cached per image.
        """
        cache_key = "browser:" + hashlib.sha256(str(image_url or "").encode("utf-8")).hexdigest()
        cached = self._image_fallback_cache.get(cache_key)
        if cached is not None:
            return cached

        elements = (element_list or "").strip()
        if len(elements) > 4000:
            elements = elements[:4000] + "\n…(truncated)"
        grounding_prompt = (
            "You are the visual-grounding step for a browser automation agent. The "
            "screenshot is the CURRENT page; each interactive element has a numbered "
            "box drawn on it. The agent's element list uses the SAME numbers:\n\n"
            f"{elements}\n\n"
            "For each numbered interactive element, output ONE short line "
            "`[N] = <what it is / its action>`. Be specific for ICON-ONLY controls "
            "with no visible text (send/submit arrow, search, attach, microphone, "
            "menu, close, like, etc.) and disambiguate by box position. List ONLY "
            "actionable controls (buttons, links, inputs, toggles); do NOT describe "
            "decorative images or body text. "
            "Write every description and do all of your reasoning in English; keep the "
            "[N] index numbers and any literal on-page text or labels "
            "verbatim (do not translate button labels)."
        )
        ...
        try:
            ...
            result_json = asyncio.run(
                vision_analyze_tool(image_url=vision_source, user_prompt=grounding_prompt)
            )
            result = json.loads(result_json) if isinstance(result_json, str) else {}
            mapping = (result.get("analysis") or "").strip()
        except Exception as exc:
            logger.warning("Browser visual grounding failed: %s", exc)
            mapping = ""
        finally:
            if cleanup_path and cleanup_path.exists():
                try:
        ...
        if mapping:
            logger.info(
                "[grounding] eb screenshot grounded: %d chars; preview=%s",
                len(mapping), mapping[:240].replace("\n", " "),
            )
        else:
            logger.warning("[grounding] eb screenshot grounding empty/failed")

        # Full prompt + response → a dedicated UTF-8 JSONL file for inspection
        # (agent.log only keeps a short preview). One line per grounding call;
        # safe to delete anytime.
        try:
            import datetime as _dt
            from fan_cli.config import get_fan_home

            _dbg = get_fan_home() / "logs" / "grounding_debug.jsonl"
            with open(_dbg, "a", encoding="utf-8") as _f:
                _f.write(json.dumps({
                    "ts": _dt.datetime.now().isoformat(timespec="seconds"),
                    "brain": getattr(self, "model", ""),
                    "elements_chars": len(elements),
        ...
        note = (
            "[Visual grounding of the current page — the numbered boxes match the "
            f"[index] elements above:\n{mapping}]"
            if mapping
            else ""
        )
        self._remember_image_note(cache_key, note)
    ...
    def _preprocess_image_content_for_text_model(self, content: Any, role: str) -> Any:
    ...
    def _prepare_messages_for_non_vision_model(self, api_messages: list) -> list:
        ...
        if self._model_supports_vision():
            # Vision model: keep image parts, but drop STALE browser screenshots
            # so the re-sent history doesn't carry a screenshot from every step.
            # (After the observation collapser runs in build_api_kwargs this is a
            # no-op — at most the latest observation still carries an image.)
            return self._prune_stale_browser_screenshots(api_messages)

        transformed = copy.deepcopy(api_messages)
        for msg in transformed:
            if not isinstance(msg, dict):
                continue
            msg["content"] = self._preprocess_image_content_for_text_model(
                msg.get("content"),
                str(msg.get("role", "user") or "user"),
            )
        return transformed

    def _prune_stale_browser_screenshots(self, api_messages: list) -> list:
        """Keep only the LATEST tool-message screenshot for a vision model.

        Browser observes attach a screenshot every step and the chat history is
        re-sent in full each turn, so without pruning the payload (and cost) grow
        linearly as every prior screenshot is re-sent. Keep the image in the most
        recent tool screenshot; replace earlier ones with their text part (the DOM
        snapshot). Operates on a copy — the canonical history keeps the images
        until normal context compression trims them.
        """
        shot_idxs = [
            i
            ...
            and m.get("role") == "tool"
            and self._content_has_image_parts(m.get("content"))
        ]
        if len(shot_idxs) <= 1:
            return api_messages

        transformed = copy.deepcopy(api_messages)
        for i in shot_idxs[:-1]:
            msg = transformed[i]
            content = msg.get("content")
            if not isinstance(content, list):
                continue
            texts = [
                ...
                and str(p.get("text", "")).strip()
            ]
            msg["content"] = "\n".join(texts) if texts else "[earlier page screenshot omitted]"
        logger.info(
            "[grounding] vision brain: kept latest screenshot, pruned %d stale",
            len(shot_idxs) - 1,
        )
        return transformed

    def _remember_image_note(self, cache_key: str, note: str) -> None:
        """Store an image-description note, bounding the cache (FIFO) so a long
        browsing session — which adds a grounding entry per observe — cannot grow
        it without limit."""
        cache = self._image_fallback_cache
        cache[cache_key] = note
        cap = 256
        if len(cache) > cap:
            for old in list(cache.keys())[: len(cache) - cap]:
                cache.pop(old, None)
    ...
    def _tool_result_content_for_active_model(self, tool_name: str, result: Any) -> Any:
        ...
        # Non-vision (text) brain + image-bearing tool result. Explicitly numbered
        # browser screenshots use fixed-model visual grounding so the brain can map
        # icon-only controls back to [index]; ordinary browser screenshots stay
        # DOM-only. Other image tools get a plain description. Cached per image; a
        # vision failure degrades to the text summary rather than breaking.
        is_browser = isinstance(tool_name, str) and tool_name.startswith("browser_")
        if is_browser:
            # Ordinary browser snapshots stay DOM-only for a text brain. An
            # explicitly requested numbered screenshot is the opt-in visual
            # grounding boundary: map its boxes back to the same [index]
            # records without restoring the old every-observation vision call.
            screenshot_meta = (
                result.get("screenshot")
                if isinstance(result.get("screenshot"), dict)
                else {}
            )
            if screenshot_meta.get("indexAnnotations") is True:
                element_list = summary
                try:
                    parsed_summary = json.loads(summary)
                    if isinstance(parsed_summary, dict) and isinstance(
                        parsed_summary.get("snapshot"), str
                    ):
                        element_list = parsed_summary["snapshot"]
                except (TypeError, ValueError):
                    pass
                for part in content:
                    if (
                    ...
                    if not url:
                        continue
                    try:
                        note = self._describe_browser_screenshot(
                            url,
                            element_list,
                        )
                    except Exception as exc:
                        logger.warning(
                            "Tool %s: numbered browser grounding failed: %s",
                            tool_name,
                            exc,
                        )
                        continue
        ...
        for part in content:
            ...
            if not url:
                continue
            try:
                note = self._describe_image_for_text_fallback(url, "tool")
            except Exception as exc:
                logger.warning("Tool %s: Qwen vision describe failed: %s", tool_name, exc)
                continue
            if note:
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        try:  # 全量 LLM 输入日志(FAN_LLM_IO_LOG=1 时;否则零开销)
            from agent.llm_io_log import log_request
            log_request("main", model=kwargs.get("model", self.model),
                        messages=kwargs.get("messages", api_messages), tools=kwargs.get("tools"))
        except Exception:
            pass
        return kwargs
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        msg = build_assistant_message(self, assistant_message, finish_reason)
        try:  # 全量 LLM 输出日志(按内容去重,避开流式 interim 重复)
            from agent.llm_io_log import log_response
            log_response("main", content=msg.get("content"), tool_calls=msg.get("tool_calls"),
    ...
    def _toolguard_controlled_halt_response(self, decision: ToolGuardrailDecision) -> str:
        ...
        if decision.code == "browser_no_progress_budget_exhausted":
            return (
                "Browser actions in this turn did not produce enough verifiable change, "
                "so automatic retries stopped after the six-attempt recovery budget was "
                "exhausted. The current step is incomplete. Retry with a different page "
                "path, element, or tab, and ask the user to take over if necessary."
            )
        if decision.code.startswith("browser_no_progress"):
            return (
                "Consecutive browser actions produced no verifiable change, so automatic "
                "retries stopped to prevent a loop. The current step is incomplete. "
                "Retry with a different page path, element, or tab, and ask the user to "
    ...
    def _append_guardrail_observation(
        ...
        *,
        ...
        decision = self._tool_guardrails.after_call(
            ...
            function_result,
            failed=failed,
            browser_state_before=getattr(self, "_browser_last_tool_state_before", None),
            browser_state_after=getattr(self, "_browser_last_tool_state_after", None),
        )
        # The executor consumes this exact post-call decision before dispatching
        # another browser call from the same assistant batch.  Do not make that
        # control boundary depend on parsing human-readable warning text.
        self._last_tool_guardrail_after_call_decision = decision
    ...
    def run_conversation(
        ...
        token_baseline = {
            key: int(getattr(self, attr, 0) or 0)
            for key, attr in metric_fields.items()
        }
        cost_baseline = float(getattr(self, "session_estimated_cost_usd", 0) or 0)
        turn_started = time.monotonic()
        result = None
        turn_error = None
        try:  # 全量 LLM I/O 日志:标记一轮对话边界(所有调用方都走此转发,覆盖全)
            from agent import llm_io_log
            llm_io_log.log_turn_start(user_message, {
                "task_id": task_id,
                "session_id": getattr(self, "session_id", None),
        ...
        try:
            ...
            try:
                from agent import llm_io_log
                token_delta = {
                    key: max(0, int(getattr(self, attr, 0) or 0) - token_baseline[key])
                    for key, attr in metric_fields.items()
                }
                if turn_error is not None:
                    ...
                    outcome = "incomplete"
                cost_status = (
                    result.get("cost_status")
                    if isinstance(result, dict)
                    else getattr(self, "session_cost_status", "unknown")
                ) or "unknown"
                cost_source = (
                    result.get("cost_source")
                    if isinstance(result, dict)
                    else getattr(self, "session_cost_source", "none")
                ) or "none"
                prompt_tokens = token_delta["prompt_tokens"]
                estimated_cost = None
                if cost_status in {"actual", "estimated", "included"}:
                    estimated_cost = max(
                        0.0,
                        float(getattr(self, "session_estimated_cost_usd", 0) or 0)
                        - cost_baseline,
                    )
                summary = {
                    "duration_ms": int((time.monotonic() - turn_started) * 1000),
                    "outcome": outcome,
                    "api_calls": int(result.get("api_calls", 0) or 0) if isinstance(result, dict) else 0,
                    **token_delta,
                    "cache_hit_ratio": (
                        round(token_delta["cache_read_tokens"] / prompt_tokens, 4)
                        if prompt_tokens > 0
                        else 0.0
                    ),
                    # Unknown pricing is not zero cost.  ``None`` keeps logs and
                    # downstream dashboards from silently making that claim.
