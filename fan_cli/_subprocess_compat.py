...
Fan is developed on Linux / macOS and tested natively on Windows too.
...
def find_node_executable(name: str) -> str | None:
    """Resolve a Node launcher, preferring executable Windows shims.

    ``shutil.which("npm")`` may select an extensionless or PowerShell shim on
    Windows.  Those cannot be launched directly by ``CreateProcessW``. Search
    PATH in the order used by Fan-owned subprocesses and prefer ``.cmd``.
    """
    if not IS_WINDOWS:
    ...
    candidates = [name] if suffix else [f"{name}.cmd", f"{name}.exe", f"{name}.bat", name]
...
def resolve_node_command(name: str, argv: Sequence[str]) -> list[str]:
    ...
    """
    resolved = find_node_executable(name)
    if resolved:
