"""
Skills configuration for Fan Agent.
`fan skills` enters this module.

Toggle individual skills or categories on/off, globally or per local runtime.
Config stored in ~/.fan/config.yaml under:

  skills:
    disabled: [skill-a, skill-b]          # global disabled list
    platform_disabled:                    # per-runtime overrides
      cron: [skill-c]
      cli: []
"""
from typing import List, Optional, Set

from fan_cli.config import cfg_get, load_config, save_config
from fan_cli.colors import Colors, color
from fan_cli.platforms import PLATFORMS as _PLATFORMS
...
def _normalize_skill_names(values) -> Set[str]:
    """Normalize YAML input into skill names without treating a string as chars."""
    if values is None:
        return set()
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    if platform is None:
        ...
        if not isinstance(config["skills"].get("platform_disabled"), dict):
