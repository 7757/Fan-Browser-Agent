"""``fan logs`` — view and filter Fan log files.
...
Usage examples::
    ...
    fan logs                    # last 50 lines of agent.log
    fan logs -f                 # follow agent.log in real time
    fan logs errors             # last 50 lines of errors.log
    fan logs gateway -n 100    # last 100 lines of gateway.log
    fan logs gui -f            # follow gui.log (dashboard/pty/ws)
    fan logs desktop -f        # follow desktop.log (Electron app boot/backend)
    fan logs --level WARNING    # only WARNING+ lines
    fan logs --session abc123   # filter by session ID substring
    fan logs --component tools  # only tool-related lines
    fan logs --since 1h         # lines from the last hour
    fan logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_fan_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Fan runs — try 'fan chat' first)")
        sys.exit(1)
    ...
    if component:
        from fan_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_fan_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_fan_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_fan_home()}/logs/")
        return

    print(f"Log files in {display_fan_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'fan chat' to generate logs)")
