...
Both the interactive CLI (``cli.py``) and the gateway (``tui_gateway``) call
...
def _approve(subsystem: str, rest: List[str], memory_store) -> str:
    ...
    applied, failed = 0, []
    for rec in targets:
        # Claim-then-apply: discard_pending is an atomic unlink, so only the
        # surface that wins the unlink applies this record. The pending store is
        # a shared FAN_HOME dir with no lock, so a concurrent approve on another
        # slash-worker could otherwise snapshot + double-apply the same staged
        # write (mostly idempotent, but a degenerate self-containing patch could
        # double-apply for real). The loser of the unlink simply skips. Trade-off:
        # a genuine apply failure (e.g. disk error) drops the record instead of
        # leaving it for retry — acceptable vs the corruption it prevents.
...
def _apply_one(subsystem: str, rec, memory_store):
    ...
    try:
        if subsystem == wa.MEMORY:
            ...
            result = json.loads(apply_skill_pending(payload, origin=rec.get("origin", "foreground")))
            return bool(result.get("success")), result.get("error", "")
...
def _set_mode(subsystem: str, rest: List[str], set_mode_fn) -> str:
    ...
    if set_mode_fn is None:
        return (f"To change {subsystem} write mode, run:\n"
                f"  fan config set {subsystem}.write_mode {mode}")
    try:
        set_mode_fn(mode)
    except Exception as e:
