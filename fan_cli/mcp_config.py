"""
MCP Server Management CLI — ``fan mcp`` subcommand.

Implements ``fan mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.fan/config.yaml under the ``mcp_servers`` key.
"""
...
from fan_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_fan_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from fan_cli.cli_output import prompt as _shared_prompt
...
def _save_mcp_server(name: str, server_config: dict) -> bool:
    ...
    shell+egress payloads rather than whitelisting command families. (upstream 972a9885e)
    """
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.fan/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``fan mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import FanTokenStorage
        return FanTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # fan_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  fan mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  fan mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  fan mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_fan_home()}/.env as {env_key}")
    ...
    try:
        try:
            configured_timeout = float(server_config.get("connect_timeout", 0))
        except (TypeError, ValueError):
            configured_timeout = 0.0
        # OAuth waits for a human browser flow; never let the ordinary MCP
        # probe's short timeout pre-empt its 300-second callback window.
        tools = _probe_single_server(
            name,
            server_config,
            connect_timeout=max(configured_timeout, 315.0),
        )
    except Exception as exc:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: fan mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_fan_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `fan mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  fan mcp add <name> --url <endpoint>')
        _info('  fan mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `fan mcp remove` + `fan mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `fan mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'fan mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``fan mcp`` subcommands."""
    ...
    if action == "picker":
        from fan_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from fan_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from fan_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `fan plugin`.
        from fan_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("fan mcp                                    Open the catalog picker (default)")
        _info("fan mcp catalog                            List Nous-approved MCPs")
        _info("fan mcp install <name>                     Install a catalog MCP")
        _info("fan mcp serve                              Run as MCP server")
        _info("fan mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("fan mcp add <name> --command <cmd>         Add a stdio server")
        _info("fan mcp add <name> --preset <preset>       Add from a known preset")
        _info("fan mcp remove <name>                      Remove a server")
        _info("fan mcp list                               List configured servers")
        _info("fan mcp test <name>                        Test connection")
        _info("fan mcp configure <name>                   Toggle tools")
        _info("fan mcp login <name>                       Re-authenticate OAuth")
        print()
