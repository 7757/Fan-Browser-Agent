"""CLI for the Fan Kanban board — ``fan kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/fan-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from fan_cli import kanban_swarm as ks
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this FAN_HOME and its
    ...
    Used by ``fan kanban create`` (and callers) to warn when a task
    ...
    """
    # Fan's desktop backend owns the embedded dispatcher. The advisory lock is
    # the authoritative cross-process liveness probe; no deleted gateway PID
    # file is involved.
    try:
        from fan_cli.kanban_dispatcher import dispatcher_lock_is_held

        lock_held = dispatcher_lock_is_held()
    except Exception:
        lock_held = None

    if lock_held:
        return (True, "a Kanban dispatcher owns the singleton lock")
    if lock_held is None:
        return (True, "")
    return (
        False,
        "No Kanban dispatcher is running — the task will sit in 'ready'. "
        "Open the Fan desktop app (its local backend owns the dispatcher), "
        "or use `fan kanban daemon --force` for standalone operation."
    )
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        "kanban",
        help="Collaboration board (tasks, links, comments)",
        description=(
            "Durable SQLite-backed task board shared across browser-agent workers. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by assigned workers in an isolated workspace."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # FAN_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `fan kanban boards switch <slug>` or the "
            "FAN_KANBAN_BOARD env var). Use `fan kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    # --- worker-run (internal, hidden) ---
    # Headless worker runtime spawned by the dispatcher
    # (kanban_db._default_spawn). Not an interactive entrypoint — hidden
    # from help. See fan_cli/kanban_worker.py.
    p_worker = sub.add_parser("worker-run", help=argparse.SUPPRESS)
    p_worker.add_argument(
        "-q",
        "--query",
        required=True,
        help="Worker prompt (e.g. 'work kanban task <id>')",
    )
    ...
    p_create.add_argument("--assignee", default=None, help="Assignee lane to assign")
    ...
    p_swarm.add_argument(
        ...
        action="append",
        default=[],
        metavar="ASSIGNEE:TITLE[:SKILL,SKILL]",
        help="Parallel worker card (repeatable)",
    )
    p_swarm.add_argument("--verifier", required=True, help="Verifier lane")
    p_swarm.add_argument("--synthesizer", required=True, help="Synthesizer/writer lane")
    ...
    p_swarm.add_argument("--created-by", default=None, help="Creator/anchor lane")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $FAN_KANBAN_AUTHOR as assignee")
    ...
    p_assign.add_argument("assignee", help="Assignee lane (or 'none' to unassign)")
    ...
    p_reassign = sub.add_parser(
        "reassign",
        help="Reassign a task to a different assignee lane, optionally reclaiming first",
    )
    p_reassign.add_argument("task_id")
    p_reassign.add_argument(
        "assignee",
        help="New assignee lane (or 'none' to unassign)",
    )
    p_reassign.add_argument(
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $FAN_KANBAN_AUTHOR or 'user')")
    ...
    p_block.add_argument(
        ...
        help=(
            "Why the task is blocked. dependency waits in todo; other kinds "
            "need a human. Repeated same-cause blocks are sent to triage."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="Compatibility daemon; the Fan desktop backend normally owns dispatch.",
    )
    ...
    p_watch.add_argument("--assignee", default=None,
                         help="Only show events for tasks assigned to this lane")
    ...
    p_runs = sub.add_parser(
        "runs",
        help="Show attempt history for a task (one row per run: assignee, "
    ...
    p_asg = sub.add_parser(
        "assignees",
        help="List known assignee lanes + task counts "
             "(current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $FAN_KANBAN_AUTHOR or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose = sub.add_parser(
        ...
        help="Decompose a triage-column task into a graph of child tasks "
             "routed to specialist lanes by description. Falls back to "
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $FAN_KANBAN_AUTHOR or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``fan kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: fan kanban <action> [options]\n"
                "Run 'fan kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # Internal worker runtime (dispatcher-spawned subprocess). Board / task
    # context arrives via FAN_KANBAN_* env vars set by _default_spawn, so it
    # bypasses the --board pinning below. The exit code is part of the
    # dispatcher's reap contract (rate-limit sentinel) — propagate it.
    if action == "worker-run":
        from fan_cli.kanban_worker import run_worker
    ...
    # env-var pin for the duration of this call. Using FAN_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `fan kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # FAN_HOME. Previously only `init` and `daemon` triggered
...
def _actor_author() -> str:
    """Best-effort author name for an interactive CLI call."""
    for env in ("FAN_KANBAN_AUTHOR", "FAN_AGENT_NAME"):
...
# ---------------------------------------------------------------------------
# Boards management (fan kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``fan kanban boards <action>``.
    ...
    DB, so a fresh FAN_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `fan kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `fan kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `fan kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `fan kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    print("Assignee lanes are logical task labels; no separate setup is required.")
    print()
    print("Next step: open the Fan desktop app so ready tasks get picked up.")
    print()
    print(
        "The desktop backend hosts an embedded dispatcher every 60 seconds\n"
        "by default (config: kanban.dispatch_interval_seconds). Without a\n"
        "running desktop backend or standalone daemon, tasks stay in 'ready'."
    )
    return 0
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — assign a lane name)")
        return 0
    # Header
    print(f"{'NAME':20s}  {'CONFIG':8s}  COUNTS")
    for entry in data:
        ...
        print(f"{entry['name']:20s}  {configured:8s}  {count_str}")
    return 0
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        task_id = kb.create_task(
            ...
            created_by=args.created_by or _actor_author(),
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from fan_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``fan kanban show <id>``
    ...
    if runs:
        ...
        for r in runs:
            ...
            print(f"  #{r.id:<3} {outcome:<12} @{r.assignee or '-'}  {el}  "
...
def _cmd_assign(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ok = kb.assign_task(conn, args.task_id, assignee)
    if not ok:
        print(f"no such task: {args.task_id}", file=sys.stderr)
        return 1
    print(f"Assigned {args.task_id} to {assignee or '(unassigned)'}")
    return 0
...
def _cmd_comment(args: argparse.Namespace) -> int:
    ...
    author = args.author or _actor_author()
    with kb.connect_closing() as conn:
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("FAN_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("FAN_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
        ...
        for tid in ids:
            ...
            if not kb.block_task(
                ...
                if where == "todo":
                    ...
                    print(f"{tid} → triage (unblock loop detected){suffix}")
                else:
...
def _cmd_unblock(args: argparse.Namespace) -> int:
    ...
    author = _actor_author() if reason else None
    failed: list[str] = []
    with kb.connect_closing() as conn:
        for tid in ids:
            if reason:
...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    # (#33488), kanban.max_in_progress_per_assignee as the per-assignee
    # cap (#21582), and kanban.max_spawn as the per-tick spawn limit
    # (#28805). Same semantics as the desktop backend dispatch path so behavior
    # matches whether the user runs the CLI directly or relies on the
    # desktop-backend dispatcher.
    try:
        from fan_cli.config import load_config
        _cfg = load_config()
        ...
        max_in_progress_per_assignee = _coerce_positive_int(
            _kanban_cfg.get("max_in_progress_per_assignee")
        )
    ...
    if getattr(args, "json", False):
        print(json.dumps({
            ...
            "skipped_per_assignee_capped": [
                {"task_id": tid, "assignee": who, "current": current}
                for (tid, who, current) in res.skipped_per_assignee_capped
            ],
    ...
    if res.skipped_per_assignee_capped:
        for tid, who, current in res.skipped_per_assignee_capped:
            print(
                f"Deferred ({who} at per-assignee cap, {current} running): {tid}"
            )
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    """Compatibility daemon for hosts that do not run the desktop backend.
    ...
    cannot run the desktop backend (for example, a headless worker host),
    but the default path exits 2
    ...
    if not getattr(args, "force", False):
        print(
            "fan kanban daemon: the dispatcher normally runs inside\n"
            "the Fan desktop backend. Open the Fan app to use Kanban.\n"
            "\n"
            ...
            "\n"
            "A machine-level lock prevents the desktop backend and this\n"
            "standalone daemon from dispatching together. For headless\n"
            "operation, rerun with --force.",
            file=sys.stderr,
        )
        return 2
    ...
    from fan_cli.kanban_dispatcher import (
        acquire_dispatcher_lock,
        release_dispatcher_lock,
    )

    dispatcher_lock, lock_state = acquire_dispatcher_lock()
    if lock_state != "held":
        if pidfile:
            try:
                Path(pidfile).unlink()
                ...
                pass
        print(
            "fan kanban daemon: dispatcher lock is "
            f"{lock_state}; refusing to run a second or unlocked dispatcher.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        ...
        f"Ctrl-C to stop. The singleton lock excludes the desktop dispatcher.",
        file=sys.stderr,
    )

    # Health telemetry: warn when every tick finds ready work but fails to
    # spawn any worker. Catches broken workers, PATH drift, missing venv,
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"Check worker health (venv, PATH, credentials) and "
                    f"`fan kanban list --status ready` / "
                    f"`fan kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a known assignee lane (i.e. one the
    ...
    try:
        ...
        if pidfile:
            try:
                ...
                pass
        release_dispatcher_lock(dispatcher_lock)
...
def _cmd_runs(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        print(json.dumps([
            {
                "id": r.id, "assignee": r.assignee, "status": r.status,
    ...
    if not runs:
        ...
        return 0
    print(f"{'#':3s}  {'OUTCOME':12s}  {'ASSIGNEE':16s}  {'ELAPSED':>8s}  STARTED")
    for i, r in enumerate(runs, 1):
        ...
        print(f"{i:3d}  {outcome:12s}  {assignee:16s}  {el:>8s}  {_fmt_ts(r.started_at)}")
        if r.summary:
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from fan_cli import kanban_specify as spec

    all_flag = bool(getattr(args, "all_triage", False))
    tenant = getattr(args, "tenant", None)
    author = getattr(args, "author", None) or _actor_author()
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    tasks via the auxiliary LLM, routed to specialist lanes by
    description. Thin wrapper over ``kanban_decompose``."""
    from fan_cli import kanban_decompose as decomp
...
Common subcommands:
  ...
  `assign <id> <assignee>`  Reassign
  `boards list`         Show all boards
  `assignees`           Known assignees + counts
