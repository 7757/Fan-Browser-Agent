"""
Configuration management for Fan Agent.

Config files are stored in ~/.fan/ for easy access:
- ~/.fan/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.fan/.env         - API keys and secrets

This module provides:
- fan config          - Show current configuration
- fan config edit     - Open config in editor
- fan config set      - Set a specific value
- fan config wizard   - Re-run setup wizard
"""
...
from fan_cli.providers import DEEPSEEK_API_KEY_ENV, DEEPSEEK_DEFAULT_MODEL
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``fan config set`` (which rewrites
    ...
    ``config.yaml`` in place: fan never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    *,
    ...
    A YAML parse error in ``~/.fan/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``fan logs``
    ...
    if fallback == "last-known-good":
        recovery = (
            "Keeping the previously loaded configuration; edits are ignored "
            "until the YAML is fixed."
        )
    elif fallback == "durable-last-known-good":
        recovery = (
            "Using the durable last-known-good configuration; the invalid "
            "config.yaml remains untouched until it is fixed."
        )
    else:
        recovery = (
            "No valid last-known-good configuration exists; falling back to "
            "defaults. User overrides may be unavailable until the YAML is fixed."
        )
    msg = f"Failed to parse {config_path}: {exc}. {recovery}"
    if backup_path is not None:
        msg += f" A copy of the corrupted file was saved to {backup_path}."
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  fan config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Fan makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Fan itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``fan update``, the TUI build.
...
#   on every plugin install / ``fan update``.
...
# * ``FAN_HOME`` / ``FAN_CONFIG`` / ``FAN_ENV`` — Fan runtime location flags. Writing these into
...
#
# IMPORTANT: ``FAN_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (FAN_GEMINI_CLIENT_ID,
# FAN_LANGFUSE_PUBLIC_KEY, FAN_SPOTIFY_CLIENT_ID, ...). The
...
# planting them.
_PROCESS_ONLY_ENV_VARS: frozenset[str] = frozenset({
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Fan runtime location — never via dashboard env writer.
    # NOT a FAN_* blanket: integration credentials (FAN_GEMINI_*,
    # FAN_LANGFUSE_*, FAN_SPOTIFY_*, ...) ARE allowed.
    "FAN_HOME", "FAN_CONFIG", "FAN_ENV",
    # Ephemeral Electron-to-dashboard capabilities. These are consumed and
    # removed at process startup and must never be persisted or restored.
    *_PROCESS_ONLY_ENV_VARS,
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    """
    if key in _ENV_VAR_NAME_DENYLIST or key.upper() in _PROCESS_ONLY_ENV_VARS:
        raise ValueError(
            f"Environment variable {key!r} is on the writer denylist. "
            "Names that influence subprocess execution (LD_PRELOAD, "
            "PYTHONPATH, PATH, EDITOR, ...) or Fan runtime location "
            "(FAN_HOME, FAN_CONFIG, ...) cannot be persisted via "
...
_LOAD_CONFIG_CACHE: Dict[
    str, Tuple[int, int, int, Dict[str, Any], Dict[str, Optional[str]]]
] = {}
...
from fan_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("FAN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_fan_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Fan is running in package-manager-managed mode.

    Two signals: the FAN_MANAGED env var (set by the systemd service),
    or a .managed marker file in FAN_HOME (set by the NixOS activation
    script, so interactive shells also see it).
    """
    return get_managed_system() is not None


def format_managed_message(action: str = "modify this Fan installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("FAN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Fan installation is managed by NixOS "
            f"(FAN_MANAGED={env_hint}).\n"
            "Edit services.fan-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Fan installation is managed by Homebrew "
            f"(FAN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade fan-agent"
        )

    return (
        f"Cannot {action}: this Fan installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Fan."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from FAN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, fan_bin
    or None if container mode is not active, we're already inside the
    container, or FAN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("FAN_DEV") == "1":
        return None

    from fan_constants import is_container
    if is_container():
        return None

    container_mode_file = get_fan_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "fan-agent")
    exec_user = info.get("exec_user", "fan")
    fan_bin = info.get("fan_bin", "/data/current-package/bin/fan")
...
# Re-export from fan_constants — canonical definition lives there.
from fan_constants import get_fan_home  # noqa: F811,E402
...
def _resolve_fan_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the FAN_UID / FAN_GID env vars set by Docker deployments.

    Docker containers running Fan commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level FAN_HOME once, but
    subdirectories created at runtime by ``ensure_fan_home()`` (especially
    for runtime-created subdirectories) need the same chown
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("FAN_UID", "").strip()
    gid_str = os.environ.get("FAN_GID", "").strip()
    try:
...
def _chown_to_fan_uid(path) -> None:
    """Chown ``path`` to ``FAN_UID:FAN_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_fan_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_fan_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the fan group can
    share state with the gateway service.

    The mode can be overridden via the FAN_HOME_MODE environment variable
    (e.g. FAN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse FAN_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``FAN_UID``/``FAN_GID``-based ownership when those env
    vars are set (#34107 — Docker deployments need this so runtime subdirectories
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("FAN_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_fan_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Fan runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("FAN_CONTAINER") or os.environ.get("FAN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set FAN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into FAN_HOME if the user doesn't have one yet."""
...
def ensure_fan_home():
    """Ensure ~/.fan directory structure exists with secure permissions.
...
def _ensure_fan_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"FAN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["fan-cli"],
    # Global active chat session cap across CLI, gateway/desktop, and cron.
    # None/0 = unbounded (default). When set, session create/resume/branch on
    # the gateway are gated; CLI is not capped (single-user surface).
    "max_concurrent_sessions": None,
    # Internal hard cap for one automatically loaded context file. None scales
    # from the active model window; no desktop setting is exposed for this.
    "context_file_max_chars": None,
    "agent": {
        "max_turns": 90,
        # Reasoning effort forwarded to the model ("" = provider default).
        # Read by tui_gateway/server.py.
        "reasoning_effort": "",
        # Max app-level retry attempts for API errors (connection drops,
        # provider timeouts, 5xx, etc.) before the agent surfaces the
        # failure.  The OpenAI SDK already does its own low-level retries
        # (max_retries=2 default) for transient network errors; this is
        # the Fan-level retry loop that wraps the whole call.  Lower
        ...
        # When a model only acknowledges an intended action without making its
        # tool call, inject one bounded continuation prompt. ``auto`` preserves
        # the existing Responses API-only behaviour; true enables it for every
        # API mode, false disables it, and a list enables matching models.
        ...
        # prompt's environment-hints block. Lets a host that wraps Fan
        ...
        # identity slot (SOUL.md). Empty by default. The FAN_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        "timeout": 180,
        # Grace period before a managed background process tree that ignored
        # SIGTERM is escalated to SIGKILL. Set to 0 for TERM-only behavior.
        ...
        # skipped. When empty, Fan auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Fan sources the user's shell rc files
    ...
    "browser": {
        # Default for omnibox text searches and browser_search calls that do
        # not explicitly choose an engine. One of: baidu, bing, google,
        # duckduckgo.
        "search_engine": "baidu",
    ...
    # Opt in via ``fan chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": True,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.fan/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: fan sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Fan truncates the
    ...
    "tool_loop_guardrails": {
        ...
        # Browser loops are a product-correctness boundary, not an optional CLI
        # preference. Two identical paths or same-family/same-error outcomes
        # close only that route; the model then gets two bounded strategy
        # pivots. A turn stops only after the total six-outcome budget.
        "browser_no_progress": {
            "warn_after": 2,
            "path_block_after": 2,
            "same_method_error_block_after": 2,
            "strategy_pivot_after": 3,
            "strategy_pivot_interval": 2,
            "strategy_pivot_limit": 2,
            "total_budget": 6,
        },
    },
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | alibaba | alibaba-coding-plan | custom
        ...
        "approval": {
            "provider": "auto",
            "model": "",           # fast/cheap model recommended (e.g. qwen3-flash)
            "base_url": "",
            "api_key": "",
            "timeout": 30,
            "extra_body": {},
        ...
        # Invoked by ``fan kanban specify`` (single id or --all). Set a
        ...
        # child tasks routed to specialist lanes by description.
        # Invoked by ``fan kanban decompose`` and the kanban
        ...
        # model; override via `fan model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Print a one-line summary of resolved modal prompts (dangerous-command
        # approval / collect questions) into scrollback so the question + the
        # decision survive the prompt_toolkit panel repaint. Capped at 120 chars.
        ...
        "tool_progress_command": False,  # Enable the /verbose command in interactive clients
        "tool_preview_length": 0,  # Max chars for tool call previews (0 = no limit, show full paths/commands)
        # Gateway runtime-metadata footer appended to the FINAL message of a turn
        # (disabled by default to keep replies minimal). When enabled, renders
        # e.g. `model · 68% · ~/projects/fan`. Per-platform overrides go under
    ...
    # for Lossless Context Management). The engine must be installed through
    # the general plugin system.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        # Built-in persistent memory and the user profile are product
        # invariants. They are always enabled for normal sessions and are no
        # longer user-configurable; ``skip_memory`` remains the explicit
        # internal escape hatch for temporary/test agents.
        ...
        #             committed. Pending: /memory pending|approve <id>|reject <id>.
        "write_mode": "on",
        "memory_char_limit": 2200,   # ~800 tokens at 2.75 chars/token
        "user_char_limit": 1375,     # ~500 tokens at 2.75 chars/token
        # External memory provider plugin (empty = built-in only).
        # Default "holographic" = local SQLite fact-store (FTS5 + HRR recall).
    ...
    # configured API-key and custom providers are supported.
    "delegation": {
        ...
        "api_mode": "",    # wire protocol for delegation.base_url: "chat_completions"
                           # or "codex_responses". Empty = chat_completions.
        # When delegate_task narrows child toolsets explicitly, preserve any
        # MCP toolsets the parent already has enabled. On by default so
        # narrowing (e.g. toolsets=["browser","file"]) expresses "I want these
        # extras" without silently stripping MCP tools the parent already has.
        # Set to false for strict intersection.
        "inherit_mcp_toolsets": True,
        "max_iterations": 50,  # per-subagent iteration cap (each subagent gets its own budget,
                               # independent of the parent's max_iterations)
        "child_timeout_seconds": 0,  # 0 (or negative) = NO wall-clock timeout (default): heavy
                                     # subagents fail only on real errors, not a countdown —
                                     # high-reasoning models on large tasks were timing out before
                                     # producing output. A positive value re-enables a per-child
                                     # cap (floored at 30s).
                                     # was the missing half; the parse logic was already ported.)
    ...
    # response. If not, Fan feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Fan auto-pauses the goal and
    ...
    # always goes to ~/.fan/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${FAN_SKILL_DIR} and ${FAN_SESSION_ID} in SKILL.md
        ...
        # remove_file), applied to BOTH foreground turns and the background
        # self-improvement review fork:
        #   on      — write freely (default, current behaviour)
        #   off     — never write; skill_manage returns a clean disabled result
        #   approve — stage the write for review instead of committing. Pending:
        #             /skills pending, /skills diff <id>, /skills approve|reject <id>.
        "write_mode": "on",
    },

    # Curator — background skill maintenance.
    #
    # Periodically reviews agent-created skills and keeps the collection tidy:
    # marks long-unused skills
    ...
    # cron daemon.
    #
    # See `fan curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # LLM umbrella-building is opt-in. The deterministic stale/archive
        # pass still runs when the curator is enabled, but no auxiliary Agent
        # may merge or rewrite skills unless this is explicitly true.
        "consolidate": False,
        ...
        # normally restored on every `fan update`, so pruning them only
        # sticks because a suppression list tells the re-seeder to leave them
        # archived. Built-ins accrue usage telemetry and their
    ...
    # stored approval from ~/.fan/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or FAN_ACCEPT_HOOKS=1.
    ...
    "approvals": {
        "mode": "manual",
        # Gateway-side approval wait ceiling (seconds). Read by tools/approval.py.
        "gateway_timeout": 300,
        "timeout": 60,
        "cron_mode": "deny",
        # User-owned terminal deny rules. Entries are case-insensitive fnmatch
        # globs and win even when a session enables yolo/mode=off.
        "deny": [],
        ...
        # Three-option prompt (Approve Once / Always Approve / Cancel).  Users
        # click "Always Approve" to silence the prompt permanently; that flips
        # this key to false.
    ...
    "security": {
        ...
        # Allow Fan to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via FAN_CRON_MAX_PARALLEL env var.
        "max_parallel_jobs": None,
        # Keep only the newest N per-run output files for each job. A
        # non-positive value disables pruning for externally-managed storage.
        "output_retention": 50,
        # Bound state.db initialization so a wedged SQLite connection cannot
        # leave a Cron job permanently marked as running.
    ...
    # todos to ready, and fires worker subprocesses for each claimable
    # ready task. Running more than one dispatcher on the same kanban.db
    # will race for claims.
    "kanban": {
        # Subscribe the desktop conversation that calls ``kanban_create`` to
        # completion/block notifications for the new task. The subscription
        # is persisted in kanban.db, so an event produced while the App is
        # closed is delivered when that same conversation is opened again.
        ...
        # same task/assignee (spawn_failed, timed_out, or crashed). Reassignment
        # resets the streak for the new assignee.
        "failure_limit": 2,
        ...
        # Assignee lane that decomposes tasks in the Triage column. When unset,
        # decomposition stays on the default lane.
        "orchestrator_assignee": "",
        # Where a child task lands if the orchestrator cannot match an
        # assignee lane. When unset, falls back to the default lane.
        "default_assignee": "",
        # Per-assignee concurrency cap (#21582). When set to a positive int,
        # no single assignee lane can have more than N workers running at once,
        # even if the global max_in_progress / max_spawn caps would allow
        # it. Tasks blocked this way defer to the next dispatcher tick.
        # Unset (None) means "no per-assignee cap" — backward-compatible
        # with existing installs. Useful for fan-out workflows that would
        # otherwise saturate one lane while leaving other lanes idle.
        "max_in_progress_per_assignee": None,
        # When true, the kanban dispatcher auto-runs the decomposer on
        # tasks that land in Triage (every dispatcher tick). When false,
        # decomposition is manual via `fan kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     fan-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Fan tools (terminal, read_file, write_file, patch,
    # search_files, todo, memory, and the browser-program interface) are
    # NEVER deferred. Explicitly enabled legacy browser atomics may defer.
    ...
    "tools": {
        "tool_search": {
            ...
            # Percentage threshold. Range 0..100.
            "threshold_pct": 10,
    ...
    # Logging — controls file logging to ~/.fan/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.fan/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.fan/sessions/session_{sid}.json`` on every turn
    ...
    "onboarding": {
        "seen": {},
        # On the user's very first message ever, offer (consent-first) to build
        # a short user profile and persist confirmed facts to the user-profile
        # memory store (target="user"). "ask" = offer once (default); "off" =
        # never offer. The offer is latched in onboarding.seen so it fires at
        # most once per install; any lookups inside the flow are separately
        # consented to in conversation.
        "profile_build": "ask",
    },
    ...
    # tier — handy for ad-hoc local sessions whose cwd is the user's
    # home directory.
    "lsp": {
    ...
    # rather than storing them in ~/.fan/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.fan/.env (or your shell) and never in config.yaml.
            ...
            # ~/.fan/bin/ on first use.  When False you must install
...
ENV_VARS_BY_VERSION: Dict[int, List[str]] = {
    3: [],  # FAL_KEY removed with image generation
...
# selection step, so this dict is intentionally empty — no single env var is
...
OPTIONAL_ENV_VARS = {
    DEEPSEEK_API_KEY_ENV: {
        "description": "DeepSeek API key for the main model",
...
def normalize_model_endpoint_identity(base_url: str) -> str:
    """Return a stable identity for credential-bound model endpoints.

    Endpoint credentials may only be reused when the destination is the same.
    Normalization therefore folds URL syntax that does not change the target
    (scheme/host case, a default port, or trailing slashes) while preserving
    case-sensitive paths and query strings.
    """
    ...
    try:
        ...
        # Preserve any explicitly embedded user-info as part of the endpoint
        # identity. It is not recommended, but treating two such URLs as the
        # same could otherwise retain credentials across distinct targets.
        userinfo = ""
        if "@" in parsed.netloc:
            userinfo = parsed.netloc.rsplit("@", 1)[0] + "@"

        path = parsed.path.rstrip("/")
        return urlunsplit((scheme, userinfo + host, path, parsed.query, ""))
    except (TypeError, ValueError):
        return raw.rstrip("/")
...
def clear_model_endpoint_credentials(
    ...
    *,
    ...
    """Remove endpoint-bound credentials from a model configuration.

    ``model.api_key`` (and its legacy ``model.api`` spelling) belongs only to
    the custom endpoint for which it was supplied. Built-in providers resolve
    credentials from their own environment/auth state. Keeping these fields
    across a provider or endpoint switch can disclose one endpoint's secret to
    another endpoint and can also select the wrong wire protocol.
    """
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.fan.config`` entries, then checks
    ...
    try:
        ...
        # should never break `fan update`.  Skill-config prompting is a
...
# Provider normalization runs on every picker/runtime read.  Deduplicate
# static-config warnings so a malformed hand-edited entry cannot flood the
# rotating logger and stall a desktop process on Windows.
_PROVIDER_NORMALIZE_WARNED: set[tuple[str, str]] = set()


def _warn_once_per_provider(
    provider_key: str, signature: str, message: str, *args: Any
) -> None:
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        # Accepted for compatibility with Fan's own keyed provider writer;
        # it duplicates the mapping key and has no runtime effect here.
        "provider",
    ...
    for url_key in ("base_url", "url", "api"):
        ...
        if isinstance(raw_url, str) and raw_url.strip():
            ...
            # Keep environment/template endpoints intact.  They are expanded
            # at runtime, so validating the raw ``${HOST}`` / ``{region}``
            # form here would silently discard an otherwise valid provider.
    ...
    if isinstance(models, dict) and models:
        ...
        # Accept both plain ids and hand-edited ``{id/name: ...}`` rows.
        # Preserve row metadata while normalizing to the dict shape consumed
        # by the runtime and picker.
...
def get_custom_provider_tls_settings(
    ...
    """Return TLS settings from the matching custom-provider entry."""
    ...
    for entry in custom_providers:
        ...
        if isinstance(ca_bundle, str) and ca_bundle.strip():
            settings["ssl_ca_cert"] = ca_bundle.strip()
        ssl_verify = _coerce_ssl_verify(entry.get("ssl_verify"))
...
def apply_custom_provider_tls_to_client_kwargs(
    ...
    """Attach matching custom-provider TLS settings to client kwargs."""
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'fan setup' to create a valid config")]
        ...
        if isinstance(fb, list):
            ...
            if not fb.get("model"):
                issues.append(ConfigIssue(
                    ...
                    "Add: model: qwen/qwen3.5-plus (or another model)",
                ))
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Fan won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'fan doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("FAN_TOOL_PROGRESS")
            old_mode = get_env_value("FAN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from FAN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from FAN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("FAN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from FAN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 12:
        ...
        if isinstance(custom_list, list) and custom_list:
            ...
            for entry in custom_list:
                ...
                for tls_field in ("ssl_ca_cert", "ssl_verify"):
                    if tls_field in entry:
                        new_entry[tls_field] = entry[tls_field]
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$FAN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_fan_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `fan plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `fan curator status` has no
    ...
    #   3. Creates `~/.fan/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_fan_home() — old installs that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_fan_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: fan config set <key> <value>")
    ...
    # metadata.fan.config in their SKILL.md frontmatter.
    ...
    # malicious entry. Preserve the stanza for auditability but mark it disabled
    # so the next startup won't spawn it (the runtime loader in mcp_tool already
    # skips it, but this persists the disable + warns).
    config = read_raw_config()
...
def _deep_merge(base: dict, override: dict) -> dict:
    ...
    for key, value in override.items():
        ...
        # ``section:`` in YAML is a present key whose value is None.  Do not
        # let that erase a structured default section and make every later
        # ``section.get(...)`` reader crash.
...
def _env_ref_snapshot(obj: Any, snapshot: Optional[Dict[str, Optional[str]]] = None) -> Dict[str, Optional[str]]:
    """Capture current values for every ``${VAR}`` referenced in config."""
    if snapshot is None:
        snapshot = {}
    if isinstance(obj, str):
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    confusion on subsequent loads.  ``model.model`` and ``model.name`` are
    also accepted legacy aliases for the canonical ``model.default`` key.
    Precedence is ``default`` > ``model`` > ``name``.
    """
    model_in = config.get("model")
    model_has_api_base = (
        isinstance(model_in, dict) and bool(model_in.get("api_base"))
    )
    model_needs_canonical_id = isinstance(model_in, dict) and bool(
    ...
    model_has_legacy_provider_key = isinstance(model_in, dict) and bool(
        model_in.get("active_provider") or model_in.get("model_provider")
    )
    model_has_retired_deepseek_model = isinstance(model_in, dict) and (
        str(model_in.get("provider") or "").strip().lower() == "deepseek"
        and str(
            model_in.get("default")
            or model_in.get("model")
            or model_in.get("name")
            or ""
        ).strip().lower() in {"deepseek-chat", "deepseek-reasoner"}
    )
    if (
        not has_root
        and not model_has_api_base
        and not model_needs_canonical_id
        and not model_has_legacy_provider_key
        and not model_has_retired_deepseek_model
    ):
        return config
    ...
    for alias_value in (config.get("api_base"), model.get("api_base")):
        if alias_value and not model.get("base_url"):
            model["base_url"] = alias_value
    config.pop("api_base", None)
    model.pop("api_base", None)

    # ``active_provider`` and ``model_provider`` appeared at both root and
    # model level in older releases. Promote a value only when the canonical
    # model.provider is empty.
    legacy_provider = (
        model.get("active_provider")
        or model.get("model_provider")
        or config.get("active_provider")
        or config.get("model_provider")
    )
    if not model.get("provider") and legacy_provider:
        model["provider"] = legacy_provider
    for legacy_key in ("active_provider", "model_provider"):
        config.pop(legacy_key, None)
        model.pop(legacy_key, None)

    # The runtime consistently resolves model.default.  Promote older aliases
    # so a valid custom-provider configuration never emits an empty model id.
    ...
    if (
        str(model.get("provider") or "").strip().lower() == "deepseek"
        and str(model.get("default") or "").strip().lower()
        in {"deepseek-chat", "deepseek-reasoner"}
    ):
        model["default"] = DEEPSEEK_DEFAULT_MODEL

    return config


def ensure_provider_config_migrated() -> bool:
    """Atomically migrate stale provider keys in the local user config.

    A missing config is left missing. Existing valid configs are rewritten only
    when root aliases or model identifiers require migration.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return False
    ...
    raw = read_raw_config()
    normalized = _normalize_root_model_keys(raw)
    normalized["_config_version"] = DEFAULT_CONFIG["_config_version"]
    if normalized == raw:
        return False
    atomic_config_write(config_path, normalized, sort_keys=False)
    return True
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _fan_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.fan/config.yaml as-is, without merging defaults or migrating.
...
def _write_validated_config_snapshot(
    config_path: Path,
    config: Dict[str, Any],
) -> bool:
    """Atomically retain raw, unexpanded last-known-good user configuration."""
    ...
    try:
        ...
        atomic_yaml_write(snapshot_path, config, sort_keys=False)
        _secure_file(snapshot_path)
        return True
    except Exception as exc:  # snapshot maintenance must not break runtime load
...
def _read_validated_config_snapshot(config_path: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not isinstance(snapshot, dict) or _config_has_validation_errors(snapshot):
            raise ValueError("snapshot failed configuration structure validation")
        return snapshot
    except FileNotFoundError:
        return None
    except Exception as exc:
        logger.warning("Last-known-good config snapshot is unusable (%s): %s", snapshot_path, exc)
        return None


def require_readable_config_before_write(config_path: Optional[Path] = None) -> None:
    """Refuse to replace a present config that cannot be read and parsed.

    ``read_raw_config`` intentionally returns ``{}`` for both an absent file
    and a failed read.  Without this chokepoint, read-modify-write callers can
    mistake the latter for a new profile and erase the user's configuration.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"Refusing to overwrite {config_path}: existing config cannot be accessed ({exc})."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            f"Refusing to overwrite {config_path}: existing config cannot be read and parsed "
            f"({exc}). Fix it manually or move it aside first."
        ) from exc


def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    """The only supported full-file writer for ``config.yaml``."""
    from utils import atomic_yaml_write
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.fan/config.yaml.
    ...
    The cache is keyed on ``str(config_path)`` so config path changes
    (which change ``FAN_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        try:
            st = config_path.stat()
            cache_key: Optional[Tuple[int, int, int]] = (st.st_mtime_ns, st.st_size, 0)
        except FileNotFoundError:
            cache_key = None

        cached = _LOAD_CONFIG_CACHE.get(path_key)
        if cached is not None and cache_key is not None and cached[:3] == cache_key:
            env_snapshot = cached[4] if len(cached) > 4 else {}
            if all(os.environ.get(name) == value for name, value in env_snapshot.items()):
        ...
        if cache_key is not None:
            try:
                ...
                if not isinstance(user_config, dict):
                    raise ValueError("configuration root must be a mapping")
                parsed_user_config = copy.deepcopy(user_config)
                ...
                if last_known_good is not None:
                    config = copy.deepcopy(last_known_good)
                else:
                    durable = _read_validated_config_snapshot(config_path)
                    if durable is not None:
                        config = _deep_merge(copy.deepcopy(DEFAULT_CONFIG), durable)
        ...
        if parsed_user_config is not None:
            snapshot_config = _normalize_root_model_keys(
                _normalize_max_turns_config(parsed_user_config)
            )
            _write_validated_config_snapshot(config_path, snapshot_config)
...
#   alibaba             (DASHSCOPE_API_KEY) — Bailian / DashScope
#   alibaba-coding-plan (ALIBABA_CODING_PLAN_API_KEY) — Coding Plan
#
...
def _explicit_config_paths(config: Dict[str, Any]) -> Set[Tuple[str, ...]]:
    """Return leaf paths explicitly present in an un-normalized YAML config."""
...
def _strip_default_values(
    ...
    """Remove implicit defaults while retaining explicitly authored values."""
    preserve = {("_config_version",)} | set(preserve_keys or ())
    ...
    def _strip(value: Any, default: Any, path: Tuple[str, ...]) -> Any:
        ...
        if isinstance(value, dict) and value:
            ...
            kept: Dict[str, Any] = {}
            for key, child in value.items():
                stripped = _strip(child, default_dict.get(key), path + (str(key),))
    ...
    for key, value in config.items():
        stripped = _strip(value, defaults.get(key), (str(key),))
        if stripped is not None:
            result[key] = stripped
    return result
...
def save_config(
    ...
    *,
    ...
    """Save the user layer to ``config.yaml``.

    By default, implicit schema defaults are removed so future Fan defaults can
    take effect without being pinned into a user's config.yaml.
    """
    with _CONFIG_LOCK:
        if is_managed():
        ...
        ensure_fan_home()
        config_path = get_config_path()
        raw_for_paths = read_raw_config()
        explicit_raw_paths = (
            _explicit_config_paths(raw_for_paths) if raw_for_paths else set()
        )
        current_normalized = _normalize_root_model_keys(_normalize_max_turns_config(config))
        normalized = copy.deepcopy(current_normalized)
        raw_source = _normalize_root_model_keys(raw_for_paths)
        raw_existing = _normalize_max_turns_config(raw_source)
        if raw_existing:
        ...
        if strip_defaults:
            effective_preserve_keys = {("_config_version",)}
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the quoted .env value subset written by ``save_env_value``."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.fan/.env.
    ...
    interactive menu render (`fan tools`, `fan setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `fan tools`
    menu paint. The mtime check
    ...
    if env_path.exists():
        ...
        for line in lines:
            ...
            if line and not line.startswith('#') and '=' in line:
                # Accept the shell-compatible form ``export KEY=value``.
                # Otherwise the credential is indexed under ``export KEY`` and
                # silently ignored by runtime provider resolution.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    1. Concatenated KEY=VALUE pairs on a single line (missing newline between
       entries, e.g. ``DASHSCOPE_API_KEY=sk-...OPENAI_BASE_URL=https://...``).
    2. Stale ``KEY=***`` placeholder entries left by incomplete setup runs.

    Uses a known-keys set (OPTIONAL_ENV_VARS + _EXTRA_ENV_KEYS) so we only
    split on real Fan env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.fan/.env in place.
...
def _quote_env_value(value: str) -> str:
    """Quote values whose dotenv syntax would otherwise change their meaning."""
    if value == "":
        return value
    needs_quoting = (
        "#" in value
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.fan/.env."""
    if is_managed():
    ...
    try:
        ...
        # instead of letting _secure_file unconditionally tighten to 0600. (upstream e4a1b35a3)
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.fan/.env and os.environ.
    ...
    if found:
        ...
        try:
            with os.fdopen(fd, 'w', **write_kw) as f:
                ...
                os.fsync(f.fileno())
            atomic_replace(tmp_path, env_path)
            # Preserve the original file mode instead of letting _secure_file
            # unconditionally tighten to 0600. Mirrors save_env_value(). (upstream 15813336c)
            if original_mode is not None:
                try:
                    os.chmod(env_path, original_mode)
                except OSError:
...
def reload_env() -> int:
    """Re-read ~/.fan/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Fan — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if key.upper() in _PROCESS_ONLY_ENV_VARS:
            continue
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Fan vars that are no longer in .env
    for key in known_keys:
        if key not in env_vars and key in os.environ:
            del os.environ[key]
            count += 1
    # These values are main-process capabilities, not user configuration.
    # Remove even an out-of-band .env entry so `/reload` cannot reintroduce
    # them before cron or another direct subprocess copies os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.fan/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a managed credential, preferring ``~/.fan/.env``.

    A runtime may inherit a stale key from its parent shell.  For credentials,
    an explicit value in Fan's own .env is the user's latest local choice and
    must win so key rotation does not leave the process stuck on a 401.
    """
...
def redact_config_value(value: Any, _depth: int = 0) -> Any:
    """Copy a config value while structurally masking credential fields."""
    if _depth > 20:
        return value
    if isinstance(value, dict):
        redacted = {}
        for key, child in value.items():
            if (
                ...
                redacted[key] = redact_config_value(child, _depth + 1)
        return redacted
    if isinstance(value, list):
        return [redact_config_value(child, _depth + 1) for child in value]
    return value
...
def show_config():
    ...
    print(color("│              ⚕ Fan Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale FAN_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("FAN_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale FAN_MAX_ITERATIONS={_env_ghost} "
                f"(run 'fan doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  fan config edit     # Edit config file", Colors.DIM))
    print(color("  fan config set <key> <value>", Colors.DIM))
    print(color("  fan setup           # Run setup wizard", Colors.DIM))
    print()
...
def _is_agent_owned_config_mutation_context() -> bool:
    """Return whether config mutation was invoked by a model-owned runtime.

    A real user running ``fan config set`` in an independent shell has none of
    these markers and remains supported. Every terminal/background process
    launched by Fan's Agent sets ``FAN_AGENT_TOOL_SESSION``; the remaining
    markers cover gateway, cron and Kanban entrypoints directly as defense in
    depth. ContextVar identity covers same-process invocation paths.
    """
    truthy = {"1", "true", "yes", "on"}
    for name in (
        "FAN_AGENT_TOOL_SESSION",
        "FAN_GATEWAY_SESSION",
        "FAN_CRON_SESSION",
        "FAN_KANBAN_TASK",
    ):
        if str(os.environ.get(name, "")).strip().lower() in truthy:
            return True
        # FAN_KANBAN_TASK is normally an opaque task id, not a boolean.
        if name == "FAN_KANBAN_TASK" and str(os.environ.get(name, "")).strip():
            return True
    try:
        from fan_cli.invocation_context import get_current_invocation_session

        if get_current_invocation_session() is not None:
            return True
    except Exception:
        pass
    return False
...
def set_config_value(key: str, value: str) -> bool:
    """Set a configuration value from a user-controlled CLI shell."""
    if _is_agent_owned_config_mutation_context():
        print(
            "Refusing to modify Fan configuration from an Agent-owned process. "
            "Run 'fan config set' yourself in a separate terminal or use the settings UI.",
            file=sys.stderr,
        )
        return False
    if is_managed():
    ...
    api_keys = [
        ...
        'SUDO_PASSWORD', 'GITHUB_TOKEN', 'HONCHO_API_KEY',
    ]
    ...
    if normalized_key in {"model.api_base", "api_base"}:
    ...
    _config_to_env_sync = {
        ...
        # Persisting to .env causes stale values to poison child processes.
    ...
    if leaf_key in _SECRET_CONFIG_KEYS and value not in (None, ""):
        from agent.redact import mask_secret
...
def config_command(args):
    ...
    # Guard the whole official mutation surface, not only `config set`.
    # In particular, an Agent-controlled EDITOR could otherwise turn
    # `fan config edit` into an indirect write, while `config migrate` also
    # rewrites the same authority file. Read-only show/check/path commands
    # remain available inside Agent-owned subprocesses.
    if (
        subcmd in {"edit", "set", "migrate"}
        and _is_agent_owned_config_mutation_context()
    ):
        print(
            "Refusing to modify Fan configuration from an Agent-owned process. "
            "Run the command yourself in a separate terminal or use the settings UI.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: fan config set <key> <value>")
            print()
            print("Examples:")
            print("  fan config set model qwen3.5-plus")
            print("  fan config set terminal.backend docker")
            print("  fan config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        if not set_config_value(key, value):
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'fan config migrate' to add them")
        ...
        print("  fan config           Show current configuration")
        print("  fan config edit      Open config in editor")
        print("  fan config set <key> <value>   Set a config value")
        print("  fan config check     Check for missing/outdated config")
        print("  fan config migrate   Update config with new options")
        print("  fan config path      Show config file path")
        print("  fan config env-path  Show .env file path")
        sys.exit(1)
