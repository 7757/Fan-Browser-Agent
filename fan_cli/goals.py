"""Persistent session goals — the Ralph loop for Fan.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Fan feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.FanSession`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current FAN_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``fan_home`` path so alternate launch contexts still pick up the right DB.
...
def migrate_goal_to_session(
    ...
    *,
    ...
    """Move one persistent goal onto a compression continuation session.

    The child is written and read back before the parent is archived, so a
    transient write failure cannot silently erase the only active copy.
    """
    if (
        not old_session_id
    ...
    try:
        ...
        persisted = load_goal(new_session_id)
        if persisted is None:
            logger.warning(
                "GoalManager: goal migration target was not persisted: %s -> %s",
                old_session_id,
                new_session_id,
            )
            return False
        ...
        logger.warning(
            "GoalManager: goal migration failed %s -> %s: %s",
            old_session_id,
            new_session_id,
            exc,
        )
...
def run_kanban_goal_loop(
    *,
    ...
    (``fan kanban worker-run -q "work kanban task <id>"``). The worker's
