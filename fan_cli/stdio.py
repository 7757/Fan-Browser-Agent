...
on Japanese locales, etc.).  Fan's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``fan_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Fan is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``FAN_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("FAN_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Fan-managed Git install directories so
    ...
    # already-running shells don't see the change — which means fan
    ...
    # install without Fan-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Fan's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Fan's
    ``fan config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Fan (User env var in Windows Settings, or
    export in a PowerShell profile) and Fan picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Fan-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\fan\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``fan`` from right after install) retain their old PATH.

    Any subprocess Fan spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Fan-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "fan", "git", "cmd"),
        os.path.join(local_appdata, "fan", "git", "bin"),
        os.path.join(local_appdata, "fan", "git", "usr", "bin"),
        # Fan venv Scripts directory — host of the fan.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes fan, but harmless to include.
        os.path.join(local_appdata, "fan", "fan-agent", "venv", "Scripts"),
