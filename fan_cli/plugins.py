...
1. **Bundled plugins** — ``<repo>/plugins/<name>/`` (shipped with fan-agent;
   ``memory/`` and ``model-providers/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.fan/plugins/<name>/``
3. **Project plugins** – ``./.fan/plugins/<name>/`` (opt-in via
   ``FAN_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``fan_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``FAN_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.fan/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("FAN_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When FAN_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Fan loggers.
    """
    ...
    logger.debug(
        "FAN_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # command needs user approval across CLI, desktop, TUI, and ACP surfaces.
...
ENTRY_POINTS_GROUP = "fan_agent.plugins"

_NS_PARENT = "fan_plugins"


class PluginToolOverrideError(PermissionError):
    """A plugin tried to replace a host tool without operator consent."""
...
class PluginManifest:
    ...
    # lookups and by ``fan plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_tool(
        ...
        CDP-backed implementation). Third-party plugins additionally require
        ...
        if (
            ...
            and not self._tool_override_allowed(name)
        ):
            plugin_id = self.manifest.key or self.manifest.name
            raise PluginToolOverrideError(
                f"Plugin {plugin_id!r} cannot replace tool {name!r}. Set "
                f"plugins.entries.{plugin_id}.allow_tool_override: true "
                "in config.yaml to authorize this replacement."
            )
    ...
    def _tool_override_allowed(self, _tool_name: str = "") -> bool:
        """Return the operator's explicit replacement permission.

        Repository-bundled plugins are part of the Fan application and
        remain trusted. User, project and entry-point plugins are opt-in to
        load, but replacing a privileged host tool is a second, fail-closed
        decision.
        """
        if (self.manifest.source or "") == "bundled":
            return True
        try:
            ...
            # Security authority is local-only and literal. Server overlays,
            # env-expanded strings, and values such as "false" cannot grant
            # host tool replacement permission.
    ...
    def inject_message(self, content: str, role: str = "user") -> bool:
        ...
        This enables local plugins such as remote-control viewers to send
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``fan honcho ...``).
    ...
    def register_command(
        ...
        """Register a slash command (e.g. ``/lcm``) in local interactive sessions.

        The handler signature is ``fn(raw_args: str) -> str | None``.
        It may also be an async callable; the client dispatch handles both.

        Unlike ``register_cli_command()`` (which creates ``fan <subcommand>``
        terminal commands), this registers in-session slash commands that users
        invoke during a conversation.

        ``args_hint`` is an optional short string (e.g. ``"<file>"`` or
        ``"days:7 format:json"``) shown by clients that expose an argument
        field. Commands without it remain available with trailing free-form
        text.
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``fan model → Configure auxiliary models`` picker
        ...
        Args:
            key: stable task key (snake_case). Used in config ``auxiliary.<key>``
                ...
                built-in task key (vision, compression, approval, mcp,
        ...
        # Lazy import to avoid circular: fan_cli.main imports plugins indirectly
        from fan_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.fan/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to fan_cli/. Two layouts are
        # supported (see ``_scan_directory`` for details):
        #
        #   - flat: ``plugins/disk-cleanup/plugin.yaml`` (standalone)
        #   - category: ``plugins/image_gen/openai/plugin.yaml`` (backend)
        #
        # ``memory/`` and ``model-providers/`` are skipped at the top level —
        # they have their own discovery systems
        ...
        bundled = self._scan_directory(
            repo_plugins,
            source="bundled",
            skip_names={"memory", "platforms", "model-providers"},
        )
        logger.debug("  bundled (top-level): %d manifest(s)", len(bundled))
        manifests.extend(bundled)

        # 2. User plugins (~/.fan/plugins/)
        user_dir = get_fan_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.fan/plugins/)
        if _env_enabled("FAN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".fan" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set FAN_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with fan and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `fan plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        override_allowed = ctx._tool_override_allowed("")
        expected_namespace = ""
        if manifest.source in {"user", "project", "bundled"}:
            slug = plugin_id.replace("/", "__").replace("-", "_")
            expected_namespace = f"{_NS_PARENT}.{slug}"
        policy_token = _registry.begin_plugin_load(
            plugin_id,
            override_allowed,
            expected_namespace,
        )
        ...
        try:
            ...
            _registry.end_plugin_load(policy_token)

        self._plugins[manifest.key or manifest.name] = loaded

    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``fan_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``fan_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_pre_tool_call_block_message(
    ...
    """Resolve ``pre_tool_call`` block or approval directives.

    Plugins that need to enforce policy (rate limiting, security
    restrictions, approval workflows) can return::

        {"action": "block", "message": "Reason the tool was blocked"}
        {"action": "approve", "message": "Reason confirmation is needed",
         "rule_key": "optional-stable-allowlist-key"}

    ``block`` vetoes the call. ``approve`` routes through the same human
    approval system as a dangerous terminal command, failing closed when no
    user can answer. The first valid directive wins; invalid or irrelevant
    hook values remain observer-only for backward compatibility.
    """
    ...
    for result in hook_results:
        ...
        if action == "block" and isinstance(message, str) and message:
            return message
        if action == "approve":
            rule_key = result.get("rule_key")
            rule_key = rule_key.strip() if isinstance(rule_key, str) else ""
            try:
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``fan tools`` TUI so plugin-provided toolsets appear
