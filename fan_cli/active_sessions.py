...
def active_session_limit_message(active_count: int, max_sessions: int) -> str:
    return (
        f"fan is at the active session limit ({active_count}/{max_sessions}). "
        "Try again when another session finishes."
    )


def _state_dir() -> Path:
    return get_fan_home() / "runtime"
...
def _pid_alive(pid: Any, process_start_time: Any = None) -> bool:
    ...
    if pid_int <= 0:
        return False
    try:
        # Our fork deleted the messaging gateway (gateway.status._pid_exists),
        # so check liveness directly via psutil (already used by
        # _process_start_time). Fail-open (assume alive) if psutil is somehow
        # unavailable, so a live session's lease is never silently dropped —
        # the create_time PID-reuse guard below still rejects a recycled pid.
        import psutil  # type: ignore

        exists = bool(psutil.pid_exists(pid_int))
    except Exception:
        return True
    if not exists:
        return False
...
def transfer_active_session(
    ...
    *,
    ...
    """Move an existing lease to a rotated session id without dropping its slot."""
