"""MCP picker — interactive `fan mcp picker` (also the default `fan mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``fan mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `fan plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from fan_cli.cli_output import prompt_yes_no
from fan_cli.curses_ui import curses_single_select
from fan_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Fan session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from fan_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `fan mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: fan mcp install <name>    Picker: fan mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Fan is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Fan — update Fan from "
                "Settings > About in the desktop app to install this entry.",
                Colors.YELLOW,
            ))
        print()
    print()
...
def show_catalog() -> None:
    """`fan mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`fan mcp picker` (and default `fan mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`fan mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from fan_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `fan mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
