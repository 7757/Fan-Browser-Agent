...
Pure display functions with no FanSession state dependency.
"""
...
# them) rather than at module level. Keep the type-only reference available to
# checkers without paying their runtime import cost when only lightweight
# banner metadata is needed.
if TYPE_CHECKING:
...
from fan_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# =========================================================================
# Build/release version display
# =========================================================================

def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Fan git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$FAN_HOME/fan-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
        fan_home = get_fan_home()
        repo_dir = fan_home / "fan-agent"
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``fan_cli/build_info.py``) and return it as a frozen
...
def format_banner_version_label() -> str:
    ...
    base = f"Fan Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # optional memory backends) — they show as unavailable at banner time because the
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else FAN_CADUCEUS
    except Exception:
        _bskin = None
        _hero = FAN_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Xingfan Technology[/]")

    if os.getenv("FAN_YOLO_MODE"):
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else FAN_AGENT_LOGO
        console.print(_logo)
        console.print()
