"""CLI subcommand: `fan curator <subcommand>`.
...
def _cmd_status(args) -> int:
    ...
    print(
        ...
        f"{'' if curator.get_consolidate() else ' (prune-only by default)'}"
    )
...
def _cmd_run(args) -> int:
    ...
    if consolidate is None and not curator.get_consolidate():
        print(
            "curator: consolidation is off — running deterministic prune-only; "
            "pass --consolidate to enable the LLM merge pass for this run"
        )
    ...
    if not synchronous:
        print("curator pass running in background — check `fan curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`fan curator status` and run `fan curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.is_agent_created(args.skill):
        print(
            f"curator: '{args.skill}' is bundled or externally owned — cannot pin "
...
def _cmd_unpin(args) -> int:
    ...
    if not skill_usage.is_agent_created(args.skill):
        print(
            f"curator: '{args.skill}' is bundled or externally owned — "
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`fan curator unpin {args.skill}`"
        )
        return 1
...
# ---------------------------------------------------------------------------
# argparse wiring (called from fan_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        ...
        help="Enable the high-risk LLM umbrella-building pass for this run; "
             "without this flag the default is deterministic prune-only",
    )
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by fan_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="fan curator")
    register_cli(parser)
