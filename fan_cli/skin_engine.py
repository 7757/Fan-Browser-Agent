"""Fan CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.fan/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Fan Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Fan "       # Response box header label
    ...
    tool_emojis:
      ...
      vision_analyze: "🔮"    # Override vision tool emoji
...
.. code-block:: python
    ...
    from fan_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Fan Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.fan/skins/

BUILT-IN SKINS
==============

- ``default`` — Classic Fan gold/kawaii (the current look)
...
Drop a YAML file in ``~/.fan/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces FAN_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces FAN_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Fan — gold and kawaii",
        "colors": {
        ...
        "branding": {
            "agent_name": "Fan Agent",
            "welcome": "Welcome to Fan Agent! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Fan ",
            "prompt_symbol": "❯",
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    return {
        ...
        "collect-border": input_rule,
        "collect-title": f"{title} bold",
        "collect-question": f"{text} bold",
        "collect-choice": dim,
        "collect-selected": f"{title} bold",
        "collect-active-other": f"{title} italic",
        "collect-countdown": input_rule,
