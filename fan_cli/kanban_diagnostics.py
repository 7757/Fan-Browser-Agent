...
class DiagnosticAction:
    ...
    * ``unblock`` — PATCH status back to ``ready`` (for stuck-blocked
      diagnostics).
    * ``cli_hint`` — print/copy a shell command. No HTTP side effect.
...
def _generic_recovery_actions(task: Any, *, running: bool) -> list[DiagnosticAction]:
    ...
    out.append(DiagnosticAction(
        kind="reassign",
        label="Reassign to a different lane",
...
def _rule_hallucinated_cards(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    completing assignee. Task stayed in its prior state; the operator
    ...
    return [Diagnostic(
        ...
        detail=(
            ...
            f"exist or weren't created by its assignee. The completion was "
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``fan kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`fan kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"fan config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"fan config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: fan kanban specify {task_id}",
            payload={"command": f"fan kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    something about this task+assignee combo is broken and each retry
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        # Spawn is failing specifically — worker setup issue.
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label="Verify worker environment: fan doctor",
            payload={"command": "fan doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label="Check model provider configuration: fan doctor",
            payload={"command": "fan doctor"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: fan kanban log {task_id}",
                payload={"command": f"fan kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_repeated_crashes(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    ``completed`` means something about the task + assignee combo is
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: fan kanban events {task_id}",
            payload={"command": f"fan kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Operator typo'd the assignee — no configured lane or external worker matches.
    * A lane was removed, leaving its tasks stranded.
    * External worker pool (Codex CLI, Claude Code lane, custom daemon)
      is down, hung, or wasn't started.
    * Dispatcher is misconfigured (wrong board, wrong FAN_HOME).
    ...
    works for configured lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "fan kanban diagnostics"},
        ),
    ]
    ...
    return [Diagnostic(
        ...
        detail=(
            ...
            f"misspelled, the lane was removed, or the external "
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Fan runtime config.
