...
Used by ``fan kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``fan_cli/goals.py`` — same aux
...
FAN_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("FAN_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Fan Agent board.
...
def _actor_author() -> str:
    """Mirror of ``fan_cli.kanban._actor_author``. Kept local to
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=FAN_KANBAN_SPECIFY_MAX_TOKENS,
    ...
    with kb.connect_closing() as conn:
        ok = kb.specify_triage_task(
            ...
            title=new_title,
            body=new_body,
            author=author or _actor_author(),
        )
    if not ok:
