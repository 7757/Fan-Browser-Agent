"""SQLite-backed Kanban board for multi-agent, multi-project collaboration.

In a fresh install the board lives at ``<root>/kanban.db`` where
``<root>`` is the Fan home. Workers share the same board, workspaces,
and logs under ``<root>/kanban/``.
...
the same Fan home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``FAN_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``FAN_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``fan kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.fan``. In Docker / custom
deployments where ``FAN_HOME`` points outside ``~/.fan`` (e.g.
``/opt/fan``), ``<root>`` is ``FAN_HOME``. Legacy env-var
overrides still work:

* ``FAN_KANBAN_DB`` — pin the database file path directly.
* ``FAN_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``FAN_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``FAN_KANBAN_DB``,
``FAN_KANBAN_WORKSPACES_ROOT``, and ``FAN_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/fan-kanban-v1-spec.pdf`` for the full
...
# Blocking is not a single state.  A dependency wait should re-enter the
# normal parent-gated todo flow, while a human/capability/transient blocker
# needs attention.  Keeping the kind lets us avoid the cron/unblock/worker
# re-block loop that otherwise burns a worker forever.
VALID_BLOCK_KINDS = {"dependency", "needs_input", "capability", "transient"}

# A task that is unblocked and then blocks again for the same cause twice is
# not making progress.  Escalate it to triage instead of leaving it in the
# automated unblock cycle.  This is deliberately independent from the
# dispatcher crash/timeout failure counter.
...
# ``FAN_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
# When a host-local worker survives TERM/KILL, keep ownership briefly instead
# of releasing the claim and spawning a duplicate beside it.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``FAN_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("FAN_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``FAN_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("FAN_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``FAN_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "FAN_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "fan_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``fan-agent``
...
def kanban_home() -> Path:
    """Return the shared Fan root that anchors the kanban board.

    Resolution order:

    1. ``FAN_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_fan_root()``, which returns the current Fan root.

    The kanban board is shared across the current Fan home by design.
    Dispatcher and worker processes must resolve the same paths or they
    cannot hand work off reliably.
    """
    override = os.environ.get("FAN_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``fan kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``FAN_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``fan kanban boards
    ...
    env = os.environ.get("FAN_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``fan kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``FAN_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("FAN_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``FAN_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("FAN_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``FAN_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("FAN_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``fan kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``FAN_SESSION_ID``. NULL for
    # tasks created from the CLI, the dashboard, or any path that doesn't
    # set the env var. Lets clients render a per-session board without
    # relying on tenant + time-window heuristics.
    session_id: Optional[str] = None
    # Why the task last blocked.  Preserved across manual/cron unblocks so a
    # same-cause re-block can be detected rather than retried indefinitely.
    block_kind: Optional[str] = None
    # Reset only after a successful completion, never on unblock.
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- to the worker, overriding the configured default model. NULL = use
    -- the normal model selection.
    model_override       TEXT,
    -- Per-task override for the consecutive-failure circuit breaker.
    -- The value is the failure count at which the breaker trips — e.g.
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``FAN_SESSION_ID``. NULL
    -- for tasks created from the CLI, dashboard, or any path that doesn't
    -- set the env var. Indexed so per-session list queries stay cheap on
    -- larger boards.
    session_id           TEXT,
    -- Typed reason set by block_task.  Dependency waits remain in todo so
    -- parent gating can resume them without a human unblock.
    block_kind           TEXT,
    -- Survives unblock; a same-cause re-block at the limit is triaged to
    -- stop an automated unblock loop.  Completion resets it to zero.
...
-- Subscription from a local desktop surface to a task. The desktop backend
-- tails task_events and resumes the originating conversation for terminal
-- lifecycle events.
...
def _resolve_busy_timeout_ms() -> int:
    ...
    Kanban is the shared task dispatch bus, so worker stampedes are
    ...
    """
    raw = os.environ.get("FAN_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _cross_process_init_lock(path: Path):
    ...
    try:
        ...
        if not acquired:
            _log.warning(
                "Kanban init lock %s was not acquired within %.0fs; "
                "continuing with in-process locking and idempotent schema init",
                lock_path,
...
def _dispatch_tick_lock(db_path: Path):
    """Non-blocking board-scoped single-writer lock for one dispatch tick."""
    ...
    try:
        ...
        # Preserve availability on filesystems that cannot create the sidecar;
        # the process-wide dispatcher singleton still protects the main path.
        acquired = True
        handle = None
    try:
        yield acquired
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    multiple workers all hitting the same shared DB) reuse one backup
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Fan treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``FAN_KANBAN_DB`` env → ``FAN_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See fan_state._WAL_INCOMPAT_MARKERS for detection logic.
                from fan_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``fan kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``FAN_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
    ...
    if "block_kind" not in cols:
        # Existing blocks retain their legacy generic meaning (NULL).  The
        # first post-upgrade block establishes a typed cause when supplied.
...
def _rebuild_drifted_tables(conn: sqlite3.Connection) -> None:
    ...
    Old boards crash the local notification reader (``int(None)`` on a NULL id in
...
# SQLite's busy_timeout uses a near-deterministic backoff, so concurrent
# writers can re-collide in lockstep under a stampede. Retrying the transaction
# boundary with jitter breaks that convoy. Only BEGIN IMMEDIATE and COMMIT are
# retried: reissuing either never replays the transaction body, preserving the
# CAS semantics inside write_txn.
_BUSY_MAX_RETRIES = 5
...
def write_txn(conn: sqlite3.Connection):
    ...
    try:
        ...
        try:
            ...
            # A COMMIT that exhausts retries can leave the transaction open;
            # clean it up so this connection remains usable for the next write.
            try:
                conn.execute("ROLLBACK")
            except sqlite3.OperationalError:
                pass
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    value = re.sub(r"[^a-z0-9_-]+", "-", assignee.strip().lower()).strip("-_")
...
def create_task(
    ...
    *,
    ...
    each name to ``fan --skills ...`` alongside the built-in
    ``kanban-worker``. Use this to pin a task to a specialist skill
    (e.g. ``skills=["translation"]`` so the worker loads the
    translation skill regardless of the default config).
    """
    ...
    # `fan --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    if skills is not None:
        ...
        if toolset_typos:
            ...
            raise ValueError(
                ...
                "Put toolsets in the current config's `toolsets:` section "
...
# Canonical sort-order mappings for ``fan kanban list --sort``.
...
def assign_task(conn: sqlite3.Connection, task_id: str, assignee: Optional[str]) -> bool:
    ...
    with write_txn(conn):
        ...
        if row["assignee"] != assignee:
            # The retry guard is scoped to the task/assignee combination. A
            # human reassigning the task is an explicit recovery action, so the
            # new assignee should not inherit the previous assignee's streak.
            conn.execute(
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `fan kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``fan kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``fan kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``fan kanban block <id>``).  This is a deliberate handoff that
...
def reassign_task(
    ...
    *,
    ...
    This is the recovery path for "this assignee lane is blocked, try a
    different one". If ``reclaim_first`` is True, any active claim is
    ...
    Returns True if the reassign landed. ``assignee`` may be ``None`` to
    unassign entirely.
    """
    if reclaim_first:
    ...
    try:
        return assign_task(conn, task_id, assignee)
    except RuntimeError:
...
def _verify_created_cards(
    ...
    * ``created_by`` matches the completing task's ``assignee``
...
def complete_task(
    ...
    *,
    ...
    completion (``fan kanban complete <id>``) works without requiring
    ...
    not created by this worker's assignee), completion is blocked
    ...
    with write_txn(conn):
        ...
        # Carry the handoff summary in the event payload so desktop
        # notifications and dashboard consumers can render it without a
        ...
        # Carry artifact paths in the event payload so local consumers can
        # show the deliverables alongside the completion message. Workers pass these via
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``FAN_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Fan' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Fan-managed storage. A board ``default_workdir`` pointing at a
...
def _try_cleanup_parent_workspaces(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    parent are now done/archived, the parent's scratch workspace is removed
    (#33774).
    """
    try:
        parents = conn.execute(
...
#      via ``fan kanban show <id>`` and the dashboard.
...
def block_task(
    ...
    *,
    ...
    """Stop a ``running``/``ready`` task and route it by block cause.

    Dependency waits return to ``todo`` and are resumed by the existing
    parent-gating machinery; they must not enter the human ``blocked`` bucket.
    Other causes remain blocked, but a repeated same-cause block after an
    unblock is escalated to ``triage`` at :data:`BLOCK_RECURRENCE_LIMIT`.
    This breaks an otherwise unbounded unblock/re-block worker loop.
    """
    ...
    with write_txn(conn):
        ...
        if current is None:
            return False
        previous_kind = (
            current["block_kind"] if "block_kind" in current.keys() else None
        )
        previous_recurrences = (
            int(current["block_recurrences"])
            if "block_recurrences" in current.keys()
            and current["block_recurrences"] is not None
            else 0
        )

        if kind == "dependency":
            # Let recompute_ready() apply normal parent gating rather than
            # making an operator/cron repeatedly unblock a dependency wait.
            if expected_run_id is None:
                cur = conn.execute(
                    """
                    UPDATE tasks
                ...
                cur = conn.execute(
                    ...
                    UPDATE tasks
                     ...
                     WHERE id = ?
                       ...
                       AND current_run_id = ?
                    """,
                    (kind, task_id, int(expected_run_id)),
                )
            if cur.rowcount != 1:
                return False
            run_id = _end_run(
        ...
        # ``block_task`` can only run after a previous unblock put the task
        # back into ready/todo flow.  A matching persisted kind therefore
        # means the task is looping on the same cause.  ``None`` intentionally
        # compares equal to legacy generic blocks as well.
        recurrences = (
            previous_recurrences + 1 if previous_kind == kind else 1
        )
        target_status = (
            "triage"
...
def decompose_triage_task(
    ...
    *,
    ...
    its assignee (typically the orchestrator lane) wakes back up to
    ...
    ``children`` is a list of dicts, each shaped like::

        {
            "title": "...",
            "body": "...",                     # optional
            "assignee": "lane-name",           # optional, None -> default fallback
...
def _git_toplevel(path: Path) -> Optional[Path]:
    """Return the git toplevel containing ``path``, if one exists."""
    try:
...
def _ensure_git_worktree(repo_root: Path, target: Path, branch_name: str) -> None:
    """Materialize or reuse a linked worktree for one Kanban task."""
    target = target.expanduser().resolve(strict=False)
    repo_common = _git_common_dir(repo_root)
    if target.exists() and repo_common is not None:
        if _git_common_dir(target) == repo_common and _is_linked_worktree_checkout(target):
            return
...
def _resolve_worktree_workspace(
    ...
    *,
    ...
    """Resolve and materialize the task's real linked worktree."""
    ...
    if not task.workspace_path:
        ...
        if not board_default:
            raise ValueError(
                ...
                "default_workdir; configure an absolute git repository"
            )
    ...
    if requested.exists() and _is_linked_worktree_checkout(requested):
        return requested, _git_current_branch(requested) or branch_name

    repo_root = _git_toplevel(requested)
    if repo_root is not None and requested == repo_root:
        target = repo_root / ".worktrees" / task.id
        _ensure_git_worktree(repo_root, target, branch_name)
        return target, branch_name
    ...
    if repo_root is None:
        raise ValueError(
            ...
            "inside a git repository and is not a git repository root"
        )
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``scratch``: a fresh dir under ``<board-root>/workspaces/<id>/``,
      ...
      same for the dispatcher and every worker, so handoff is
      path-stable.
    ...
    - ``worktree``: a real linked git worktree at the explicit target, or at
      ``<board.default_workdir repo>/.worktrees/<task-id>`` when no target is
      supplied. Missing/non-git anchors fail closed instead of using process CWD.
...
# After this many consecutive non-success attempts on a task/assignee, the
...
# without thrashing. Overridable via ``FAN_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Fan
    worker. Expected steady-state on multi-lane setups; NOT an
    operator-actionable failure. Tracked separately so health telemetry
    can distinguish "real stuck" (nothing spawned but spawnable work
    available) from "correctly idle" (nothing spawnable in the queue)."""
    skipped_per_assignee_capped: list[tuple[str, str, int]] = field(default_factory=list)
    """Tasks deferred this tick because their assignee is already at
    ``kanban.max_in_progress_per_assignee`` (#21582). Each entry is
    ``(task_id, assignee, current_running_count)``. NOT an
    operator-actionable failure — the task will be picked up on a
    subsequent tick when the assignee has capacity. Separate bucket so
    telemetry / dashboards can show "this assignee lane is busy" vs
    ...
    """True when another process owns this board's current dispatch tick."""
...
def _worker_survived_termination(termination: dict) -> bool:
    """Return whether a signalled host-local worker is still alive."""
    return bool(
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    """Extend a surviving worker's claim so another worker cannot spawn."""
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``fan kanban
...
def _clear_failure_counter(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    success means the task + assignee combination is working and any
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """Return True iff there is at least one ready assigned task."""
    row = conn.execute(
        "SELECT 1 FROM tasks "
...
def dispatch_once(
    ...
    *,
    ...
    max_in_progress_per_assignee: Optional[int] = None,
) -> DispatchResult:
    """Run one dispatcher tick under a non-blocking board writer lock."""
    try:
        ...
        return _dispatch_once_locked(
            ...
            max_in_progress_per_assignee=max_in_progress_per_assignee,
        )
    ...
    *,
    ...
    if max_in_progress is not None and ready_rows:
        ...
        if max_spawn is None or max_spawn > remaining:
            max_spawn = remaining
    spawned = 0
    # Per-assignee concurrency cap (#21582): when set, track how many
    # workers each assignee already has in flight, and refuse to spawn
    # when this would push that assignee past the cap. Prevents
    # fan-out workloads from melting a single assignee lane while leaving other lanes idle.
    # Tasks blocked this way go to skipped_per_assignee_capped (not
    # skipped_unassigned — the operator-actionable signal is different:
    # "this assignee lane is busy, try again later" not "this needs routing").
    _per_assignee_cap = max_in_progress_per_assignee if (
        isinstance(max_in_progress_per_assignee, int)
        and max_in_progress_per_assignee > 0
    ) else None
    _per_assignee_running: dict[str, int] = {}
    if _per_assignee_cap is not None:
        for prow in conn.execute(
            "SELECT assignee, COUNT(*) AS n FROM tasks "
            "WHERE status = 'running' AND assignee IS NOT NULL "
            "GROUP BY assignee"
        ):
            _per_assignee_running[prow["assignee"]] = int(prow["n"])
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Per-assignee concurrency cap (#21582): even if there's global
        # headroom, refuse to spawn for an assignee that's already at
        # its in-flight cap. Prevents one assignee lane from being overwhelmed by a fan-out
        # while the global max_in_progress / max_spawn caps still allow
        # work on other lanes.
        if _per_assignee_cap is not None:
            current = _per_assignee_running.get(row_assignee, 0)
            if current >= _per_assignee_cap:
                result.skipped_per_assignee_capped.append(
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `fan kanban tail` — without
        ...
        if dry_run:
            ...
            if _per_assignee_cap is not None and row_assignee:
                _per_assignee_running[row_assignee] = (
                    _per_assignee_running.get(row_assignee, 0) + 1
                )
            continue
        ...
        try:
            ...
            # iterations in this same tick respect the per-assignee cap
            # (#21582). Subsequent ticks re-query from the DB.
            if _per_assignee_cap is not None and claimed.assignee:
                _per_assignee_running[claimed.assignee] = (
                    _per_assignee_running.get(claimed.assignee, 0) + 1
                )
        except Exception as exc:
            auto = _record_spawn_failure(
...
def _module_fan_argv() -> list[str]:
    """Return the interpreter-bound Fan CLI invocation."""
    # ``fan_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``fan`` package — there is no
    # ``fan`` package to import.
    return [sys.executable, "-m", "fan_cli.main"]


def _absolute_fan_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Fan shim."""
...
def _fan_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Fan executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_fan_argv()
    return [_absolute_fan_path(path)]


def _resolve_fan_argv() -> list[str]:
    """Resolve the ``fan`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$FAN_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("fan")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\fan.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m fan_cli.main`` — fallback for setups where
       Fan is launched from a venv and the ``fan`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_fan_bin`` for the same reason. Kept
    local (not imported from gateway) because ``fan_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("FAN_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _fan_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _fan_path_argv(resolved_env_bin)
        return _module_fan_argv()

    fan_bin = _safe_which_no_cwd("fan") if _IS_WINDOWS else shutil.which("fan")
    if fan_bin:
        return _fan_path_argv(fan_bin)
    return _module_fan_argv()


def _kanban_worker_skill_available(fan_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker only
    when that skill resolves in the current Fan home. Preloading a missing
    skill is fatal at CLI startup (``ValueError: Unknown skill(s):
    kanban-worker``), aborting the worker before the agent loop runs. Gate
    the flag on actual resolvability; the kanban lifecycle contract is still
    injected via ``KANBAN_GUIDANCE``, so omitting the flag only drops the
    supplementary pattern library.
    """
    from pathlib import Path as _Path

    # An unset FAN_HOME means the worker falls back to the default root
    # home (``~/.fan``), which ships the bundled skill.
    base = _Path(fan_home) if fan_home else (_Path.home() / ".fan")
    skills_root = base / "skills"
    if not skills_root.is_dir():
        return False
    # Canonical bundled location first (cheap), then a bounded scan for
    # installs that have it nested elsewhere.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``fan kanban worker-run -q ...`` worker subprocess.
    ...
    assignee_arg = _canonical_assignee(task.assignee) or "default"
    prompt = f"work kanban task {task.id}"
    # The worker is model-driving and therefore needs the active provider
    # allowlist, but it must not inherit tool/skill, auxiliary-model,
    # Bitwarden, desktop-runtime, or Fan control-plane credentials.
    from tools.environments.local import fan_subprocess_env

    env = fan_subprocess_env(
        inherit_provider_credentials=True,
        # Kanban workers are full Fan Browser Agents. When dispatched from the
        # desktop gateway they must reuse its authenticated loopback browser;
        # no other internal capability is inherited.
        inherit_browser_runtime_capability=True,
    )

    if task.tenant:
        env["FAN_TENANT"] = task.tenant
    env["FAN_KANBAN_TASK"] = task.id
    env["FAN_KANBAN_WORKSPACE"] = workspace
    if workspace and os.path.isabs(workspace) and os.path.isdir(workspace):
        # file_tools and context-file discovery prefer TERMINAL_CWD over the
        # subprocess cwd. Never inherit the dispatcher's workspace into a
        # worker assigned to a different checkout.
        env["TERMINAL_CWD"] = workspace
    env["FAN_KANBAN_AUTHOR"] = assignee_arg
    if task.branch_name:
        env["FAN_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["FAN_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["FAN_KANBAN_CLAIM_LOCK"] = task.claim_lock
    if task.goal_mode:
        env["FAN_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["FAN_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    env["FAN_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["FAN_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    env["FAN_KANBAN_BOARD"] = _normalize_board_slug(board) or get_current_board()

    cmd = [*_resolve_fan_argv(), "--accept-hooks"]
    if _kanban_worker_skill_available(env.get("FAN_HOME")):
    ...
    cmd.extend(["kanban", "worker-run", "-q", prompt])
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`fan` executable not found on PATH. "
            "Install Fan Agent or activate its venv before running the kanban dispatcher."
        )
    return proc.pid
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``fan kanban daemon`` is systemd-friendly.
...
def _relative_age(timestamp: Optional[int], now: Optional[int] = None) -> str:
    """Render a compact age marker for worker handoffs and history."""
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown:
        ...
        for offset, run in enumerate(shown):
            ...
            assignee = run.assignee or "(unknown)"
            outcome = run.outcome or run.status
            lines.append(f"### Attempt {idx} — {outcome} ({assignee}, {ts})")
    ...
    if parent_ids:
        ...
        for pid in parent_ids:
            ...
            if not wrote_header:
                ...
                lines.append(
                    "_Parent handoffs are point-in-time snapshots. If a result "
                    "is not recent and affects current work, verify it against "
                    "the live source before treating it as current._"
                )
                wrote_header = True
            completed_at = (
                run.ended_at
            ...
            age = _relative_age(completed_at, context_now)
    ...
    if task.assignee:
        ...
        if role_rows:
            ...
            for row in role_rows:
                ...
                age = _relative_age(row["ended_at"], context_now)
                timestamp = f"{ts}, {age}" if age else ts
                s = (row["summary"] or "").strip().splitlines()
                first = s[0][:200] if s else "(no summary)"
                lines.append(f"- {row['id']} — {row['title']} ({timestamp}): {first}")
            lines.append("")
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            age = _relative_age(c.created_at, context_now)
            timestamp = f"{ts}, {age}" if age else ts
            # Render author with explicit "comment from worker" framing so
            # operator-controlled assignee names like "fan-system"
            ...
            lines.append(f"comment from worker `{safe_author}` at {timestamp}:")
...
# ---------------------------------------------------------------------------
# Local desktop notification subscriptions
# ---------------------------------------------------------------------------
...
def add_notify_sub(
    ...
    *,
    ...
    notifier_assignee: Optional[str] = None,
) -> None:
    """Register a local desktop conversation for task lifecycle updates."""
    now = int(time.time())
    with write_txn(conn):
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (task_id, platform, chat_id, thread_id or "", user_id, notifier_assignee, now),
        )
        if notifier_assignee:
            ...
            conn.execute(
                ...
                """,
                (notifier_assignee, task_id, platform, chat_id, thread_id or ""),
            )
...
def list_notify_subs(
    ...
    for row in rows:
        item = dict(row)
        item["notifier_assignee"] = item.pop("notifier_profile", None)
        result.append(item)
    return result
...
def retarget_notify_subs(
    conn: sqlite3.Connection,
    *,
    old_chat_id: str,
    new_chat_id: str,
    platforms: Iterable[str] = ("desktop", "tui"),
) -> int:
    """Move persisted desktop subscriptions across a session continuation.

    Context compression rotates the durable conversation id.  Notification
    subscriptions must follow that continuation or an event created before
    compression would remain addressed to the ended parent session. Existing
    destination rows are merged with a monotonic cursor so the operation is
    idempotent and never replays an already acknowledged event.
    """
    old_chat_id = str(old_chat_id or "")
    new_chat_id = str(new_chat_id or "")
    platform_list = [str(p) for p in platforms if str(p)]
    ...
    with write_txn(conn):
        rows = conn.execute(
            "SELECT * FROM kanban_notify_subs WHERE chat_id = ? "
            f"AND platform IN ({placeholders})",
            (old_chat_id, *platform_list),
        ).fetchall()
        for row in rows:
            ...
            destination = conn.execute(
                ...
                "AND platform = ? AND chat_id = ? AND thread_id = ?",
                (task_id, platform, new_chat_id, thread_id),
            ).fetchone()
            if destination is None:
                conn.execute(
                    "UPDATE kanban_notify_subs SET chat_id = ? WHERE task_id = ? "
                    "AND platform = ? AND chat_id = ? AND thread_id = ?",
                    (new_chat_id, task_id, platform, old_chat_id, thread_id),
                )
            else:
                conn.execute(
                    ...
                    "AND thread_id = ?",
                    (
                        max(int(destination["last_event_id"]), int(row["last_event_id"])),
                        min(int(destination["created_at"]), int(row["created_at"])),
                        row["user_id"],
                ...
                conn.execute(
                    "DELETE FROM kanban_notify_subs WHERE task_id = ? "
                    "AND platform = ? AND chat_id = ? AND thread_id = ?",
                    (task_id, platform, old_chat_id, thread_id),
                )
            moved += 1
    return moved
...
def unseen_events_for_sub(
    ...
    *,
    ...
    the desktop has successfully delivered the notifications.
    """
    row = conn.execute(
    ...
    *,
    ...
    with write_txn(conn):
        conn.execute(
            ...
            "WHERE task_id = ? AND platform = ? AND chat_id = ? AND thread_id = ?",
            (
                int(new_cursor), int(new_cursor), task_id,
...
# ---------------------------------------------------------------------------
# Assignee enumeration (configured lanes + per-assignee board stats)
# ---------------------------------------------------------------------------

def list_configured_assignees() -> list[str]:
    """Return assignee lane names configured for the current Fan home."""
    ...
    def add(value: object) -> None:
        name = _canonical_assignee(str(value)) if value is not None else None
        if name:
            names.add(name)

    add(kanban_cfg.get("default_assignee"))
    add(kanban_cfg.get("orchestrator_assignee"))
    lanes = kanban_cfg.get("assignee_lanes") or []
    if isinstance(lanes, list):
        for lane in lanes:
            if isinstance(lane, dict):
                add(lane.get("name") or lane.get("id"))
            else:
                add(lane)
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    """Return every assignee name known to the board or configuration.

    Each entry is ``{"name": str, "configured": bool, "counts": {status: n}}``.
    A name is included when it's a configured assignee lane OR when
    any non-archived task has it as the assignee. Used by:

    - ``fan kanban assignees`` for the terminal.
    - The dashboard assignee dropdown (so a fresh lane appears in
      the picker even before it's been given any task).
    - Router heuristics ("who's overloaded?") without scanning
      the whole board.
    """
    configured = set(list_configured_assignees())
    ...
    names = sorted(configured | set(counts.keys()))
    return [
        {
            "name": name,
