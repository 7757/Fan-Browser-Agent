...
Tracks per-skill usage metadata in a sidecar JSON file (~/.fan/skills/.usage.json)
...
# regardless of curator.prune_builtins, pin state, or LLM judgment. `plan` powers
# the /plan slash-command flow; silently archiving it turns the command into
# "Unknown command" with no signal. Protection is by skill name (frontmatter
# name:). Not a substitute for curator.prune_builtins: false (which exempts ALL
# built-ins). (upstream 8e223b36)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.fan/skills/.bundled_manifest (format: "name:hash" per line).
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.fan/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``fan update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Fan metadata, VCS, virtualenv/dependency, and cache dirs
        if is_excluded_skill_path(skill_md):
            continue
        # An external dir may be mounted below the local skills root.  It is
        # discoverable, but never becomes curator-owned merely by appearing in
        # this walk.
        ...
        # Protected built-ins are never curation candidates (transition walk + LLM pass).
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.fan/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``fan curator
    list-archived`` to help users pass a name to ``fan curator restore``.
    """
...
def is_agent_created(skill_name: str) -> bool:
    """Whether a name resolves to a curator-manageable local skill."""
    off_limits = _read_bundled_manifest_names()
...
def is_curation_eligible(skill_name: str, skill_path: Optional[Path] = None) -> bool:
    ...
    Agent-created local skills are eligible. Bundled built-ins become eligible
    only when ``curator.prune_builtins`` is enabled. Skills in
    ``skills.external_dirs`` are never eligible because they have an external
    owner. Protected built-ins are never eligible.
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.fan/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.fan/skills/. Restores to the flat
    ...
    # Try exact name match first, then only the timestamped duplicate shape
    # archive_skill() itself creates on a collision.
    ...
    if not candidates:
        ...
        def _is_timestamped_copy(path: Path) -> bool:
            if not path.is_dir() or not path.name.startswith(prefix):
                return False
            suffix = path.name[len(prefix):]
            if len(suffix) != 14 or not suffix.isascii() or not suffix.isdigit():
                return False
            return _read_skill_name(path / "SKILL.md", fallback="") == skill_name

        candidates = sorted(
            [p for p in archive_root.rglob("*") if _is_timestamped_copy(p)],
            key=lambda p: p.name,
            reverse=True,
        )
    if not candidates:
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.fan/skills/<skill>/SKILL.md) and category-nested
    (~/.fan/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
...
def _find_external_skill_dir(skill_name: str) -> Optional[Path]:
    """Locate a skill by frontmatter name under configured external roots."""
    from agent.skill_utils import get_external_skills_dirs

    for base in get_external_skills_dirs():
        if not base.exists():
            continue
        for skill_md in base.rglob("SKILL.md"):
            if is_excluded_skill_path(skill_md):
...
def provenance(skill_name: str) -> str:
    """Classify a skill's origin as bundled or locally authored."""
    if is_bundled(skill_name):
        return "bundled"
    return "agent"
