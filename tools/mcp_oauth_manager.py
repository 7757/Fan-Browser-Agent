...
`mcp_tool.py`, and `fan_cli/mcp_config.py`. This module is the ONLY place
...
def _same_endpoint(a: str, b: str) -> bool:
    """Compare token endpoints while ignoring query strings and fragments."""
...
# ---------------------------------------------------------------------------
# FanMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_fan_provider_class() -> Optional[type]:
    ...
    class FanMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            # A config-supplied client ID is owned by the operator.  An
            # invalid_client response there means config must be fixed, not
            # that Fan should delete state and retry dynamic registration.
            self._fan_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`FanTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, FanTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import FanTokenStorage
                        if isinstance(storage, FanTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import FanTokenStorage
            if not isinstance(storage, FanTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            """Reset a dead dynamic client registration after ``invalid_client``.

            The action is deliberately narrow: only a 400/401 from the
            discovered token endpoint with the OAuth error code can prove the
            cached registration is stale.  It never applies to a configured
            pre-registered client, unrelated MCP responses, or
            ``invalid_client_metadata`` registration validation errors.
            """
            try:
                if self._fan_preregistered:
                    return
                ...
                token_endpoint = (
                    ...
                    and getattr(metadata, "token_endpoint", None)
                    else ""
                )
                request = getattr(response, "request", None)
                request_url = str(request.url) if request is not None else ""
                if not token_endpoint or not request_url:
                    return
                ...
                # SDK state must agree with disk: the next auth flow sees no
                # client and re-runs dynamic registration.
                self.context.client_info = None
                self._initialized = False
            except Exception as exc:
                logger.debug(
                    "MCP OAuth '%s': poisoned-client recovery skipped: %s",
                    self._server_name,
                    exc,
                )
    ...
    return FanMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_FAN_PROVIDER_CLS: Optional[type] = _make_fan_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`FanMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            # Fail fast rather than warn + proceed into a browser OAuth flow
            # under a headless gateway turn.
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no cached "
                f"tokens found. OAuth-protected MCP servers are not supported in this build "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``fan mcp remove <name>`` and (indirectly) by
        ``fan mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
