...
it is automatically downloaded from GitHub releases to $FAN_HOME/bin/tirith.
...
def _get_fan_home() -> str:
    """Return the Fan home directory, respecting FAN_HOME env var."""
    return str(get_fan_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_fan_home(), ".tirith-install-failed")
...
def _fan_bin_dir() -> str:
    """Return $FAN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_fan_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $FAN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_fan_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $FAN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $FAN_HOME/bin/tirith
    ...
    fan_bin = os.path.join(_fan_bin_dir(), "tirith")
    if os.path.isfile(fan_bin) and os.access(fan_bin, os.X_OK):
        _resolved_path = fan_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return fan_bin
