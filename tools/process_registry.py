...
Background processes execute THROUGH the environment interface (local
execution — the remote/container backends were removed).
...
CHECKPOINT_PATH = get_fan_home() / "processes.json"
...
class ProcessSession:
    ...
    host_start_time: Optional[float] = None      # psutil create_time — PID-reuse guard
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        # Track sessions whose completion output was consumed via wait/log.
        # Gateway watchers skip only these; poll is deliberately read-only.
        self._completion_consumed: set = set()
        # A CLI-style inline drain should still deduplicate a completion the
        # same turn already observed via poll, without suppressing the desktop
        # gateway's autonomous notify_on_complete delivery.
    ...
    def _is_host_pid_alive(pid: Optional[int]) -> bool:
        ...
        # ``psutil.pid_exists`` for a cross-platform existence check. (The
        # former ``gateway.status._pid_exists`` helper was removed with the
        # messaging gateway; ``psutil`` is a hard dependency.)
        try:
            import psutil
    ...
    @staticmethod
    def _safe_host_start_time(pid: Optional[int]) -> Optional[float]:
        """Return the kernel-backed creation time for a host PID."""
        if not pid:
            return None
        try:
            import psutil
    ...
    @classmethod
    def _host_pid_is_ours(cls, pid: Optional[int], expected_start: Optional[float]) -> bool:
        """Return whether a live PID still identifies the process Fan spawned."""
        if not cls._is_host_pid_alive(pid):
            return False
        if expected_start is None:
            # Legacy checkpoints did not record an identity baseline.
            return True
        live_start = cls._safe_host_start_time(pid)
        return live_start is not None and abs(live_start - expected_start) < 1e-6

    @staticmethod
    def _proc_alive(proc) -> bool:
        """Return whether a captured psutil process is running and not a zombie."""
        try:
            import psutil
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        try:
            from fan_cli.config import DEFAULT_CONFIG, cfg_get, read_raw_config

            value = cfg_get(read_raw_config(), "terminal", "daemon_term_grace_seconds")
            if value is None:
    ...
    @classmethod
    def _terminate_host_pid(cls, pid: int, expected_start: Optional[float] = None) -> None:
        ...
        if expected_start is not None and not cls._host_pid_is_ours(pid, expected_start):
            logger.warning(
                "Refusing to terminate host pid %d: process identity no longer matches",
                pid,
            )
            return
        ...
        for proc in targets:
            try:
                ...
                proc.kill()
                logger.info(
                    "Escalated to SIGKILL for pid %d after %.1fs grace",
                    proc.pid,
                    grace,
                )
            except (psutil.NoSuchProcess, psutil.AccessDenied, OSError):
    ...
    def spawn_local(
        ...
        Local execution only (remote backends were removed).
    ...
    def is_completion_consumed(self, session_id: str) -> bool:
        """Check if completion output was consumed via wait/read_log."""
    ...
    def _drain_should_skip(
        ...
        session_id: str,
        *,
        include_poll_observed: bool,
    ) -> bool:
        return (
            session_id in self._completion_consumed
            or (
                include_poll_observed
                and session_id in self._poll_observed
            )
        )
    ...
    def drain_notifications(
        self,
        *,
        include_poll_observed: bool = True,
    ) -> "list[tuple[dict, str]]":
        ...
        while not self.completion_queue.empty():
            ...
            if (
                ...
                and self._drain_should_skip(
                    _evt_sid,
                    include_poll_observed=include_poll_observed,
                )
            ):
                continue
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `fan update` restarting the gateway) is still
        ...
        7 minutes in the original report).
        ...
        try:
            if session._pty:
                ...
                try:
                    ...
                    if session.pid and self._host_pid_is_ours(
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    """
    evt = _redact_process_result(evt)
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Fan'}"
    elif _reason == "lost":
...
def _redact_process_result(result: dict) -> dict:
    """Redact process output/command before model, UI, or history exposure."""
    if not isinstance(result, dict):
        return result
    from agent.redact import redact_sensitive_text, redact_terminal_output

    redacted = dict(result)
    command = redacted.get("command") or ""
    for field in ("output", "output_preview"):
        value = redacted.get(field)
        if isinstance(value, str) and value:
            redacted[field] = redact_terminal_output(value, command)
    if isinstance(command, str) and command:
        redacted["command"] = redact_sensitive_text(command, code_file=True)
    processes = redacted.get("processes")
    if isinstance(processes, list):
        redacted["processes"] = [
            _redact_process_result(item) if isinstance(item, dict) else item
