"""Base class for all Fan execution environment backends.
...
# FAN_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("FAN_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {FAN_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_fan_home() / "sandboxes"
...
def _popen_bash(
    ...
    """
    # A desktop-launched shell must not briefly create a visible console window
    # on Windows.  `windows_hide_flags()` is a no-op on other platforms and
    # preserves an explicit backend-specific creationflags choice.
...
def _cwd_marker(session_id: str) -> str:
    return f"__FAN_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Fan backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/fan-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/fan-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # Use the backend-aware helper: LocalEnvironment translates native
        # Windows paths to Git-Bash form before this bootstrap `cd` runs.
        ...
        # caused ``C:/Users/.../fan-snap-*.sh: No such file or directory``
        # errors on Windows, leaking via stderr (merged into stdout on Linux
        # backends) into every terminal-tool response.
        _quoted_snap = shlex.quote(self._snapshot_path)
        # Keep the temporary file in the same directory so ``mv`` is atomic.
        # ``$BASHPID`` is the actual concurrent subshell PID; ``$$`` can be
        # inherited by backgrounded subshells and make two writers collide.
        _quoted_snap_tmp = shlex.quote(self._snapshot_path + ".tmp.") + "$BASHPID"
        _quoted_cwd_file = shlex.quote(self._cwd_file)
        bootstrap = (
            # Snapshot/cwd metadata can contain exported credentials. Clamp
            # only Fan-owned writes; user commands retain their own umask.
            f"umask 077\n"
            f"export -p > {_quoted_snap_tmp}\n"
            f"declare -f | grep -vE '^_[^_]' >> {_quoted_snap_tmp}\n"
            f"alias -p >> {_quoted_snap_tmp}\n"
            f"echo 'shopt -s expand_aliases' >> {_quoted_snap_tmp}\n"
            f"echo 'set +e' >> {_quoted_snap_tmp}\n"
            f"echo 'set +u' >> {_quoted_snap_tmp}\n"
            f"mv -f {_quoted_snap_tmp} {_quoted_snap} || rm -f {_quoted_snap_tmp}\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("__fan_ec=$?")
        # Applied after the user command so Fan does not alter files the user
        # intentionally creates. It protects only snapshot/cwd metadata below.
        parts.append("umask 077")

        # Re-dump env vars with atomic replacement so concurrent commands
        # cannot source a truncated snapshot while another command updates it.
        if self._snapshot_ready:
            parts.append(
                f"{{ export -p > {_quoted_snap_tmp} && mv -f {_quoted_snap_tmp} {_quoted_snap}; }} "
                f"2>/dev/null || rm -f {_quoted_snap_tmp} 2>/dev/null || true"
            )
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"FAN_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via FAN_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __FAN_CWD_{session}__ marker from stdout output.
    ...
    def _before_execute(self) -> None:
        ...
        Local executes on the host filesystem directly, so the base
        implementation is a no-op; alternative backends may override.
        """
        pass
    ...
    def execute(
        ...
        *,
        ...
        # Callers that already produce shell-safe wrappers pass
