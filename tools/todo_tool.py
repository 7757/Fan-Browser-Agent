...
# Bounds on persisted todo state. The list is re-read after every context
# compression (format_for_injection), so unbounded item content or count
# defeats the compression it rides through. These caps keep a single oversized
# item (model-authored or replayed from caller-supplied history) from inflating
# the re-injection block. Generous relative to real plans.
MAX_TODO_CONTENT_CHARS = 4000
MAX_TODO_ITEMS = 256
# Upper bound on a single todo tool-result payload accepted during history
# hydration. Caller-supplied conversation history is replayed to rebuild the
# store, so reject oversized results before parsing and re-injecting them.
...
def todo_tool(
    ...
    if todos is not None:
        # Some providers serialize tool-array arguments twice. Accept a valid
        # JSON list, but fail clearly instead of letting a string/non-dict
        # value crash TodoStore while a turn is in progress.
