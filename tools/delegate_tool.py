...
# Must match fan_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# Nested delegation is granted only by role='orchestrator'; model-emitted
# delegation never selects a child toolset. Each child inherits the parent's
# effective toolsets, with the leaf safety exclusions applied below.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``fan-cli`` (which bundles
    all core tools), the child may request individual toolsets such as ``browser``
    or ``terminal``.  A simple name-based intersection would reject them
    because ``"browser" != "fan-cli"``.
...
# Hard per-summary ceiling layered on top of the dynamic parent-headroom
# budget below.  A child that ignores its concise-summary instruction must not
# be able to consume the parent's entire context window.
...
#     operation (terminal command, browser action, large file read)
...
def _build_child_system_prompt(
    ...
    *,
    ...
    parts.append(
        ...
        "Keep your final summary tight: lead with outcomes, prefer bullets "
        "over process narration, and do not replay the whole investigation. "
        "Your response is returned to the parent agent as a summary and "
        "overlong summaries crowd out its context window."
    )
...
def _emit_parent_console(parent_agent, line: str) -> None:
    """Emit delegation progress without corrupting an ACP stdio stream."""
...
def _inherit_parent_base_url(parent_agent, fallback_base_url: Optional[str]) -> Optional[str]:
    """Inherit the endpoint actually used by the parent's live client.

    A parent's public ``base_url`` can be stale after runtime routing changes;
    subagents must follow the active client or they can retry against an old
    endpoint with credentials for the new one.
    """
...
def _build_child_agent(
    ...
    # ACP transport overrides from trusted delegation configuration.
    ...
    routing subagents to a different provider:model pair (for example, a
    cheap/fast custom model while the parent uses the main provider).
    """
    from run_agent import AIAgent
    import uuid as _uuid

    # The parent capability boundary is authoritative. Ignore any legacy or
    # direct caller-supplied selection so a model cannot narrow/choose a
    # subagent's tools and accidentally bypass the parent's policy setup.
    toolsets = None
    ...
    # so disabled tools don't leak to subagents.
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. fan-cli) so that individual
        # toolset names (e.g. browser, terminal) are recognised during intersection.
    ...
    # Extract the parent's API key so subagents inherit provider credentials.
    ...
    # parent-level OpenRouter filters (e.g. `only=["Google"]`) would
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.fan/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    """Persist a full over-budget child summary for later inspection."""
    try:
        from fan_constants import get_fan_home

        cache_dir = get_fan_home() / "cache" / "delegation"
        cache_dir.mkdir(parents=True, exist_ok=True)
        path = cache_dir / f"subagent-summary-{task_index}-{time.time_ns()}.txt"
...
def _trim_summary_with_footer(
    ...
    """Keep the beginning and outcome-heavy end of an overlong summary."""
    ...
    # Prefer clean line boundaries without throwing away a disproportionate
    # amount of the selected text.
    newline = head.rfind("\n")
    ...
    footer = [
        ...
        (
            ...
            f"of {original_len:,} total to protect the parent context window."
        ),
    ]
    if spill_path:
        ...
        footer.extend(
            [
                f"Full subagent output saved to: {spill_path}",
                (
        ...
        footer.append("Full output could not be saved; only this head and tail were retained.")
    footer.append("─────────────────────────────────────")
    return (
        head
...
def _parent_summary_char_budget(parent_agent, n_summaries: int) -> Optional[int]:
    """Calculate each child's fair share of the parent's remaining context."""
    try:
        ...
        if not isinstance(used_tokens, (int, float)) or used_tokens < 0:
            used_tokens = 0
        reserved_tokens = getattr(compressor, "max_tokens", 0) or 0
        headroom_tokens = context_length - int(used_tokens) - int(reserved_tokens)
        if headroom_tokens <= 0:
            return _MIN_SUMMARY_CHARS
        batch_token_budget = int(headroom_tokens * _SUMMARY_HEADROOM_FRACTION)
        return max(
            _MIN_SUMMARY_CHARS,
            (batch_token_budget // max(1, n_summaries)) * 4,
        )
    except Exception:
...
def _apply_summary_budget(results: List[Dict[str, Any]], parent_agent) -> None:
    """Trim child summaries before a fan-out batch can overflow its parent."""
    summaries = [
        entry
    ...
    cfg = _load_config()
    try:
        static_cap = int(cfg.get("max_summary_chars", DEFAULT_MAX_SUMMARY_CHARS))
    except (TypeError, ValueError):
        static_cap = DEFAULT_MAX_SUMMARY_CHARS
    dynamic_cap = _parent_summary_char_budget(parent_agent, len(summaries))
    caps = [cap for cap in (static_cap, dynamic_cap) if cap and cap > 0]
    if not caps:
        return
    cap = min(caps)
    ...
    for entry in summaries:
        ...
        logger.debug(
            "[subagent-%s] summary trimmed %d to ~%d chars (spill=%s)",
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    # Background delegations detach from the parent and outlive its turn; the
    # heartbeat must NOT keep touching the (already-returned) parent's activity
    # timestamp in that case (see _heartbeat_loop).
    _is_background = bool(_kwargs.get("background", False))
    ...
    def _heartbeat_loop():
        while not _heartbeat_stop.wait(_HEARTBEAT_INTERVAL):
            # A background delegation has already returned the parent's turn, so
            # touching the parent's activity timestamp + writing a stale
            # "subagent working" desc would suppress any future inactivity-timeout
            # consumer that reuses a long-lived parent agent. Only sync delegation
            # (parent genuinely blocked) needs the parent kept "alive".
            if _is_background or parent_agent is None:
                continue
...
def delegate_task(
    ...
    if n_tasks == 1:
        ...
        # tools/async_delegation.py), drained by the gateway notification
        # poller / CLI process loop. Batch async is rejected earlier.
        if background:
            ...
            def _detach_from_parent():
                # Detach the child from the parent's interrupt-propagation list.
                # _build_child_agent registered it there (correct for sync
                # children, which block the parent's turn), but a dispatched
                # BACKGROUND child must survive parent-turn interrupts (Ctrl+C,
                # mid-turn steering), cache evicts (release_clients), and session
                # close (/new) — otherwise the detached subagent dies with
                # whatever the parent was doing when it was dispatched. Its
                # lifecycle is then owned by the async-delegation registry
                # (interrupt_fn below) and _run_single_child's finally block
                # (which closes its resources). Called ONLY after a successful
                # dispatch: detaching before would orphan the child's already-
                # built client pool on a rejected dispatch (runner never runs,
                # so _run_single_child's finally never closes it).
                if hasattr(parent_agent, "_active_children"):
                    try:
            ...
            def _async_runner(_child=child, _goal=_t["goal"]):
                return _run_single_child(0, _goal, _child, parent_agent, background=True)
            ...
            if dispatch.get("status") == "dispatched":
                # Hand the child's lifecycle to the async registry only now that
                # dispatch succeeded (see _detach_from_parent).
                _detach_from_parent()
                return json.dumps(
                    {
                        "status": "dispatched",
            ...
            # Rejected (at capacity or schedule failure). The child was built
            # eagerly (its httpx/OpenAI client pool is already open) but never
            # ran, so close it here instead of leaking until session end. It was
            # NOT detached (we only detach on success), so a later parent.close()
            # is a harmless idempotent second close. Surface a tool error so the
            # model can fall back to synchronous delegation.
            try:
                if hasattr(child, "close"):
                    child.close()
            except Exception:
                logger.debug("Failed to close rejected background delegation child", exc_info=True)
            return tool_error(
    ...
    # A batch can return several independently long summaries at once. Bound
    # their combined footprint before any memory or parent-context consumer
    # sees them; full output remains available from the spill file.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if configured_base_url:
        ...
        # Use the same URL-based mode detector as the main runtime.
        ...
        # Explicit delegation.api_mode in config always wins. Invalid values
        # fail closed instead of silently falling back to another transport.
        parsed_api_mode = _parse_api_mode(configured_api_mode)
        if parsed_api_mode:
            api_mode = parsed_api_mode
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers are the API-key and custom endpoints configured for this build."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate API key or endpoint in Fan configuration."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (fan_cli/config.py load_config()) so that
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "- Tasks needing user interaction -> subagents cannot use collect\n"
        ...
        "- Model routing is configuration-level, not selectable per call. "
        "Subagents inherit the parent's model/provider and fallback chain unless "
        "the user has configured delegation.model or delegation.provider globally.\n"
        ...
        "delegate_task, collect, memory, execute_code.\n"
        "- Orchestrator subagents (role='orchestrator') retain "
        "delegate_task so they can spawn their own workers, but still "
        "cannot use collect, memory, or execute_code. "
        f"Orchestrators are bounded by max_spawn_depth={max_depth} for this "
        f"user and can be disabled globally via "
        "delegation.orchestrator_enabled=false.\n"
        "- A subagent model is not selectable per call. Children inherit "
        "the parent model and fallback chain unless all subagents are pinned "
        "through delegation.provider / delegation.model in config.yaml.\n"
...
def _strip_model_hidden_task_fields(tasks: Any) -> Any:
    """Remove transport overrides from model-originated delegation tasks.

    ACP processes are a trusted configuration decision.  A tool call can set
    arbitrary executable/argument values even when an older cached schema
    still exposes these fields, so strip them at every model-facing boundary.
    """
