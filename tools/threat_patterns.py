...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    (r'unset\s+\w*(?:CODEX|FAN|AGENT|OPENAI)\w*', "env_var_unset_agent", "context"),

    # ── Known C2 / red-team framework names (near-zero false positive
    #    outside security research; warn-only by default) ─────────────
    # Every token here must be a distinctive tool brand. Common words can
    # appear in a legitimate AGENTS.md or SOUL.md and must not block the whole
    # context file ("praxis" is a normal word / valid agent name, not a C2
    # framework signal).
    ...
    (r'\$HOME/\.fan/\.env|\~/\.fan/\.env', "fan_env", "strict"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*(?:AGENTS\.md|CLAUDE\.md|\.cursorrules|\.clinerules)', "agent_config_mod", "strict"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*\.fan/(config\.yaml|SOUL\.md)', "fan_config_mod", "strict"),
...
def scan_for_threats(content: str, scope: str = "context") -> List[str]:
    ...
    - ``"strict"`` (broad): adds persistence / SSH backdoor / exfil-URL
      patterns — appropriate for user-mediated local writes where false
      positives can be resolved interactively.
    ...
    # Compatibility/full-width Unicode must not let a token such as
    # ``ｃａｔ`` bypass the text-only threat rules.  Check invisibles above on
    # the original input because normalization can erase some code points.
...
def first_threat_message(content: str, scope: str = "strict") -> Optional[str]:
    ...
    (memory or skill writes) where the caller just needs a
    yes/no + a message.
    """
