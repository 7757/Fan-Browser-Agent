...
# A completion can already be in the shared queue when its originating
# conversation is replaced or closed. Keep a small bounded tombstone set so a
# late poller can still reject that event after its status record is pruned.
_cancelled_delivery_ids: set[str] = set()
_cancelled_delivery_order: deque[str] = deque()
_MAX_CANCELLED_DELIVERY_IDS = 256
...
def _mark_delivery_cancelled_locked(record: Dict[str, Any]) -> None:
    """Prevent a delegation result from re-entering a retired session.

    Caller must hold ``_records_lock``.
    """
    record["delivery_cancelled"] = True
    delegation_id = str(record.get("delegation_id") or "")
    if not delegation_id or delegation_id in _cancelled_delivery_ids:
        return
    if len(_cancelled_delivery_order) >= _MAX_CANCELLED_DELIVERY_IDS:
        expired = _cancelled_delivery_order.popleft()
        _cancelled_delivery_ids.discard(expired)
    _cancelled_delivery_order.append(delegation_id)
    _cancelled_delivery_ids.add(delegation_id)
...
def _finalize(delegation_id: str, result: Dict[str, Any], status: str) -> None:
    ...
    with _records_lock:
        ...
        if record is None:
            return
        delivery_cancelled = bool(record.get("delivery_cancelled"))
    ...
    if delivery_cancelled:
        logger.info(
            "Async delegation %s completed after its session closed; suppressing result",
            delegation_id,
        )
        return
...
def is_async_delegation_delivery_cancelled(delegation_id: str) -> bool:
    """Return whether a queued completion must not be delivered anywhere."""
    delegation_id = str(delegation_id or "")
    if not delegation_id:
        return True
    with _records_lock:
        record = _records.get(delegation_id)
        return delegation_id in _cancelled_delivery_ids or bool(
            record and record.get("delivery_cancelled")
        )


def cancel_session_delegations(session_key: str, reason: str = "session closed") -> int:
    """Interrupt and suppress every async delegation owned by ``session_key``.

    A background child may take a moment to observe its interrupt signal.  Its
    eventual completion must still be discarded: a result from a closed,
    compressed, or superseded conversation must never become a new turn in a
    ...
    with _records_lock:
        for record in _records.values():
            if str(record.get("session_key") or "") != session_key:
                continue
            already_cancelled = bool(record.get("delivery_cancelled"))
            _mark_delivery_cancelled_locked(record)
            if record.get("status") != "running" or already_cancelled:
                continue
            cancelled += 1
            interrupt_fn = record.get("interrupt_fn")
            if callable(interrupt_fn):
                callbacks.append(interrupt_fn)

    for interrupt_fn in callbacks:
        try:
            interrupt_fn()
        except Exception:
            logger.debug("Failed to interrupt session-scoped async delegation", exc_info=True)

    if cancelled:
        logger.info("Cancelled %d async delegation(s): %s", cancelled, reason)
    return cancelled
