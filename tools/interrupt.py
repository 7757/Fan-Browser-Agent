...
# Opt-in debug tracing — pairs with FAN_DEBUG_INTERRUPT in
...
def clear_current_thread_interrupt() -> None:
    """Clear an interrupt bit on the current execution thread.

    Approval can block while a user decides whether a dangerous command or
    script may run. If a stale Stop lands during that wait, the approved
    execution must start from a clean slate; a later, genuine Stop will set
    the bit again and is still observed by the child-process polling loop.
    """
    set_interrupt(False)
