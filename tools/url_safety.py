...
Limitations (documented, not fixable at pre-flight level):
  ...
  - Redirect-based bypass is mitigated by httpx event hooks that re-validate
    each redirect target in network-enabled tools.
"""
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``FAN_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("FAN_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``FAN_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
async def async_is_safe_url(url: str) -> bool:
    ...
    vision download hooks) instead of ``is_safe_url``.
    """
