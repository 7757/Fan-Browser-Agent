...
machine as Fan itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Fan ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _run(cmd: list[str], timeout: float = 3.0) -> tuple[int, str, str]:
    ...
    ``subprocess.run`` performs an unbounded second pipe read after a timeout
    on Windows. A descendant that inherited those pipes can therefore hang
    the probe indefinitely, so timeout cleanup here never reads them again.
    """
    proc = None
    try:
...
def _probe_worker(generation: int) -> None:
    """Compute and publish the probe result without blocking prompt callers."""
    global _CACHED_LINE
    try:
...
def _ensure_probe_started() -> None:
    """Start exactly one probe worker for the current cache generation."""
