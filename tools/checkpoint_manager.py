...
-----------------------------------------------------------------------------
    ...
    ~/.fan/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/fan/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_fan_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/fan"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Fan convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("FAN_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Fan infrastructure — it must NOT inherit
...
def _repair_bare_repo_dirs(store: Path) -> None:
    """Recreate empty bare-repository directories removed by ``git gc``."""
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared by deleting ~/.fan/checkpoints/legacy/.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear by deleting ~/.fan/checkpoints/legacy/ when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "fan@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Fan Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``FAN_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at FAN_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "FAN_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `fan checkpoints` CLI
# ---------------------------------------------------------------------------
