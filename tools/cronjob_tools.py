"""
Cron job management tools for Fan Agent.
...
#      saying "cat ~/.fan/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `fan-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.fan/.env`, which tripped `read_secrets` and silently
...
# Keep runtime cron scanning on the same canonical set as skill installation;
# a local subset previously omitted directional isolates and invisible math
# operators that can split an injection directive without visible characters.
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via fan model.
...
def _validate_cron_base_url(
    ...
    """Reject an endpoint override that could exfiltrate a stored credential.

    A cron job is model-callable.  Without this boundary, it can select a
    named provider (and its configured API key) then replace its endpoint with
    an attacker-controlled host.  An override is safe only for bare custom
    BYOK routing, a named custom provider's configured host, or a built-in
    provider's own host.
    """
    ...
    try:
        from fan_cli.auth import PROVIDER_REGISTRY, resolve_provider
    ...
    # Bare custom resolves the endpoint/key from that explicit BYOK route; it
    # does not reuse a named provider's stored credential for this override.
    if prov.lower() == "custom":
        return None

    try:
        custom_provider = _get_named_custom_provider(prov)
    except Exception:
        custom_provider = None
    if custom_provider:
        configured_host = base_url_hostname(
            str(custom_provider.get("base_url") or "")
        )
        if configured_host and base_url_host_matches(bu, configured_host):
            return None
        return (
            ...
            f"configured endpoint ({configured_host or 'unknown'})."
        )

    try:
        resolved = resolve_provider(prov)
    except Exception:
        resolved = prov
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within FAN_HOME/scripts/.
    ...
    # Only relative paths within ~/.fan/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.fan/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.fan/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_fan_home() / "scripts"
...
def _execute_job_now(job: Dict[str, Any]) -> Dict[str, Any]:
    """Make a job due, claim it atomically, then execute it synchronously."""
    job_id = job["id"]
    try:
        ...
        claimed = claim_job_for_immediate_fire(job_id)
        if claimed is None:
            return {
                "claimed": False,
                "success": False,
                "error": "Job is already being fired or changed; not run again.",
            }

        heartbeat_stop, heartbeat_thread = _start_claim_heartbeat(claimed)
        try:
            processed = run_one_job(claimed)
        finally:
            _stop_claim_heartbeat(heartbeat_stop, heartbeat_thread)

        refreshed = get_job(job_id) or {}
        succeeded = refreshed.get("last_status") == "ok"
        return {
            "claimed": True,
            "success": bool(processed and succeeded),
            "error": refreshed.get("last_error"),
        }
    except Exception as exc:
        logger.error(
            "Failed to execute cron job %s immediately: %s",
            job_id,
            exc,
            exc_info=True,
        )
...
def cronjob(
    ...
    try:
        ...
        if normalized in {"run", "run_now", "trigger"}:
            execution = _execute_job_now(job)
            result = _format_job(get_job(job_id) or {"id": job_id})
            result["executed"] = execution.get("claimed", False)
            result["execution_success"] = execution.get("success", False)
            if not execution.get("claimed", False):
                result["execution_skipped"] = execution.get("error") or (
        ...
        if normalized == "update":
            ...
            # Re-validate the effective stored pair for every update. This
            # catches legacy/directly-written unsafe jobs even when the update
            # changes an unrelated field, while allowing remediation in the
            # same request by clearing or replacing the override.
            eff_provider = (
...
The final response and execution details are saved in local Cron history. Cron
jobs run autonomously with no user present, so they cannot ask questions or
        ...
        "properties": {
            ...
            "model": {
                ...
                "properties": {
                    "provider": {
                        "type": "string",
                        "description": "Provider name (e.g. 'alibaba' or 'custom:<name>' for a provider defined in custom_providers config — always include the ':<name>' suffix, never pass the bare 'custom'). Omit to use and pin the current provider."
                    },
                    "model": {
                        "type": "string",
                        "description": "Model name (e.g. 'qwen3-max')"
                    }
                },
                "required": ["model"]
            },
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is saved verbatim in local Cron history. Relative paths resolve under {display_fan_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
                "description": (
                    ...
                    "Set True to skip the LLM entirely: the scheduler just runs ``script`` on schedule and saves its stdout verbatim. No tokens, no agent loop, no model override honoured. "
                    "\n\n"
                    "REQUIREMENTS when True: ``script`` MUST be set (``prompt`` and ``skills`` are ignored). "
                    "\n\n"
                    "RESULT SEMANTICS when True: "
                    "(a) non-empty stdout is saved verbatim in local Cron history; "
                    "(b) empty stdout produces no report content; "
                    "(c) non-zero exit / timeout is saved as an error so a broken watchdog cannot fail silently. "
                    "\n\n"
            ...
            "enabled_toolsets": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Optional list of toolset names to restrict the job's agent to (e.g. [\"browser\", \"terminal\", \"file\", \"delegation\"]). When set, only tools from these toolsets are loaded, significantly reducing input token overhead. When omitted, all default tools are loaded. Infer from the job's prompt — e.g. use \"browser\" for web interaction, \"terminal\" if it runs scripts, \"file\" if it reads files, \"delegation\" if it calls delegate_task. On update, pass an empty array to clear."
            },
            "workdir": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    Available in interactive CLI and desktop sessions.
    The cron system is internal (JSON file-based scheduler ticked by the desktop backend),
    ...
    return (
        env_var_enabled("FAN_INTERACTIVE")
        or env_var_enabled("FAN_GATEWAY_SESSION")
        or env_var_enabled("FAN_EXEC_ASK")
    )
