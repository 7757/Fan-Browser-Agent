...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__FAN_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__FAN_FENCE_" in line or "\x1b]" in line
...
class SearchResult:
    ...
    # Content-mode search results repeat the same keys and full path for every
    # match.  Grouping becomes worthwhile once there are several matches, but
    # remains opt-in so non-model callers keep the stable structured shape.
    _DENSIFY_MIN_MATCHES: ClassVar[int] = 5

    def _densify_matches(self) -> Optional[str]:
        """Return a compact, path-grouped representation of content matches."""
        ...
        for match in self.matches:
            ...
            # Do not strip the content: search output can contain meaningful
            # indentation or trailing whitespace and the compact form must be
            # as lossless as the original structured result.
        ...
        if self.matches:
            ...
            if dense is not None:
                result["matches_format"] = (
                    "path-grouped: each file path is followed by indented "
                    "'<line>: <content>' match rows"
                )
...
def _search_stdout_and_limit(result: ExecuteResult) -> tuple[str, Optional[str]]:
    ...
    _exec appends a ``[Command timed out after Ns]`` marker on exit code 124;
    strip it so it isn't parsed as a match, and flag the result as a soft
    truncation (``search_timeout``) so partial results are kept + labelled
    rather than discarded or surfaced as an error.
    """
...
def _lint_yaml_inproc(content: str) -> tuple[bool, str]:
    ...
    This is deliberately syntax-only: ``safe_load`` rejects valid
    multi-document manifests and application-defined tags such as ``!Sub``.
    """
    try:
...
# Structured data formats where invalid syntax always means a corrupt write.
# Python remains a post-write diagnostic because callers may intentionally
# write partial source while editing; other extensions retain existing logic.
...
def _pattern_has_regex_newline(pattern: str) -> bool:
    """Whether a line-oriented content regex tries to cross a newline."""
...
def _maybe_warn_line_oriented_newline_pattern(
    ...
    """Turn an impossible multiline search into an actionable warning."""
    ...
    result.error = None
    result.warning = (
        "0 results found. search_files content search is line-oriented and "
        "does not enable ripgrep -U/--multiline, so `\\n` cannot match line "
        "breaks. Use context=N to inspect neighboring lines, or `\\\\n` to "
        "search for a literal backslash followed by n."
    )
    return result
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        into extra tokens on every single line. An A/B test (2
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        Git Bash consumes backslashes in native Windows drive paths. Reuse the
        local environment's MSYS translator so file commands and terminal
        ``cd`` use the same ``/c/...`` representation.
        """
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.fan-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".fan-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.fan-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.fan-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        JSON/YAML/TOML candidate content is syntax-checked before anything
        touches disk. After a successful write, runs a post-first / pre-lazy
        lint check via
        ...
        # Fail closed before mkdir/temp-file/rename.  The previous behavior
        # wrote malformed structured data and only reported a lint warning
        # afterward, so callers could observe a successful file mutation.
        ext = os.path.splitext(path)[1].lower()
        inproc_linter = (
            LINTERS_INPROC.get(ext)
        ...
        # clean the temp up on any failure so we never leak a ``.fan-tmp``
    ...
    def _search_with_grep(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # Exclude hidden directories (matching ripgrep's default behavior).
        # This prevents searching inside .git and other hidden directories.
