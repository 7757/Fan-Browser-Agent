#!/usr/bin/env python3
"""
Skills Guard — Security scanner for local skill content.

It uses regex-based static analysis to detect known-bad patterns
...
Trust levels:
  - builtin:   Ships with Fan. Never scanned, always trusted.
  - trusted:   explicitly reviewed upstream skill repositories. Caution verdicts allowed.
  - community: Everything else. Any findings = blocked unless --force.

Usage:
    from tools.skills_guard import scan_skill, should_allow_install, format_scan_report

    result = scan_skill(Path("skills/some-skill"), source="local")
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.fan/\.env|\~/\.fan/\.env',
     "fan_env_access", "critical", "exfiltration",
     "directly references Fan secrets file"),
    ...
    (r'\.fan/config\.yaml|\.fan/SOUL\.md',
     "fan_config_mod", "critical", "persistence",
     "references Fan configuration files directly"),
    ...
    # Retired credentials can still exist in user-authored files. Detection is
    # intentionally retained even though Fan no longer supports that provider.
    (r'sk-ant-[A-Za-z0-9_-]{90,}',
     "retired_provider_key_leaked", "critical", "credential_exposure",
     "possible retired provider API key in skill content"),
    (r'AKIA[0-9A-Z]{16}',
...
def scan_skill(skill_path: Path, source: str = "community") -> ScanResult:
    ...
    A skill may ship a `.skillignore` file with
...
def scan_skill_cached(
    ...
    *,
    ...
    """Return a scan plus an exact-content, exact-origin attestation."""
...
# scan.
_SKILL_IGNORE_FILENAMES = (".skillignore",)
...
def _load_skill_ignore(skill_dir: Path):
    """Build a matcher from a skill's `.skillignore`.
