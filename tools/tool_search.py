"""Progressive tool disclosure ("tool search") for Fan Agent.
...
``tool_call`` — and surfaced on demand. Core Fan tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._FAN_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    """
    try:
        from toolsets import _FAN_CORE_TOOLS
        return frozenset(_FAN_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_FAN_CORE_TOOLS``. Core tools are never deferred
    ...
    try:
        ...
        if entry is None:
            return False
        # The three browser-program tools are Fan's default model interface.
        # Keep them resident so an ordinary browser request cannot fall back to
        # the terminal. The legacy fifty-tool compatibility surface is
        # intentionally deferrable when explicitly enabled.
        if entry.toolset == "browser_program":
            return False
...
def should_activate(
    ...
    if not context_length or context_length <= 0:
        ...
        # — the cliff above which multiple model families show quality drops.
