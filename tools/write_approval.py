...
(see ``fan_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<FAN_HOME>/pending/{memory,skills}/<id>.json``
...
def get_write_mode(subsystem: str) -> str:
    ...
    if subsystem not in _SUBSYSTEMS:
        return MODE_ON
    try:
        from fan_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_fan_home() / "pending" / subsystem
...
def _interactive_approval_available() -> bool:
    """True only when the inline prompt has a channel it can actually use.

    The only inline channel _prompt_inline_memory_approval reaches is a
    registered CLI approval callback. prompt_dangerous_approval(...) with no
    callback has NO gateway round-trip — under a gateway/desktop turn it falls
    to input(), which EOFs/times out to "deny" and silently drops a legitimate
    memory write. Gateway turns resolve approvals via the per-session queue,
    which this inline path does not use, so they have no usable inline channel
    here and must STAGE (safe failure → /memory pending) rather than fast-deny.
    Scripts, cron, and background threads likewise have no callback → stage.
    """
    try:
...
def skill_pending_diff(record: Dict[str, Any]) -> str:
    ...
    if action == "edit":
        ...
        if current:
            # Use the SAME fuzzy engine _patch_skill runs on apply, so the
            # preview reflects the real match / re-indent / unescape / uniqueness
            # semantics instead of a naive str.replace — which silently renders
            # "no change" on a fuzzy-but-not-exact match, or shows all-N replaced
            # on an ambiguous match that apply will actually reject.
            from tools.fuzzy_match import fuzzy_find_and_replace
            new, _mc, _strat, err = fuzzy_find_and_replace(
                current, old_s, new_s, payload.get("replace_all", False)
            )
            if err:
                return f"(patch will NOT apply cleanly: {err})"
        else:
