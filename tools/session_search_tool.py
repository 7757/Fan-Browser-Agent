...
the get_anchored_view / get_messages_around primitives in fan_state.
...
# Third-party integrations tag their sessions with FAN_SESSION_SOURCE=tool
# so they don't clutter the user's session history.
_HIDDEN_SESSION_SOURCES = ("tool",)

# Scheduled jobs can produce a large, repetitive corpus. Keep them searchable,
# but rank human conversations first so cron summaries cannot crowd the user's
# own history out of a small discover result set.
...
def _order_for_recall(raw_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Stable-sort automation below interactive session results."""
    return sorted(
        raw_results,
        key=lambda result: 1
        if (result.get("source") or "") in _DEMOTED_SESSION_SOURCES
        else 0,
    )
...
def session_search(
    ...
    Scroll wins over read/discovery when an anchor is set — the agent has
    asked for a specific slice.
    """
    if db is None:
        try:
...
def check_session_search_requirements() -> bool:
    ...
    try:
        from fan_state import DEFAULT_DB_PATH
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "SOURCE-FIRST LIMIT\n\n"
        "  This tool searches Fan conversation history only; it is not evidence "
        "about the current contents of an external source. When the user provides "
        "a URL, contact, app/thread, file path, account, website, or live system, "
        "inspect that original source first when it is accessible. Use session_search "
        "only as secondary context for what was previously said. If the source is "
        "inaccessible, explain that before falling back to history, and never conclude "
        "'not found' or 'no prior correspondence' from session_search alone.\n\n"
        "FOUR CALLING SHAPES\n\n"
        ...
        "(`deploy*`).\n\n"
        "WHEN TO USE\n\n"
        "  Reach for this when the question is about Fan conversation history itself: "
        "\"what did we do about X\", \"where did we leave Y\", or \"find the session "
        "where Z\". If the user supplied a direct source identifier, inspect that source "
        "first when accessible; session_search can then provide historical context. "
        "The session DB records what was said when, while source tools show current state."
    ),
    "parameters": {
        "type": "object",
        "properties": {
