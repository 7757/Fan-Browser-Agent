...
~/.fan/skills/. Existing skills (bundled or user-created)
...
# Agent-created skills are scanned when skills.guard_agent_created is on.
try:
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via config.yaml: skills.guard_agent_created: true.
    """
    try:
...
# All skills live in ~/.fan/skills/ (single source of truth)
...
def _is_path_redirect(path: Path) -> bool:
    ...
    ``shutil.rmtree`` to content outside the skills root. ``is_junction`` only
    exists on Python 3.12+ Windows; gate with ``hasattr``.
    """
    try:
...
def _validate_delete_target(skill_dir: Path) -> Optional[str]:
    ...
    parent discovered by walking the skills roots, so the agent can't inject an
    arbitrary path; this is defense-in-depth. Even if discovery or a poisoned
    tree hands us a bad directory, never recursively delete:
      ...
      3. a directory reached via a symlink / junction (rmtree would follow it
         into content outside the skills tree).
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"unpinning it in config.yaml (curator.pinned) if they want to delete it. "
...
def _curator_consolidation_delete_guard(
    ...
    """Refuse unverified deletes made by the background curator.

    A curator consolidation may remove a source skill only after declaring the
    existing umbrella that absorbed it.  The deterministic inactivity sweep
    archives stale skills directly through ``skill_usage.archive_skill``; it
    must never reach ``skill_manage(delete)``.  Failing closed here prevents a
    hallucinated curator decision from permanently deleting an active skill.
    """
    try:
    ...
    return {
        ...
        "error": (
            ...
            "consolidation pass may only archive a skill after its content "
            "has been absorbed into an existing umbrella. Pass "
            "absorbed_into=<umbrella>; unverified pruning is not permitted."
        ),
        "_fail_closed": True,
    }
...
def _background_review_write_guard(
    ...
    """Refuse autonomous curator mutations of externally-owned skills.

    This is intentionally narrower than normal foreground tool behavior:
    when a user asks to edit their external skill, that remains allowed.  The
    autonomous background curator must only mutate local Fan-managed content.
    """
    try:
        from tools.skill_provenance import is_background_review
    ...
    # A pinned skill is intentionally protected from every autonomous curator
    # mutation.  Foreground edits remain user-directed, but a background
    # review has no user present to consent to changing a pinned skill.
    try:
        from tools import skill_usage
        ...
        if is_external_skill_path(skill_dir):
            return {
                ...
                "error": (
                    ...
                    "the skill is in skills.external_dirs and is externally owned."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if skill_usage.is_protected_builtin(name):
            return {
                "success": False,
                "error": f"Refusing background curator {action} for protected built-in skill '{name}'.",
            }
        if skill_usage.is_bundled(name):
            return {
                "success": False,
                "error": f"Refusing background curator {action} for bundled skill '{name}'.",
            }
    except Exception:
        logger.debug("skill ownership guard lookup failed for %s", name, exc_info=True)
    return None


def _background_review_preflight(action: str, name: str) -> Optional[Dict[str, Any]]:
    """Catch background-review writes before approval staging or dispatch."""
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.fan/skills/) first, then any
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    # Check for name collisions across all local and configured skill directories.
    existing = _find_skill(name)
    if existing:
        return {
            "success": False,
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Validate an explicitly declared consolidation target before deleting.
    if is_consolidation:
    ...
    # Defense-in-depth before the recursive delete:
    # never let a poisoned tree redirect rmtree outside the skills root.
    unsafe = _validate_delete_target(skill_dir)
    if unsafe:
        return {"success": False, "error": unsafe}

    # Curator work must remain recoverable.  Foreground deletes retain their
    # existing explicit hard-delete semantics, but a background consolidation
    # is archived so a mistaken umbrella merge can be restored.
    try:
        from tools.skill_provenance import is_background_review
...
def apply_skill_pending(payload: Dict[str, Any], origin: str = "foreground") -> str:
    """Replay a staged skill write, bypassing the gate. Returns the tool result
    JSON string. Called by the /skills approve handler.

    Restores the staged write's origin into the skill_provenance ContextVar so
    post-success curator telemetry (mark_agent_created) sees the real provenance
    instead of the approval thread's default 'foreground'. Without this, an
    agent-self-created skill approved under skills.write_mode=approve loses its
    created_by="agent" mark and the curator never auto-manages it.
    """
    from tools.skill_provenance import set_current_write_origin, reset_current_write_origin
    token = _skill_gate_bypass.set(True)
    origin_token = set_current_write_origin(origin or "foreground")
    try:
        return skill_manage(
...
def skill_manage(
    ...
    # is itself replaying an already-approved staged write (apply_skill_pending).
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_fan_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `fan curator unpin <name>`. "
