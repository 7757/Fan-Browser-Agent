...
Schema shapes they accept. Schemas that most cloud
...
* ``anyOf`` / ``oneOf`` unions whose only purpose is to permit ``null`` for
  optional fields (common Pydantic/MCP shape). Strict tool-schema validators
  can reject these; collapse them to the non-null branch.
...
def _strip_ref_siblings(node: Any) -> Any:
    ...
    Strict draft-07 backends (Fireworks-hosted Kimi, etc.) fail tool requests
    with "keyword(s) ['default'] not allowed at the same level as $ref".
    ...
    *,
    ...
    Args:
        ...
        keep_nullable_hint: If True, set ``nullable: true`` on the replacement
            ...
            literal string ``"null"`` to Python ``None``). Many validators
            accept ``nullable: true`` but strict producers may
            prefer False.
...
def _sanitize_node(node: Any, path: str) -> Any:
    ...
    for key, value in node.items():
        # Several tool-call backends reject JSON Schema ``type`` arrays, but
        # collapsing a meaningful union to its first type silently changes the
        # tool contract. Keep a single non-null type as ``type: X`` and express
        # multiple non-null types as an ``anyOf`` of singular schemas.
        if key == "type" and isinstance(value, list):
            ...
            if len(non_null) == 1:
                ...
                continue
            if len(non_null) >= 2:
                out["anyOf"] = [{"type": item_type} for item_type in non_null]
                if has_null:
                    out.setdefault("nullable", True)
                continue
            # An all-null union is still a valid, meaningful type. Empty or
            # malformed arrays retain the permissive object fallback.
...
def strip_pattern_and_format(tools: list[dict]) -> tuple[list[dict], int]:
    ...
    ``\\w``, ``\\s`` and most ``format`` values. Cloud providers generally
    accept these keywords and rely on
