...
This module provides the glue:
    - ``FanTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Fan Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__fan_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses FAN_HOME so each profile gets its own OAuth tokens.
    Layout: ``FAN_HOME/mcp-tokens/``
    """
    try:
        from fan_constants import get_fan_home
        base = Path(get_fan_home())
    except ImportError:
        base = Path(os.environ.get("FAN_HOME", str(Path.home() / ".fan")))
...
def suppress_interactive_oauth():
    """Prevent background MCP discovery from reading stdin for OAuth.

    A ContextVar follows the discovery context into the coroutine scheduled on
    Fan's dedicated MCP event-loop thread.
    """
...
def _raise_if_non_interactive(lead: str) -> None:
    """Fail at an OAuth browser boundary when no user can complete it."""
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `fan mcp login <server>` interactively to "
...
# ---------------------------------------------------------------------------
# FanTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class FanTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        FAN_HOME/mcp-tokens/<server_name>.json         -- tokens
        FAN_HOME/mcp-tokens/<server_name>.client.json   -- client info
        FAN_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Fan records an absolute wall-clock ``expires_at`` alongside the
    ...
    def poison_client_registration(self) -> bool:
        """Discard a rejected dynamic OAuth client registration.

        Tokens deliberately stay intact.  Only the cached client registration
        and discovery metadata are removed so the next SDK authorization flow
        re-runs dynamic registration instead of repeatedly submitting a client
        the identity provider has deleted.
        """
        ...
        try:
            ...
            # A backup failure must not turn a provably stale client into a
            # permanent outage; the caller's detection path is best-effort.
            logger.warning("Could not back up MCP OAuth client info at %s: %s", client_path, exc)
        client_path.unlink(missing_ok=True)
        self._meta_path().unlink(missing_ok=True)
        logger.warning(
            "MCP OAuth '%s': cached client registration was rejected; "
            "removed client metadata to force re-registration (backup: %s)",
            self._server_name,
            backup.name,
        )
        return True
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Fan.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    """
    # A cached token may exist but be expired or revoked. In that case the SDK
    # reaches this authorization-code flow even though build_oauth_auth's
    # token-file check passed. Do not open a browser or wait for a callback in
    # a background gateway/cron context.
    _raise_if_non_interactive(
        "MCP OAuth requires browser authorization but no interactive session "
        "is available (non-interactive/background context)."
    )
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    # Reaching the callback means refresh fell through to an auth-code flow.
    # Fail before binding a listener or waiting the full OAuth timeout when a
    # cached-but-unusable token is encountered in a non-interactive process.
    _raise_if_non_interactive(
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. OAuth-protected MCP servers are not supported in this build; "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = FanTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Fan Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        # Fail fast instead of warning + proceeding into an OAuth flow that needs
        # a browser — under a headless gateway turn that would otherwise hang /
        # fail obscurely mid-connect.
        raise OAuthNonInteractiveError(
            "MCP OAuth for "
            f"'{server_name}': non-interactive environment and no cached tokens "
            "found. The OAuth flow requires browser authorization. Run "
            f"OAuth for {server_name} cannot be completed in this build — "
            "remove the server from config.yaml or use a non-OAuth server."
        )
