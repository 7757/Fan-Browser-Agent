...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    """Trim line-numbered read output without discarding the whole page.

    ``read_file`` uses line offsets for pagination, while the model-facing
    safety guard is character based.  A page containing a few very long lines
    used to exceed the guard and return no content at all.  Keep complete
    lines that fit, then let the caller expose a continuation offset.  When
    the first line alone exceeds the budget, clamp it on a Python code-point
    boundary so the result is still useful rather than empty.
    """
...
def _expand_tilde(path: str | os.PathLike[str]) -> str:
    """Expand the current profile's ``~`` exactly as tool subprocesses do."""
    raw = os.fspath(path)
...
def _live_cwd_if_owned(env, task_id: str) -> str | None:
    """Return the shared terminal env's cwd only for its owning session.

    CWD-only desktop sessions intentionally share the ``default`` terminal
    environment.  Its live cwd therefore belongs to whichever session most
    recently ran a command.  ``terminal_tool`` stamps ``cwd_owner`` before
    execution; rejecting another session's stamp prevents relative file tools
    from being routed into the wrong workspace.
    """
    if env is None:
        return None
    ...
    task = str(task_id or "")
    if owner and task and owner != "default" and task != "default" and owner != task:
        return None
    return live
...
def _get_live_tracking_cwd(task_id: str = "default") -> str | None:
    ...
    if cached is not None:
        ...
        if live_cwd:
            _remember_last_known_cwd(task_id, live_cwd)
            return live_cwd
        if env is None and getattr(cached, "cwd", None):
            legacy_cwd = getattr(cached, "cwd", None)
            _remember_last_known_cwd(task_id, legacy_cwd)
            return legacy_cwd
...
def _authoritative_workspace_root(task_id: str = "default") -> str | None:
    ...
    if live:
        return live
    # A session's currently registered workspace is an explicit user/UI
    # choice. It must win over an older preserved cwd when a worktree or
    # workspace has changed while the terminal environment was idle.
    registered = _registered_task_cwd_override(task_id)
    if registered:
        return registered
    # The idle reaper may have removed the live terminal environment before a
    # subsequent relative file operation. Keep the last directory observed for
    # this same session so resolution does not silently fall back to the app
    # process cwd. This is deliberately session-keyed: CWD-only sessions share
    # a local environment, but must never inherit one another's directories.
...
def _resolve_base_dir(task_id: str = "default") -> Path:
    ...
    if root:
        # On Windows a terminal launched from Git Bash/Cygwin can report
        # /c/... or /cygdrive/c/... even though file APIs require C:\\....
        # Normalize before Path decides whether this is absolute.
        from tools.environments.local import _msys_to_windows_path
        base = Path(_expand_tilde(_msys_to_windows_path(str(root))))
    else:
        base = Path(os.getcwd())
...
def _resolve_path_for_task(filepath: str, task_id: str = "default") -> Path:
    ...
    p = Path(_expand_tilde(_msys_to_windows_path(filepath)))
    if p.is_absolute():
        return p.resolve()
...
def _path_resolution_warning(filepath: str, resolved: Path, task_id: str = "default") -> str | None:
    ...
    try:
        ...
        if Path(_expand_tilde(_msys_to_windows_path(filepath))).is_absolute():
            return None
        workspace_root = _authoritative_workspace_root(task_id)
        if not workspace_root:
            return None  # No authoritative workspace root to compare against.
        root = Path(
            _expand_tilde(_msys_to_windows_path(str(workspace_root)))
        ).resolve()
...
def _is_blocked_device(
    ...
    # Inspect every explicit symlink target before realpath collapses aliases
    # such as /dev/stdin into a terminal-specific /proc fd path. A bounded,
    # cycle-aware walk keeps malicious link graphs from hanging the guard.
    seen: set[str] = set()
    current = normalized
    for _ in range(20):
        try:
...
_fan_config_resolved: str | None = None
_fan_config_resolved_loaded = False


def _get_fan_config_resolved() -> str | None:
    """Return the resolved absolute path of the Fan config file (cached)."""
    global _fan_config_resolved, _fan_config_resolved_loaded
    if _fan_config_resolved_loaded:
        return _fan_config_resolved
    _fan_config_resolved_loaded = True
    try:
        from fan_cli.config import get_config_path
        _fan_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _fan_config_resolved = str(Path(_expand_tilde("~/.fan/config.yaml")).resolve())
        except Exception:
            _fan_config_resolved = None
    return _fan_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Shared Fan authority-state boundary. In addition to config.yaml this
    # covers other durable files that can become effective runtime
    # configuration on a later load.
    if is_write_denied(resolved):
        return (
            f"Refusing to write to protected Fan/system state: {filepath}\n"
            "Agent file tools cannot modify authentication or execution-policy state. "
            "Use a separate user-controlled shell or the settings UI."
        )
    # Prevent agents from modifying the Fan config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    fan_config = _get_fan_config_resolved()
    if fan_config and (resolved == fan_config or normalized == fan_config):
        return (
            f"Refusing to write to Fan config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.fan/config.yaml directly or use 'fan config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Container-side Fan mirror prefix — always ``None`` now.

    Only the Docker backend created a ``/root/.fan`` mirror; remote/container
    backends were removed, so there is never a mirror prefix to guard.
    """
    return None


def _check_fan_state_mirror_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in a sandbox mirror
    of authoritative Fan state.

    Two detectors run in order:

    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.fan/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Fan process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.fan/…`` path.

    Returns ``None`` when the write is in-scope or outside Fan state. This is
    defense-in-depth, NOT a security boundary — the terminal tool runs as the
    same OS user and can write any of these paths directly. See
    ...
    # Resolve via the task's cwd so relative paths are classified against
    # the right base.
    try:
        resolved = str(_resolve_path_for_task(filepath, task_id))
    except (OSError, ValueError):
...
# Per-session last-known CWD. Kept independently of the shared local terminal
# environment so cleanup/recreation cannot redirect relative file operations.
_last_known_cwd: dict[str, str] = {}


def _remember_last_known_cwd(task_id: str, cwd: str | None) -> None:
    """Persist a safe last-known working directory for one logical session."""
    if not cwd:
        return
    expanded = _expand_tilde(str(cwd))
    if not os.path.isabs(expanded):
        return
    with _file_ops_lock:
        _last_known_cwd[str(task_id or "default")] = expanded


def _last_known_cwd_for(task_id: str = "default") -> str | None:
    """Return the preserved CWD for this exact session, if one exists."""
    with _file_ops_lock:
        return _last_known_cwd.get(str(task_id or "default"))
...
def _looks_like_read_file_line_numbered_content(content: str) -> bool:
    """Detect a read_file ``LINE_NUM|CONTENT`` display echoed into a write."""
    if not isinstance(content, str):
        return False
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    if cached is not None:
        with _env_lock:
            if task_id in _active_environments:
                ...
                # The environment was cleaned up. Preserve its CWD only when
                # it belongs to this raw session; the shared local environment
                # may otherwise still describe another desktop session.
                old_cwd = _live_cwd_if_owned(
                    getattr(cached, "env", None), raw_task_id
                )
                if old_cwd:
                    _remember_last_known_cwd(raw_task_id, old_cwd)
                with _file_ops_lock:
    ...
    with task_lock:
        ...
        if terminal_env is None:
            ...
            cwd = (
                overrides.get("cwd")
                or _last_known_cwd_for(raw_task_id)
                or config["cwd"]
            )
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        if is_extractable_document(str(_resolved)):
            try:
                ...
                if content_len > max_chars:
                    ...
                    result_dict["hint"] = (
                        f"Output reached the {max_chars:,}-character read budget "
                    ...
                    if len(trimmed.split("\n", 1)[0]) >= max_chars:
                        result_dict["hint"] += (
                            " The first line exceeded the budget and was clamped "
                            "mid-line; its remainder cannot be recovered by offset."
                        )
        ...
        # ── Fan internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under FAN_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == FAN_HOME (e.g. "auth.json") still hits the
        ...
        if content_len > max_chars:
            ...
            result_dict["next_offset"] = next_offset
            result_dict["hint"] = (
                f"Output reached the {max_chars:,}-character read budget after "
                f"{lines_kept} line(s) (showing lines {offset}-{shown_end} of "
                f"{total_lines}). Use offset={next_offset} to continue."
            )
            if len(trimmed.split("\n", 1)[0]) >= max_chars:
                result_dict["hint"] += (
                    " The first line exceeded the budget and was clamped mid-line; "
                    "its remainder cannot be recovered by offset."
                )
...
def write_file_tool(path: str, content: str, task_id: str = "default") -> str:
    ...
    Sandbox-mirror guardrails prevent writes to per-task copies of Fan state
    that the host process will not read.
    """
    sensitive_err = _check_sensitive_path(path, task_id)
    if sensitive_err:
        return tool_error(sensitive_err)
    mirror_warning = _check_fan_state_mirror_path(path, task_id)
    if mirror_warning:
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    if mode == "patch" and patch:
        ...
        def _check_v4a_path(v4a_path: str) -> str | None:
            ...
            if has_traversal_component(v4a_path):
                return tool_error(
                    ...
                    "path in an Update, Add, Delete, or Move File header."
                )
            return None

        # Keep this parser aligned with the V4A parser itself: it accepts a
        # no-space ``***Update File:`` header, which must not bypass the
        # sensitive-path guard here.
        for _m in _re.finditer(r'^\*\*\*\s*(?:Update|Add|Delete)\s+File:\s*(.+)$', patch, _re.MULTILINE):
            v4a_path = _m.group(1).strip()
            path_error = _check_v4a_path(v4a_path)
            if path_error:
                return path_error
            _paths_to_check.append(v4a_path)
        # V4A Move has two filesystem targets.  Validate both so neither a
        # source traversal nor a sensitive destination can bypass the guard.
    ...
    for _p in _paths_to_check:
        ...
        if sensitive_err:
            return tool_error(sensitive_err)
        mirror_warning = _check_fan_state_mirror_path(_p, task_id)
        if mirror_warning:
            return tool_error(mirror_warning)
    try:
...
def search_tool(pattern: str, target: str = "content", path: str = ".",
                ...
                task_id: str = "default", densify: bool = False) -> str:
    ...
    try:
        ...
        result_dict = result.to_dict(densify=densify)
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with fan_tools.write_file() for very "
            "large files."
        )
...
def _handle_search_files(args, **kw):
    ...
    return search_tool(
        ...
        # Direct tool results go back into model context and benefit from the
        # compact form. execute_code's fan_tools API promises structured
        # ``matches`` and marks its nested calls accordingly.
        densify=kw.get("result_mode") == "model",
    )
