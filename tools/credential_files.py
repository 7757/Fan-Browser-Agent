...
try:  # pragma: no cover - exercised through the fail-closed tests
    from agent.file_safety import get_read_block_error
except ImportError:  # Keep a visible fail-closed sentinel for partial installs.
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.fan",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``FAN_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside FAN_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    fan_home = _resolve_fan_home()

    # Reject absolute paths — they bypass the FAN_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to FAN_HOME)",
            relative_path,
        )
        return False

    host_path = fan_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape FAN_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, fan_home)
    ...
    # FAN_HOME contains the application's master credential stores. Path
    # containment blocks traversal outside FAN_HOME, but it does not stop a
    # skill from requesting .env, auth.json, or an MCP token by its valid
    # relative name. Reuse the canonical agent read deny-list so the mount
    # surface cannot grant a sandbox access that read_file itself denies.
    ...
    if denied:
        logger.warning(
            "credential_files: refused %r — it is a protected credential "
            "store; a skill may mount its own service token, not Fan's "
            "master credentials",
            relative_path,
        )
        return False
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="fan-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching fan_constants.get_fan_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_fan_dir()`` for backward compatibility with old directory layouts.
    """
    from fan_constants import get_fan_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_fan_dir(new_subpath, old_name)
        if host_dir.is_dir():
