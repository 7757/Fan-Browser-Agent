...
running under the dispatcher (env var ``FAN_KANBAN_TASK`` set) or when
the current configuration explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``fan chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``fan kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``fan kanban complete …``
   inside the container, where ``fan`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.fan/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``fan kanban …``), the dashboard
(``fan dashboard``), and the slash command (``/kanban …``) — all
three bypass the agent entirely. The tools are for dispatcher-spawned
worker handoffs and for configured orchestrator sessions that route work
through the board.
"""
...
def _config_has_kanban_toolset() -> bool:
...
def _check_kanban_mode() -> bool:
    ...
    1. ``FAN_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current configuration has ``kanban`` in its toolsets config.

    Humans running ``fan chat`` without the kanban toolset see zero
    kanban tools. Workers spawned by the kanban dispatcher (gateway-
    embedded by default) and orchestrator sessions with the kanban
    toolset enabled see the Kanban lifecycle tool surface.
    """
    if os.environ.get("FAN_KANBAN_TASK"):
        return True
    return _config_has_kanban_toolset()
...
def _check_kanban_orchestrator_mode() -> bool:
    ...
    board state. Sessions that explicitly opt into the kanban toolset
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("FAN_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("FAN_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("FAN_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``FAN_KANBAN_TASK`` set
    ...
    Orchestrator sessions (kanban toolset enabled but **no**
    ``FAN_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``FAN_KANBAN_DB`` → ``FAN_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets callers override the env-pinned
    active board without restarting Fan.
    """
...
def _goal_judge_available() -> bool:
    """Whether completion can safely enforce the goal-mode judge gate.

    ``judge_goal`` intentionally reports ``continue`` when no auxiliary model
    is available.  Treating that as a real rejection would leave every
    goal-mode worker unable to close its own card during an auxiliary outage,
    so the gate is deliberately fail-open until a usable judge is resolved.
    """
    try:
...
def _current_invocation_session():
    """Return the context-local desktop invocation, if one is bound."""
    try:
        from fan_cli.invocation_context import get_current_invocation_session

        return get_current_invocation_session()
    except Exception:
        return None


def _maybe_auto_subscribe(kb, conn, task_id: str) -> bool:
    """Best-effort subscribe the calling desktop conversation to ``task_id``.

    The durable conversation key comes exclusively from a ContextVar bound by
    the desktop backend. We deliberately do not infer it from process-global
    environment variables: concurrent desktop conversations share one Python
    process, and a stale environment value would route a task update to the
    wrong user conversation. CLI/cron/detached calls have no bound desktop
    return channel and remain unsubscribed.

    Subscription bookkeeping must never turn a successful ``kanban_create``
    into a failed tool call. Any failure is logged and surfaced only through
    the response's ``subscribed`` flag.
    """
    try:
        from fan_cli.config import load_config
        ...
        if kanban_cfg.get("auto_subscribe_on_create", True) is False:
            return False

        invocation = _current_invocation_session()
        if invocation is None or not invocation.session_key:
            return False
        ...
        kb.add_notify_sub(
            ...
            task_id=task_id,
            platform="desktop",
            chat_id=invocation.session_key,
        )
        return True
    except Exception as exc:
        logger.warning(
            "kanban_create succeeded but desktop notification subscription "
            "failed for task %s: %s",
            task_id,
            exc,
        )
...
#   - No-op outside dispatcher-spawned worker context (no ``FAN_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``FAN_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``FAN_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``FAN_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("FAN_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("FAN_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("FAN_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set FAN_KANBAN_TASK in the env)"
        )
...
def _handle_complete(args: dict, **kw) -> str:
    ...
    if artifacts is not None:
        ...
        # Carry the artifact list inside metadata so it rides the existing
        # completed-event payload and remains available to local task views.
        if artifacts:
            if metadata is None:
                metadata = {}
    ...
    if isinstance(metadata, dict):
        try:
            redacted_metadata = redact_sensitive_text(
                json.dumps(metadata, ensure_ascii=False),
                force=True,
            )
            metadata = json.loads(redacted_metadata)
        except (TypeError, ValueError, json.JSONDecodeError):
            return tool_error("metadata could not be safely redacted for persistence")
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
            # A goal-mode worker can call kanban_complete directly before its
            # outer loop gets to evaluate the response.  Keep the acceptance
            # decision at this write boundary as well, but fail open when the
            # inexpensive auxiliary judge is not actually available.
            task = kb.get_task(conn, tid)
            if task and task.goal_mode and _goal_judge_available():
                verdict = "done"
                reason = ""
                try:
                    from fan_cli.goals import judge_goal
                    ...
                    logger.warning(
                        "goal completion judge failed; allowing completion: %s",
                        judge_exc,
                        exc_info=True,
                    )
                if verdict != "done":
                    return tool_error(
                        ...
                        f"Provide explicit acceptance evidence in the summary, "
                        f"or create continuation tasks with parents=[{tid}] "
                        "and keep this task active."
                    )
            try:
                ok = kb.complete_task(
                    conn, tid,
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # FAN_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``fan-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("FAN_KANBAN_AUTHOR") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    if not assignee:
        return tool_error(
            "assignee is required — name the assignee lane that should execute this "
            "task (the dispatcher will only spawn tasks with an assignee)"
        )
    body = args.get("body")
    parents = args.get("parents") or []
    tenant = args.get("tenant") or os.environ.get("FAN_TENANT")
    # Stamp the durable originating conversation. Desktop sessions use a
    # ContextVar because several conversations execute concurrently inside the
    # same gateway process; the env fallback remains only for legacy
    # single-session CLI/ACP callers.
    invocation = _current_invocation_session()
    session_id = (
        args.get("session_id")
        or (invocation.session_key if invocation is not None else None)
        or os.environ.get("FAN_SESSION_ID")
    )
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (FAN_KANBAN_TASK set)
    ...
    # (kanban toolset, no FAN_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("FAN_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("FAN_KANBAN_AUTHOR") or "worker",
                session_id=session_id,
            )
            new_task = kb.get_task(conn, new_tid)
            subscribed = _maybe_auto_subscribe(kb, conn, new_tid)
            return _ok(
                task_id=new_tid,
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to FAN_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: FAN_KANBAN_DB env → "
    "FAN_KANBAN_BOARD env → the 'current' symlink under the kanban "
    "home → 'default'. Pass an explicit slug only when the caller "
    "needs to override the env-pinned active "
...
KANBAN_LIST_SCHEMA = {
    "name": "kanban_list",
    "description": (
        "List Kanban task summaries so an orchestrator session can discover "
...
KANBAN_COMPLETE_SCHEMA = {
    ...
    "description": (
        ...
        "in ``artifacts`` so the local task record keeps the deliverable "
        "paths alongside the completion summary."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "created_cards": {
                ...
                "description": (
                    ...
                    "created by this worker's assignee lane; any phantom "
            ...
            "artifacts": {
                ...
                "description": (
                    ...
                    "\"/tmp/report.pdf\"]. These paths are stored with "
                    "the local task result and can be opened from the "
                    "workspace. Skip "
                    "intermediate scratch files and references that "
                    "are not the deliverable. The path must exist "
                    "on disk when the task completes."
                ),
            },
            "board": _board_schema_prop(),
        },
...
KANBAN_BLOCK_SCHEMA = {
    ...
    "description": (
        ...
        "Use kind='dependency' when another task must finish (it returns to "
        "todo and auto-resumes); use 'needs_input', 'capability', or "
        "'transient' for a genuine human-visible blocker. Repeated same-cause "
        "blocks are escalated to triage instead of looping forever."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "reason": {
                ...
                "description": (
                    ...
                    "two sentences. Don't paste the whole conversation."
                ),
            },
            "kind": {
                "type": "string",
                ...
                "description": (
                    "Why work is blocked. Dependency waits resume through the "
                    "board; the other kinds request human attention."
                ),
            },
            "board": _board_schema_prop(),
        },
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "assignee": {
                "type": "string",
                "description": (
                    "Assignee lane that should execute this task "
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to FAN_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
            ...
            "triage": {
                ...
                "description": (
                    ...
                    "— a specifier lane is expected to flesh out "
            ...
            "skills": {
                ...
                "description": (
                    ...
                    "The names must match skills available on the "
                    "assignee lane."
                ),
            },
            "goal_mode": {
...
KANBAN_UNBLOCK_SCHEMA = {
    ...
    "description": (
        ...
        "sessions with the kanban toolset can unblock routed work; "
