...
# (FAN_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_fan_home() / "memories"
...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        original text so the user can still inspect poisoned entries directly
        in ``MEMORY.md`` / ``USER.md`` and remove them with a unique exact
        substring — silently dropping them would hide the attack from the user.
        ...
        for entry in entries:
            ...
            if findings:
                ...
                sanitized.append(
                    ...
                    f"inspect the source file directly, then use "
                    f"memory(action=remove) with a unique exact substring "
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            # For add (append-only), skip the drift guard — appending never
            # clobbers existing content. The guard remains active for
            # replace/remove, where a full-file rewrite could discard content.
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        # XML 角括号标签包裹运行时数据块(对齐 的结构化写法,取代 ═══ 分隔线 + 自由文本头)。
        if target == "user":
            tag, note = "user_profile", "who the user is"
        else:
            tag, note = "memory", "your persistent notes"
        usage = f"{pct}% used — {current:,}/{limit:,} chars"
        return f'<{tag} note="{note}" usage="{usage}">\n{content}\n</{tag}>'
...
def _missing_old_text_error(
    store: "MemoryStore",
    target: str,
    action: str,
) -> str:
    """Return current entries so a dropped optional field is recoverable."""
    entries = store._entries_for(target)
    return json.dumps(
        {
            "success": False,
            "error": (
                f"'{action}' requires old_text: a short unique substring of "
                f"the entry to {action}. Reissue the call using one of the "
                "current_entries below."
            ),
            "current_entries": entries,
            "usage": f"{store._char_count(target):,}/{store._char_limit(target):,}",
        },
        ensure_ascii=False,
    )
...
def load_on_disk_store() -> MemoryStore:
    """Load a config-sized memory store for commands without a live agent."""
    try:
        from fan_cli.config import load_config

        memory_config = load_config().get("memory") or {}
...
def memory_tool(
    ...
    """
    if store is None:
        return tool_error("Memory is not available in this environment.", success=False)
    ...
    # in that commit is N/A to our tri-state write_approval.)
...
MEMORY_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "old_text": {
                "type": "string",
                "description": "REQUIRED for 'replace' and 'remove': a short unique substring identifying the existing entry. Omit only for 'add'."
            },
        },
