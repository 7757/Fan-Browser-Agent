...
from fan_constants import get_fan_home, get_skills_dir, display_fan_home
import os
import re
...
# All skills live in ~/.fan/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits and bundled
# skills all coexist here without polluting the git repo.
FAN_HOME = get_fan_home()
SKILLS_DIR = FAN_HOME / "skills"

# Skill discovery reads every SKILL.md recursively.  Cache the metadata for a
# short interval, but make its validity depend on the local/external directory
# topology, the config-driven disabled set, and the active platform.  Returning
# copies is deliberate: web/API callers annotate entries for display and must
# never mutate the shared cache.
_SKILLS_CACHE: Dict[str, Tuple[Tuple[Any, ...], float, List[Dict[str, Any]]]] = {}
_SKILLS_CACHE_TTL_SECONDS = 30.0
_SKILLS_CACHE_KEY_DISABLED = "with_disabled"
_SKILLS_CACHE_KEY_FILTERED = "filtered"

# Limits chosen for progressive-disclosure efficiency
MAX_NAME_LENGTH = 64
MAX_DESCRIPTION_LENGTH = 1024

_ENV_VAR_NAME_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")
# Thread-local (not a module global): each gateway turn wires its secret-capture
# callback on its own run() thread, and the skill-setup that reads it runs on
# that same thread. A process-global would let concurrent multi-session turns
# route one session's secret prompt to another session's UI.
_cb_tls = threading.local()


def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from FAN_HOME/.env."""
    env_path = get_fan_home() / ".env"
...
def set_secret_capture_callback(callback) -> None:
    """Thread-local: scoped to the calling turn's thread (see _cb_tls)."""
    _cb_tls.secret = callback
...
def _capture_required_environment_variables(
    ...
    # Non-interactive local workers cannot present a secret prompt. Interactive
    # desktop/TUI clients set FAN_INTERACTIVE and register a secret-capture
    # callback that routes to a secure secret.request overlay.
    if _is_gateway_surface() and not env_var_enabled("FAN_INTERACTIVE"):
        return {
    ...
    secret_cb = getattr(_cb_tls, "secret", None)
    if secret_cb is None:
        return {
            "missing_names": missing_names,
            "setup_skipped": False,
            "gateway_setup_hint": None,
...
def _is_gateway_surface() -> bool:
    return bool(os.environ.get("FAN_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_fan_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.fan/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``FAN_SESSION_PLATFORM``.
    """
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``FAN_PLATFORM`` environment variable
    3. ``FAN_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from fan_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("FAN_PLATFORM") or _get_session_platform()
...
def _skills_scan_signature(
    dirs_to_scan: List[Path], disabled: Set[str]
) -> Tuple[Any, ...]:
    """Return a cheap signature for skill-discovery inputs.

    A root directory's mtime alone misses additions/removals inside category
    subdirectories.  Include every root and its immediate child-directory
    mtimes, then use the short TTL below to bound staleness for in-place
    SKILL.md edits.  Disabled names and platform are non-filesystem inputs and
    must therefore be part of the signature as well.
    """
    try:
    ...
    dirs_signature: List[Tuple[str, int]] = []
    for directory in dirs_to_scan:
        try:
            newest = directory.stat().st_mtime_ns
        except OSError:
            continue
        try:
            ...
            pass
        dirs_signature.append((str(directory), newest))

    return (tuple(dirs_signature), frozenset(disabled), platform)


def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.fan/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``fan skills`` config UI). Default False
    ...
    # Load disabled names once: changing the config must invalidate the cache
    # even when no skill-directory mtime changes.
    disabled = set() if skip_disabled else _get_disabled_skill_names()

    # Resolve the local skills dir on each call.  FAN_HOME can be context-local
    # for profile-scoped gateway/cron work, so the import-time constant must not
    # decide which profile gets scanned.
    dirs_to_scan: List[Path] = []
    active_skills_dir = get_skills_dir()
    ...
    # The signature is intentionally calculated before the scan.  If a writer
    # races this traversal, the next caller gets a different signature and
    # rescans instead of trusting a torn result for the TTL.
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_fan_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from fan_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable it in config.yaml (plugins.disabled list)."
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.fan/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `fan skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.fan.* first (agentskills.io convention), fall back to top-level
        fan_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            fan_meta = metadata.get("fan", {}) or {}

        tags = _parse_tags(fan_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            fan_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
