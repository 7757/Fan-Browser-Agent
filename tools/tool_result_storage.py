...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/fan-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/fan-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/fan-results"
HEREDOC_MARKER = "FAN_PERSIST_EOF"
...
def maybe_persist_tool_result(
    ...
    # The result is large enough to persist. If no env was supplied (e.g. a
    # browser-only turn that never spun up a terminal env), lazily create one
    # via env_factory so the full output is written to a real file the model
    # can read_file — instead of falling through to the dead-end inline
    # truncation below. Verified: the local env writes fine on Windows.
    if env is None and env_factory is not None:
        try:
            env = env_factory()
        except Exception as exc:
            logger.warning("env_factory failed for %s: %s", tool_use_id, exc)
