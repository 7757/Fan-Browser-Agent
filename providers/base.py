...
def _profile_user_agent() -> str:
    """Return a ``fan-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from fan_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"fan-cli/{_ver}"
    except Exception:
        return "fan-cli"
...
class ProviderProfile:
    ...
    # multimodal models via tool results on their compatible API.
    ...
    def fetch_models(
        ...
        *,
        ...
        # fan-cli UA so the catalog endpoint is reachable.
